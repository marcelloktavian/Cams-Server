...
//inisialisasi input penjualan B2B
$id_trans=$_GET['ids'];
$sql_init="SELECT b.*,k.nama as kategori,s.nama as salesman,c.nama AS customer,a.kecamatan,e.nama AS expedition FROM b2bso b LEFT JOIN mst_b2bcategory_sale k ON b.id_kategori=k.id LEFT JOIN mst_b2bsalesman s on b.id_salesman=s.id LEFT JOIN mst_b2bcustomer c ON b.id_customer=c.id LEFT JOIN mst_address a ON b.id_address=a.id LEFT JOIN mst_b2bexpedition e ON b.id_expedition=e.id WHERE b.id_trans ='".$id_trans."' ";
...
td13.appendChild(generateDisc(baris1));
td14.appendChild(generateSUBTOTALQTY(baris1));
td15.appendChild(generateSUBTOTAL(baris1));
...
function hitungjml(a)
{
	...
	var total=0;
	var qty=0;
	qty=parseInt(qty36)+parseInt(qty37)+parseInt(qty38)+parseInt(qty39)+parseInt(qty40)+parseInt(qty41)+parseInt(qty42)+parseInt(qty43)+parseInt(qty44)+parseInt(qty45);
