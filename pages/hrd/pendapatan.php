...
$allow_add = is_show_menu(ADD_POLICY, pendapatan, $group_acess);
$allow_edit = is_show_menu(EDIT_POLICY, pendapatan, $group_acess);
$allow_delete = is_show_menu(DELETE_POLICY, pendapatan, $group_acess);
	...
	if(isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
		...
		if ($_REQUEST["_search"] == "false") {
			$where = "WHERE deleted=0 AND `type`='pendapatan' ";
			} else {
			$operations = array(
			 'eq' => "= '%s'",            // Equal
			 'ne' => "<> '%s'",           // Not equal
		   ...
		   $where = sprintf(" where deleted=0 AND `type`='pendapatan' AND %s ".$operations[$_REQUEST["searchOper"]], $_REQUEST["searchField"], $value);
        ...
        foreach($data1 as $line) {
			...
			if ($statusToko == 'Tutup') {
				...
				if($allow_edit){
					$edit = '<a onclick="javascript:popup_form(\''.BASE_URL.'pages/hrd/pendapatan.php?action=edit&id='.$line['id_penpot'].'\',\'table_tabel_pendapatan\')" href="javascript:;">Edit</a>';				
				}else{
					$edit = '<a onclick="javascript:custom_alert(\'Not Allowed\')" href="javascript:;">Edit</a>';	
				}
        	
				if($allow_delete){
					$delete = '<a onclick="javascript:link_ajax(\''.BASE_URL.'pages/hrd/pendapatan.php?action=delete&id='.$line['id_penpot'].'\',\'table_tabel_pendapatan\')" href="javascript:;">Delete</a>';
				}else{
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'process') {
		if(isset($_POST['id_penpot'])) {
			...
			$stmt = $db->prepare("INSERT INTO hrd_pendapatan_potongan(`kode_penpot`,`nama_penpot`,`metode_pethitungan`,`persentase_kehadiran`,`total_pendapatan`,`objek_pph21`,`sifat`,`type`,`type_pengaruh`,`user`,`lastmodified`) VALUES(?, ?, ?, ?, ?, ?, ?, 'pendapatan',?,?, NOW())");
...
<?php
    ...
    if ($statusToko == 'Tutup') {
		...
		if($allow_add) {
			echo '<button type="button" onclick="javascript:popup_form(\''.BASE_URL.'pages/hrd/pendapatan.php?action=add\',\'table_tabel_pendapatan\')" class="btn">Tambah</button>';
		}
	}
?>
</div>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#table_tabel_pendapatan").jqGrid({
            url:'<?php echo BASE_URL.'pages/hrd/pendapatan.php?action=json'; ?>',
            /*postData: {
            ...
            datatype: "json",
            colNames:['ID','Kode Pendapatan','Nama Pendapatan','Metode Perhitungan','Dipengaruhi Persentase Kehadiran','Mempengaruhi Total Pendapatan','Objek Pajak PPh21','Sifat','Edit','Delete'],
            colModel:[
            ...
            rowNum:20,
            rowList:[10,20,30],
            pager: '#pager_table_tabel_pendapatan',
            ...
            rownumbers: true,
            sortorder: "asc",
            caption:"Master Pendapatan",
            ondblClickRow: function(rowid) {
                alert(rowid);
            }
        });
        $("#table_tabel_pendapatan").jqGrid('navGrid','#pager_table_tabel_pendapatan',{edit:false,add:false,del:false});
    })
</script>