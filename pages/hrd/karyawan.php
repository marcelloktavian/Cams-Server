...
$allow_delete = is_show_menu(DELETE_POLICY, karyawan, $group_acess);
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'duplicate') {
		$stmt1 = $db->prepare("INSERT INTO `hrd_karyawan` (`no_karyawan`,`id_jabatan`,`nama_karyawan`,`nik`,`npwp`,`jkn_kis`,`bpjs_tk`,`no_telp`,`periode`,`tipe`,`id_ptkp`,`email`,`alamat`,`rekening`,`upah_tetap`,`up_bpjs`,`up_bpjs_tk`,`tanggungan_tambahan`,`total_pendapatan`,`total_potongan`,`total`,`user`,`lastmodified`
		  ) 
			(SELECT '','','','','','','','','','','','','','',upah_tetap,up_bpjs,up_bpjs_tk,tanggungan_tambahan,total_pendapatan,total_potongan,total,'".$_SESSION['user']['username']."',lastmodified FROM hrd_karyawan WHERE id_karyawan =? ) ");
		$stmt1->execute(array($_GET['id']));

		$id = $db->lastInsertId();

		$stmt = $db->prepare("INSERT INTO `hrd_karyawandet` (`id_karyawan`,`id_penpot`,`persen`,`value`,`subtotal`,`keterangan`
		  ) 
			(SELECT $id,id_penpot,persen,`value`,subtotal,keterangan FROM hrd_karyawandet WHERE id_karyawan =? ) ");
		$stmt->execute(array($_GET['id']));
		//var_dump($stmt);die;
...
<script type="text/javascript"> 
    ...
    $(document).ready(function(){
        ...
        $("#table_karyawan").jqGrid({
            ...
            subGridModel: [
			            	{ 
			            		name : ['No','Pendapatan/Potongan','Tipe','%','Value','Subtotal'], 
			            		width : [30,250,100,50,100,100],
			            		align : ['right','center','center','center','right','right'],
			            	} 
			            ],
