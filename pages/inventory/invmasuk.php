...
$allow_add = is_show_menu(ADD_POLICY, MUTASIMASUK, $group_acess);
$allow_edit = is_show_menu(EDIT_POLICY, MUTASIMASUK, $group_acess);
$allow_delete = is_show_menu(DELETE_POLICY, MUTASIMASUK, $group_acess);
$allow_post = is_show_menu(POST_POLICY, MUTASIMASUK, $group_acess);
...
if(isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
	...
	$startdate = isset($_GET['startdate_invmasuk'])?$_GET['startdate_invmasuk']:date('Y-m-d');
	$enddate = isset($_GET['enddate_invmasuk'])?$_GET['enddate_invmasuk']:date('Y-m-d'); 
	$filter=$_GET['filter'];
        ...
        $sql = "SELECT p.*,i.nama as sumber FROM `invmasuk` p LEFT JOIN mst_inventory i on p.id_inventory=i.id ".$where;
        // var_dump($sql);die;
        $q = $db->query($sql);
        $count = $q->rowCount();
        ...
        foreach($data1 as $line) {
          ...
          if ($statusToko == 'Tutup') {
        	...
        	{
        		if($line['state']=='0'){
        			$edit = '<a onclick="window.open(\''.BASE_URL.'pages/inventory/invmasuk_detail_edit.php?ids='.$line['id_trans'].'\',\'table_invmasuk\')" href="javascript:;">Edit</a>';
        		}
        		else
        		{
        	...
        	if($allow_post)
        	{
        		...
        		if($line['state']=='0'){
        			$posting = '<a onclick="javascript:link_ajax(\''.BASE_URL.'pages/inventory/invmasuk.php?action=post&id='.$line['id_trans'].'\',\'table_invmasuk\')" href="javascript:;">POST</a>';
        		}
			   ...
			   //delete
        	if($allow_delete)
        		$delete = '<a onclick="javascript:link_ajax(\''.BASE_URL.'pages/inventory/invmasuk.php?action=delete&id='.$line['id_trans'].'\',\'table_invmasuk\')" href="javascript:;">Delete</a>';
        	else
      ...
      elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'post') {
       $sql = $db->query("SELECT state FROM `invmasuk` where id_trans='".$_GET['id']."'");
       $data = $sql->fetchAll(PDO::FETCH_ASSOC);
       foreach($data as $d){
        if ($d['state']=='0') {
            //update state posting = 1
          $stmt = $db->prepare("UPDATE invmasuk SET state='1' WHERE id_trans=?");
          $stmt->execute(array($_GET['id']));

            //insert inventory
          $stmt = $db->prepare("INSERT INTO inventory (id_trans,id_product,namabrg,size,qty,lastmodified)
            SELECT id_trans,id_product,namabrg,size,jumlah_beli,NOW() FROM invmasuk_detail WHERE id_trans=?"); 
          $stmt->execute(array($_GET['id']));

          $q = $db->query("SELECT imdet.id_product FROM `invmasuk_detail` imdet where imdet.id_trans='".$_GET['id']."'");
          $data1 = $q->fetchAll(PDO::FETCH_ASSOC);
          foreach($data1 as $line){
    ...
    elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'delete') {
     ...
     $stmt->execute(array($_GET['id']));
		...
		//update invmasuk agar deleted jadi 1
     $stmt = $db->prepare("update invmasuk set deleted=1 WHERE id_trans=?");
     $stmt->execute(array($_GET['id']));
		//var_dump($stmt);die;
  ...
  elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'json_sub') {
   ...
   $q = $db->query("SELECT pd.* FROM `invmasuk_detail` pd ".$where);
  ...
  <form id="report_project_form" method="" action="" class="ui-helper-clearfix">
   ...
   <div class="ui-corner-all form-control">
    <table>
     <tr>
      <td>
       <input value="" type="text" class="required datepicker"   id="startdate_invmasuk" name="startdate_invmasuk">
     </td>
     <td> s.d.  
       <input value="" type="text" class="required datepicker"  id="enddate_invmasuk" name="enddate_invmasuk">
     </td>
     <td> Filter
       <input value="" type="text" id="filter_invmasuk" name="filter_invmasuk">(Kode,Catatan,totalqty)
     </td>
   </tr>
 </table>
</div>
...
<div class="btn_box">
 ...
 if ($allow_add) {
   ?>
   <a href="javascript: void(0)" onclick="window.open('pages/inventory/invmasuk_detail.php');">
...
<table id="table_invmasuk"></table>
<div id="pager_table_invmasuk"></div>
...
<script type="text/javascript">
	...
	$('#startdate_invmasuk').datepicker({
		dateFormat: "dd/mm/yy"
	});
	$('#enddate_invmasuk').datepicker({
		dateFormat: "dd/mm/yy"
	});
	$( "#startdate_invmasuk" ).datepicker( 'setDate', '<?php echo date('d/m/Y')?>' );
	$( "#enddate_invmasuk" ).datepicker( 'setDate', '<?php echo date('d/m/Y')?>' );
	
	
	function gridReload(){
		var startdate = $("#startdate_invmasuk").val();
		var enddate = $("#enddate_invmasuk").val();
		var filter = $("#filter_invmasuk").val();
		var v_url ='<?php echo BASE_URL?>pages/inventory/invmasuk.php?action=json&startdate_invmasuk='+startdate+'&enddate_invmasuk='+enddate+'&filter='+filter;
		jQuery("#table_invmasuk").setGridParam({url:v_url,page:1}).trigger("reloadGrid");
	}
	...
	$(document).ready(function(){
		...
		$("#table_invmasuk").jqGrid({
			url:'<?php echo BASE_URL.'pages/inventory/invmasuk.php?action=json'; ?>',
			datatype: "json",
			...
			subGrid : true,
			subGridUrl : '<?php echo BASE_URL.'pages/inventory/invmasuk.php?action=json_sub'; ?>',
			subGridModel: [
			{ 
		...
		$("#table_invmasuk").jqGrid('navGrid','#pager_table_invmasuk',{edit:false,add:false,del:false,search:false});
