...
echo"<form id='form2' name='form2' action='' method='post'>
    <table width='100%'>
  	<tr>
    	<td  class='fontjudul'>MUTASI MASUK BARANG</td>
		<td class='fontjudul'> TOTAL QTY <input type='text' class='' name='totalqty' id='totalqty' value='0' style='text-align:right;font-size: 30px;background-color:white;height:40px;border:1px dotted #f30; border-radius:4px; -moz-border-radius:4px;' />		
		</td>
    </tr>
</table>
    <hr />
...
function cetak(){
	...
	{ 
		...
		{	
		hitungrow();
		document.form2.action="invmasuk_save.php?jum="+baris1;
...
<?php 
	$sql_detail="SELECT * FROM `inventory_balance` WHERE id='".$id."' and deleted=0";
	...
	$i=1;
		...
		?>
			...
			document.getElementById('BARCODE'+<?=$i;?>+'').value = '<?=$rs1['id'];?>';
			document.getElementById('IDP'+<?=$i;?>+'').value = '<?=$rs1['id'];?>';
			document.getElementById('NamaBrg'+<?=$i;?>+'').value = '<?=addslashes($rs1['nama']);?>';
