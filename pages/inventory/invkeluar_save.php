...
include("../../include/koneksi.php");
	...
	{
	$q = mysql_fetch_array( mysql_query('select id_trans from invkeluar order by id_trans desc limit 0,1'));
	...
	$id="OTV".$temp."".str_pad($temp2, 5, 0, STR_PAD_LEFT);	
	return $id;
	}	
	...
	//------simpan master---------------------
    $sql_master="";
	$sql_master="insert into invkeluar(id_trans,id_inventory,kode,totalqty,tgl_trans,catatan,user,lastmodified)values('".$id_pkb."','".$id_inventory."','".$kode."','".$totalqty."','".$tgl."','".$keterangan."','".$id_user."',NOW())";
	...
	for ($i=1; $i<$row; $i++)
	{
		...
		{
		//---simpan detail---
		$query = "INSERT INTO invkeluar_detail( id_trans,id_product,namabrg,jumlah_beli,size,subtotal,id_oln_auto) VALUES ('".$id_pkb."','".$id_product."','".$namabrg."','".$Qty."','".$Size."','".$Subtotal."','".$id_oln_auto."')";
		//var_dump($query);die;
	...
	//-update detail------------------------
		...
		$totalqty += str_replace(",","", $_POST['Qty'.$i]);
			...
			{
				if($delete=='' && $id_detail==''){
				$sql_insert="INSERT INTO invkeluar_detail( id_trans,id_product,namabrg,size,jumlah_beli,subtotal) VALUES ('".$id_pkb."','".$id_product."','".$namabrg."','".$Size."','".$Qty."','".$Subtotal."')" ;
				//echo "<script> alert('INSERT delete= $delete,id_detail=$id_detail,id_part=$id_product,baris=$i');</script>";	
				mysql_query($sql_insert) ;	
				}
				else if($delete=='' && $id_detail!=''){
				$sql_update="update invkeluar_detail set id_product = '".$id_product."', id_trans= '".$id_pkb."', namabrg= '".$namabrg."', size= '".$Size."', jumlah_beli= '".$Qty."',subtotal='".$Subtotal."' where id_inv_d = '".$id_detail."'";
				//var_dump($sql_update);die;
				//echo "<script> alert('UPDATE delete= $delete,id_detail=$id_detail,baris=$i');</script>";
		    	mysql_query($sql_update);
				}
				else if($delete!='' && $id_detail==''){
				$sql_delete="delete from invkeluar_detail where id_inv_d ='".$delete."'";
		...
		$sql_master_up="";
		$sql_master_up="update invkeluar set tgl_trans='".$tgl."',kode='".$kode."',nama='".$nama."',id_inventory='".$id_inventory."', totalqty='".$totalqty."',catatan='".$keterangan."',user='".$id_user."',lastmodified=now() where id_trans='".$_GET['id_trans']."'";
