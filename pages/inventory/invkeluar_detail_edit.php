...
$().ready(function() {	
 ...
 <?php
  ...
  $sql_mst="SELECT im.*,DATE_FORMAT(im.tgl_trans, '%Y-%m-%d') as tgl,i.nama as sumber FROM `invkeluar` im left join mst_inventory i on im.id_inventory=i.id WHERE im.id_trans= '".$_GET['ids']."'";
...
echo"<form id='form2' name='form2' action='' method='post'>
    <table width='100%'>
  	<tr>
    	<td  class='fontjudul'>EDIT MUTASI KELUAR BARANG</td>
...
function cetak(){
	...
	{ 
		...
		{	
		hitungrow() ;
		document.form2.action="invkeluar_save.php?id_trans=<?=$_GET['ids']?>&jum="+baris1;
...
<?php 
	$sql_detail="SELECT id.* from invkeluar_detail id WHERE  id.id_trans ='".$id_trans."' ORDER BY id.namabrg ASC";
