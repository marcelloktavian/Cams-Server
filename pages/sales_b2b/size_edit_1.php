...
<?php
  ...
  $sizelist = '';
  $count=1;
  $sql_size = "SELECT size FROM mst_b2bproductsgrp_detail det WHERE det.id_productsgrp='".$id."' AND deleted=0";
...
<?php
    ...
    for ($i=0; $i < count($ex); $i++) { 
        ...
        ?>
            addNewRow1();
            document.getElementById('Size'+<?=$i+1;?>+'').focus();
            document.getElementById('Size'+<?=$i+1;?>+'').value = '<?=$ex2[0];?>';
            document.getElementById('Qty'+<?=$i+1;?>+'').value = '<?=$ex2[1];?>';
        <?php
    }
?>
</script>
