...
<div class="ui-widget ui-form">    
  ...
  <div class="ui-widget-content ui-corner-bottom">
         ...
         <div class="ui-corner-all form-control">
            <table>
               <tr>
                  ...
                  <td>
                     <div class="ui-corner-all form-control">
                        <select name="pelanggan_id" id="pelanggan_id" class="my-select">
    ...
    if ($statusToko == 'Tutup') {
        echo '<button type="button" onclick="javascript:custom_alert(\'Maaf, Toko Sudah Tutup\')" class="btn">ADD CUSTOMER</button>';
    }else{
                    if ($allow_add) {
                       ?>
    ...
    if ($statusToko == 'Tutup') {
        echo '<button type="button" onclick="javascript:custom_alert(\'Maaf, Toko Sudah Tutup\')" class="btn">ADD SALES</button>';
    }else{

                if ($allow_add) {
                   ?>
                   <button onclick="javascript:window_open('<?php echo BASE_URL ?>pages/sales_b2b/trb2bsogrp_detail.php?id_cust='+$('#pelanggan_id').val())" class="btn" type="button">ADD ORDER</button>
                   <?php
               }}
               ?>
           </div>
   ...
   load_pelanggan = function (){
      ...
      $.ajax({
         ...
         success:function(result) {
           ...
           $('#pelanggan_id').select2().trigger('change');
       }
   });
