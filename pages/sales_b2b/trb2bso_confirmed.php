  ...
  else
  {
     $delete = '<a onclick="javascript:custom_alert(\'Tidak Bisa Cancel\')" href="javascript:;">Cancel</a>';
      ...
      elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'process') {
        ...
        // $stmt = $db->prepare("INSERT INTO jurnal_detail (SELECT '','$idparent',id_akun,no_akun,nama_akun,`status`,kredit,debet,'',0,'$id_user',NOW() FROM jurnal_detail LEFT JOIN jurnal ON jurnal.id=jurnal_detail.id_parent WHERE keterangan LIKE '%".$_POST['id']."%') ");
        // $stmt->execute();
                
        // update trolnso agar state jadi nol dan dikembalikan ke sales_order dan stkirim jadi 0 lagi
        $stmt1 = $db->prepare("update olnso set state='0',stkirim=0 WHERE id_trans=?");
        $stmt1->execute(array($_POST['id']));

        // $stmt = $db->prepare("UPDATE jurnal SET deleted=1 WHERE keterangan like '%".$_POST['id']."%'");
		// $stmt->execute();
        ...
        if ($_POST['qty'] > 0) {
            ...
            $r['message'] = 'Sudah ada barang terkirim';
                ...
                $stmt2 = $db->prepare("Update b2bso set state='0',lastmodified=now() WHERE id_trans=?");
    ...
    elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'delete') {
     ...
     if ($_GET['qty'] > 0) {
      ...
      $affected_rows = $stmt->rowCount();
      if($affected_rows > 0) {
            // $stmt = $db->prepare("UPDATE jurnal SET deleted=1 WHERE keterangan like '%".$_GET['id']."%'");
            // $stmt->execute();
            ...
            // insert jurnal detail
            // $stmt = $db->prepare("INSERT INTO jurnal_detail (SELECT '','$idparent',id_akun,no_akun,nama_akun,`status`,kredit,debet,'',0,'$id_user',NOW() FROM jurnal_detail LEFT JOIN jurnal ON jurnal.id=jurnal_detail.id_parent WHERE keterangan LIKE '%".$_GET['id']."%') ");
            // $stmt->execute();
