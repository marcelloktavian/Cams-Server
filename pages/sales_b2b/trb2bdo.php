...
$allow_delete = is_show_menu(DELETE_POLICY, DeliveryOrderB2B, $group_acess);
	...
	if(isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
        ...
        foreach($data1 as $line) {
			...
			if($allow_delete)
				...
				$delete = '<a onclick="javascript:popup_form(\''.BASE_URL.'pages/sales_b2b/trb2bdo.php?action=pass&nofaktur='.$line['no_faktur'].'&id='.$line['id_trans'].'\',\'table_b2bdo\')" href="javascript:;">Cancel</a>';
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'pass') {
		//tgl beda
		include 'trb2bdo_pass_form.php';exit();
		exit;
	}
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'process_pass') {
		...
		if($affected_rows > 0) {
			...
			// insert jurnal
			$stmt = $db->prepare("INSERT INTO jurnal (SELECT NULL,'$masterNo',NOW(),REPLACE(keterangan,'Penjualan','CANCELLED'),total_debet,total_kredit,0,'$id_user',NOW() FROM jurnal WHERE jurnal.keterangan LIKE '%".$_POST['nofaktur']."%') ");
			$stmt->execute();
			...
			$stmt = $db->prepare("INSERT INTO jurnal_detail (SELECT '','$idparent',id_akun,no_akun,nama_akun,`status`,kredit,debet,'',0,'$id_user',NOW() FROM jurnal_detail LEFT JOIN jurnal ON jurnal.id=jurnal_detail.id_parent WHERE jurnal.keterangan LIKE '%".$_POST['nofaktur']."%') ");
			$stmt->execute();
			...
			$totalfaktur = 0;
			$totalkirim = 0;
			$qry = $db->prepare("SELECT id_transb2bso,totalfaktur,totalkirim FROM `b2bdo` where id_trans='".$_POST['id']."' ");
			$qry->execute();
			$id = $qry->fetchAll();
			foreach ($id as $ids) {
			...
			//update b2bso detail
			$qry = $db->prepare("SELECT * FROM `b2bdo_detail` where id_trans='".$_POST['id']."' ");
			$qry->execute();
			$id = $qry->fetchAll();
			foreach ($id as $ids) {
			$totalqty = 0;
