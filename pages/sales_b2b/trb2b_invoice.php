...
error_reporting(0);
	...
	$sql_jual="SELECT do.*,(so.ref_kode) as no_po,DATE_FORMAT(so.tgl_trans,'%d/%m/%Y')AS tglsales,so.nama,so.alamat,c.nama AS customer,e.kode AS kodekendaraan,e.nama AS expedition,s.nama AS salesman,DATE_FORMAT(do.tgl_trans,'%d/%m/%Y')as tgl FROM `b2bdo` do LEFT JOIN `b2bso` so on do.id_transb2bso=so.id_trans LEFT JOIN `mst_b2bcustomer` c ON (do.id_customer=c.id) LEFT JOIN `mst_b2bexpedition` e ON (do.id_expedition=e.id) LEFT JOIN `mst_b2bsalesman` s ON (do.id_salesman=s.id)
