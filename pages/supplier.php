...
//--------------end of searching--------------------		
        ...
        foreach($data1 as $line) {
			...
			if(in_array($_SESSION['user']['access'], $allowEdit))
				$edit = '<a onclick="javascript:popup_form(\''.BASE_URL.'pages/supplier.php?action=edit&id='.$line['id'].'\',\'table_supplier\')" href="javascript:;">Edit</a>';
			else
				$edit = '<a onclick="javascript:custom_alert(\'Not Allowed\')" href="javascript:;">Edit</a>';
        	
			if(in_array($_SESSION['user']['access'], $allowDelete))
				$delete = '<a onclick="javascript:link_ajax(\''.BASE_URL.'pages/supplier.php?action=delete&id='.$line['id'].'\',\'table_supplier\')" href="javascript:;">Delete</a>';
			else
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'process') {
		if(isset($_POST['id'])) {
			$stmt = $db->prepare("UPDATE tblsupplier SET namaperusahaan=?,alamat=?,telp1=?,telp2=?,fax=?,contactperson=?,HP=?,email=?,user=?, lastmodified = NOW() WHERE id=?");
			...
			$stmt = $db->prepare("INSERT INTO tblsupplier(`namaperusahaan`,`alamat`,`telp1`,`telp2`,`fax`,`contactperson`,`HP`,`email`,`user`,`lastmodified`) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
...
<?php
	...
	if(in_array($_SESSION['user']['access'], $allow)) {
		echo '<button type="button" onclick="javascript:popup_form(\''.BASE_URL.'pages/supplier.php?action=add\',\'table_supplier\')" class="btn">Tambah</button>';
	}
?>
</div>
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#table_supplier").jqGrid({
            url:'<?php echo BASE_URL.'pages/supplier.php?action=json'; ?>',
            /*postData: {
