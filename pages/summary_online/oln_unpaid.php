...
$allow_post = is_show_menu(POST_POLICY, UnpaidOnline, $group_acess);
	...
	if(isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
       ...
       $where = "WHERE TRUE AND ((p.piutang-p.pelunasan)>0) ";
       } else {
       $operations = array(
        'eq' => "= '%s'",            // Equal
        'ne' => "<> '%s'",           // Not equal
	  ...
	  $where = sprintf(" where TRUE AND ((p.piutang-p.pelunasan)>0) AND %s ".$operations[$_REQUEST["searchOper"]], $_REQUEST["searchField"], $value);
	 //echo"<script>alert('where=$where')</script>";
        ...
        foreach($data1 as $line) {
			...
			if ($statusToko == 'Tutup') {
                $edit = '<a onclick="javascript:custom_alert(\'Maaf, Toko Sudah Tutup\')" href="javascript:;">Pay</a>';
            } else {
			if($allow_post)
					$edit = '<a onclick="window.open(\''.BASE_URL.'pages/summary_online/piutang_detail_lunasi.php?ids='.$line['id_trans'].'\',\'table_jual_blmbyr\')" href="javascript:;">Pay</a>';
            else
                $edit = '<a onclick="javascript:custom_alert(\'Not Allowed\')" href="javascript:;">Pay</a>';
			/*
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#table_jual_blmbyr").jqGrid({
            url:'<?php echo BASE_URL.'pages/summary_online/oln_unpaid.php?action=json'; ?>',
            datatype: "json",
            ...
            rownumbers: true,
            sortorder: "asc",
            caption:"Unpaid Online",
            ondblClickRow: function(rowid) {
                alert(rowid);
            },
            subGrid : true,
            subGridUrl : '<?php echo BASE_URL.'pages/summarY_online/oln_unpaid.php?action=json_sub'; ?>',
            subGridModel: [
			            	{ 
