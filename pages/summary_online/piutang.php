...
<?php
	if(isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
		...
		$startdate = isset($_GET['startdate_piutang'])?$_GET['startdate_piutang']:date('d/m/Y');
		$enddate = isset($_GET['enddate_piutang'])?$_GET['enddate_piutang']:date('d/m/Y'); 
     ...
     // }
		...
		$sql = "SELECT p.*,j.nama as dropshipper FROM `trpiutang` p Left Join `mst_dropshipper` j on (p.id_customer=j.id) ".$where;
        $q = $db->query($sql);
		$count = $q->rowCount();
    //    var_dump($sql);
        ...
        foreach($data1 as $line) {
			...
			$edit = '<a onclick="window.open(\''.BASE_URL.'pages/summary_online/piutang_nota.php?id_trans='.$line['id_trans'].'\',\'table_piutang\')" href="javascript:;">Receipt</a>';
			}
			else
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'json_sub') {
        ...
        $q = $db->query("SELECT pd.id_trans, pd.id_detail,pd.id_transjual,pd.faktur,pd.piutang,pd.piutang_update,pd.bayar, pd.bank FROM `trpiutang_detail` pd ".$where);
 ...
 <div class="ui-widget-header ui-corner-top padding5">
    ...
    <div class="ui-widget-content ui-corner-bottom">
        <form id="report_project_form" method="" action="" class="ui-helper-clearfix">
            ...
            <div class="ui-corner-all form-control">
				...
				<tr>
				<td>
				 <input value="" type="text" class="required datepicker"   id="startdate_piutang" name="startdate_piutang">
				</td>
				<td> s.d.  
				<input value="" type="text" class="required datepicker"  id="enddate_piutang" name="enddate_piutang">
				</td>
				</tr>
				</table>
            </div>
    ...
    if ($statusToko == 'Tutup') {
        echo '<button type="button" onclick="javascript:custom_alert(\'Maaf, Toko Sudah Tutup\')" class="btn">Print</button>';
                ...
                ?>
				<button onclick="javascript:window_open('<?php echo BASE_URL ?>pages/summary_online/piutangrpt.php?action=preview&start='+$('#startdate_piutang').val()+'&end='+$('#enddate_piutang').val())" class="btn" type="button">Print</button>
                <?php } ?>
            </div>
       	</form>
   	</div>
...
<table id="table_piutang"></table>
<div id="pager_table_piutang"></div>
<div class="btn_box">
<!--
...
<script type="text/javascript">
	...
	$('#startdate_piutang').datepicker({
		dateFormat: "dd/mm/yy"
	});
	$('#enddate_piutang').datepicker({
		dateFormat: "dd/mm/yy"
	});
	$( "#startdate_piutang" ).datepicker( 'setDate', '<?php echo date('d/m/Y')?>' );
	$( "#enddate_piutang" ).datepicker( 'setDate', '<?php echo date('d/m/Y')?>' );
	
	
	function gridReloadPiutang(){
		var startdate_piutang = $("#startdate_piutang").val();
		var enddate_piutang = $("#enddate_piutang").val();
		var v_url ='<?php echo BASE_URL?>pages/summary_online/piutang.php?action=json&startdate_piutang='+startdate_piutang+'&enddate_piutang='+enddate_piutang ;
		jQuery("#table_piutang").setGridParam({url:v_url,page:1}).trigger("reloadGrid");
	}
    ...
    $(document).ready(function(){
		...
		//alert('kdie');
        $("#table_piutang").jqGrid({
            url:'<?php echo BASE_URL.'pages/summary_online/piutang.php?action=json'; ?>',
            /*postData: {
            ...
            colNames:['ID','Dropshipper','Tanggal Transaksi','Qty','TotalBayar','Tunai','Bank','Lihat'],
            colModel:[
            ...
            subGrid : true,
            subGridUrl : '<?php echo BASE_URL.'pages/piutang/piutang.php?action=json_sub'; ?>',
            subGridModel: [
			            	{ 
        ...
        $("#table_piutang").jqGrid('navGrid','#pager_table_piutang',{edit:false,add:false,del:false,search:false});
    })
</script>