...
if (isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
	...
	include 'postingprebank_form.php';
	exit();
	exit;
	...
	$sql_detail = "SELECT pd.*,d.disc as discdp,ROUND((pd.harga_satuan * (1 - d.disc)), 0) AS nett_price,(ROUND((pd.harga_satuan * (1 - d.disc)), 0) * ( pd.jumlah_beli )) AS subtotal_nett FROM `olnsodetail` pd inner join olnso p on pd.id_trans=p.id_trans left join mst_dropshipper d on p.id_dropshipper=d.id " . $where;
...
<script type="text/javascript">
	...
	function gridReloadJual() {
		...
		jQuery("#table_jualcash").setGridParam({
			url: v_url,
			page: 1
		}).trigger("reloadGrid");
	}
	...
	$(document).ready(function() {
		...
		$("#table_jualcash").jqGrid({
			...
			colModel: [{
				...
				{
					name: 'discount_faktur',
					index: 'discount_faktur',
					align: 'right',
					width: 80,
					searchoptions: {
						sopt: ['cn']
		...
		$("#table_jualcash").jqGrid('navGrid', '#pager_table_jualcash', {
			edit: false,
			add: false,
			del: false,
			search: false
