...
error_reporting(0);
	...
	$id_faktur=$_GET['id_trans'];
	$sql = mysql_query("SELECT a.*,b.nama as dropshipper FROM trpiutang a 
	left join mst_dropshipper b on a.id_customer = b.id
...
<form id="form2" name="form2" action="" method="post"  onSubmit="return validasi(this)">
    ...
    <tr>
      <td height="123" valign="top"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
          ...
          <tr>
            ...
            <td class="style9" width="1%"><div align="center">:</div></td>
            <td class="style9" width="30%">PT.AGUNG KEMUNINGWIJAYA</td>
            <td class="style9" width="19%">Kepada Yth.</td>           
            <td class="style9" width="1%"><div align="center">:</div></td>           
            <td class="style9" width="30%"class="style9"><?=$rs['dropshipper'];?></td>           
		  </tr>
          <tr>
		  <td width="19%" class="style9">NPWP</td>
            <td class="style9" width="1%"><div align="center">:</div></td>
            <td class="style9" width="30%">01.448.609.6.445.000</td>
  ...
  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
    ...
    <?
	$sql_detpiutang="select a.id_transjual,DATE_FORMAT(b.tgl_trans,'%d-%m-%Y') as tgl_jual,a.faktur,b.kode, a.bayar,a.bank,a.piutang,a.piutang_update,(a.piutang-a.piutang_update) as totalbyr from trpiutang_detail a left join olnso b on a.id_transjual = b.id_trans where a.id_trans ='".$id_faktur."'";
	//var_dump($sql_detpiutang);die;
	$sq2 = mysql_query($sql_detpiutang);
	//--------------------end trpiutangdetail---------------------------------
  ...
  ?>
	...
	//update nilai sisa
    if ($rs2['piutang_update'] > 0){ 
	$sisapiutangdt+=$rs2['piutang_update'];
	$simpan_deposit= 0;
	}
	else if ($rs2['piutang_update'] < 0)
	{
	 $sisapiutangdt = 0;
	 $simpan_deposit = -$rs2['piutang_update'];
	}
   }
    ?>
	<?php
  ...
  <table>
	...
	<tr>
	  ...
	  <!--
      <td width="120" class="style9" align="left">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<strong>SIMPAN DEPOSIT</strong></td>
      <td width="100">
		<div id="total" align="right" class="style20">
		<? //echo number_format($simpan_deposit,0);?>
		</div>
	  </td>
      -->	  
    </tr>
