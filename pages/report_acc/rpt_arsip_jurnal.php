...
$akun = $_GET['akun'];
if($akun == ''){
    $sql="SELECT det.`no_akun`, det.`nama_akun`, det.`debet`, det.`kredit`, DATE_FORMAT(mst.`tgl`,'%d/%m/%Y') AS tanggal, mst.`no_jurnal` , mst.`keterangan`, DATE_FORMAT(STR_TO_DATE('$startdate','%d/%m/%Y'),'%d/%m/%Y') AS tglawal, DATE_FORMAT(STR_TO_DATE('$enddate','%d/%m/%Y'),'%d/%m/%Y') AS tglakhir FROM jurnal_detail_archive det
    LEFT JOIN jurnal_archive mst ON mst.id=det.id_parent
    ...
    if(mysql_num_rows($get_detail) == 0){
        $sql="SELECT det.`no_akun`, det.`nama_akun`, det.`debet`, det.`kredit`, DATE_FORMAT(mst.`tgl`,'%d/%m/%Y') AS tanggal, mst.`no_jurnal`, mst.`keterangan` ,DATE_FORMAT(STR_TO_DATE('$startdate','%d/%m/%Y'),'%d/%m/%Y') AS tglawal, DATE_FORMAT(STR_TO_DATE('$enddate','%d/%m/%Y'),'%d/%m/%Y') AS tglakhir FROM jurnal_detail_archive det LEFT JOIN jurnal_archive mst ON mst.id=det.id_parent WHERE det.deleted=0 AND mst.deleted=0 AND det.`no_akun`='$akun' AND mst.tgl BETWEEN STR_TO_DATE('$startdate','%Y/%m/%d') AND STR_TO_DATE('$enddate','%Y/%m/%d') ORDER BY mst.tgl ASC, no_jurnal ASC, CASE WHEN det.kredit = 0 THEN 0 ELSE 1 END";
    }
    else{
        $sql="SELECT det.`no_akun`, det.`nama_akun`, det.`debet`, det.`kredit`, DATE_FORMAT(mst.`tgl`,'%d/%m/%Y') AS tanggal, mst.`no_jurnal`, mst.`keterangan` ,DATE_FORMAT(STR_TO_DATE('$startdate','%d/%m/%Y'),'%d/%m/%Y') AS tglawal, DATE_FORMAT(STR_TO_DATE('$enddate','%d/%m/%Y'),'%d/%m/%Y') AS tglakhir FROM jurnal_detail_archive det LEFT JOIN jurnal_archive mst ON mst.id=det.id_parent WHERE det.deleted=0 AND mst.deleted=0 AND det.`no_akun` IN ".$detail_akun." AND mst.tgl BETWEEN STR_TO_DATE('$startdate','%Y/%m/%d') AND STR_TO_DATE('$enddate','%Y/%m/%d') ORDER BY mst.tgl ASC, no_jurnal ASC, CASE WHEN det.kredit = 0 THEN 0 ELSE 1 END";
    }
}
