...
<form id="form2" name="form2" action="" method="post"  onSubmit="return validasi(this)">
  ...
  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
	...
	$sql_detail = "SELECT do.*,date_format(do.tgl_trans,'%d-%m-%Y') as dodate,date_format(do.tgljatuhtempo,'%d-%m-%Y')as tgl_jatuhtempo,so.tgl_trans as tgl_order,so.nama,so.alamat,c.nama AS customer,e.nama AS expedition,s.nama AS salesman FROM `b2bdo` do LEFT JOIN `b2bso` so on do.id_transb2bso=so.id_trans LEFT JOIN `mst_b2bcustomer` c ON (do.id_customer=c.id) LEFT JOIN `mst_expedition` e ON (do.id_expedition=e.id) LEFT JOIN `mst_b2bsalesman` s ON (do.id_salesman=s.id)".$where." ORDER BY no_faktur ASC"; 
  ...
  ?>
    <tr>
      ...
      <td class="style_detail"><div align="center"><?=$rs2['no_faktur'];?></div></td>
      <td class="style_detail"><div align="center"><?=$rs2['customer'];?></div></td>
      <td class="style_detail"><div align="center"><?=$rs2['dodate'];?></div></td>
      <td class="style_detail"><div align="center"><?=$rs2['tgl_jatuhtempo'];?></div></td>
