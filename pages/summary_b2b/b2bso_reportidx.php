...
if(isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
	...
	$startdate = isset($_GET['startdate_b2bsorpt'])?$_GET['startdate_b2bsorpt']:date('Y-m-d');
	$enddate = isset($_GET['enddate_b2bsorpt'])?$_GET['enddate_b2bsorpt']:date('Y-m-d'); 
	...
	$filter = isset($_GET['filter'])?$_GET['filter']:''; 
        ...
        $where = "WHERE TRUE AND p.deleted='0' AND (p.totalqty <> 0) AND (p.piutang > 0)";
		...
		*/
		...
		if($startdate != null){
			$where .= " AND DATE(tgl_trans) BETWEEN STR_TO_DATE('$startdate','%d/%m/%Y') AND STR_TO_DATE('$enddate','%d/%m/%Y') AND p.deleted='0' AND (p.totalqty <> 0)";
		}	
		...
		$sql = "SELECT p.*,k.nama as kategori,j.nama as customer,s.nama as salesman,e.nama as expedition, IF(p.totalqty <> (select SUM(jumlah_kirim) as totkirim FROM b2bso_detail det WHERE det.id_trans=p.id_trans),'Belum Selesai','Selesai') as statusnya FROM `b2bso` p Left Join `mst_b2bcategory_sale` k on (p.id_kategori=k.id) Left Join `mst_b2bsalesman` s on (p.id_salesman=s.id) Left Join `mst_b2bcustomer` j on (p.id_customer=j.id) Left Join `mst_b2bexpedition` e on (p.id_expedition=e.id) ".$where;
		$q = $db->query($sql);
		$count = $q->rowCount();
        // var_dump($sql);
		...
		foreach($data1 as $line) {
            ...
            $totkirim = 0;
            $status='';
            $totbeli = $line['totalqty'];

            $sqltotal = $db->query("select SUM(jumlah_beli) as totbeli,SUM(jumlah_kirim) as totkirim FROM b2bso_detail WHERE id_trans='".$line['id_trans']."' ");
		    $datatotal = $sqltotal->fetchAll(PDO::FETCH_ASSOC);
            foreach($datatotal as $tot) {
                $totkirim = $tot['totkirim'];
            }
			...
			<form id="report_project_form" method="" action="" class="ui-helper-clearfix">
				...
				<div class="ui-corner-all form-control">
					<table>
						<tr>
							<td>
								<input value="" type="text" class="required datepicker"   id="startdate_b2bsorpt" name="startdate_b2bsorpt">
							</td>
							<td> s.d.  
								<input value="" type="text" class="required datepicker"  id="enddate_b2bsorpt" name="enddate_b2bsorpt">
							</td>
				
							<td> Filter
							 	<input value="" type="text" id="filterb2bso_rpt" name="filterb2bso_rpt">(Kode Order,Customer,Salesman)
							</td>
...
<div class="ui-corner-all form-control">
	<button onclick="gridReloadb2bsoRpt()" class="btn" type="button">Cari</button>
	<button onclick="printb2bsoRpt()" class="btn" type="button">Cetak</button>
</div>
</form>
</div>
</div>
...
<table id="table_b2bdorpt"></table>
<div id="pager_table_b2bdorpt"></div>
...
<script type="text/javascript">
	...
	$('#startdate_b2bsorpt').datepicker({
		dateFormat: "dd/mm/yy"
	});
	$('#enddate_b2bsorpt').datepicker({
		dateFormat: "dd/mm/yy"
	});
	$( "#startdate_b2bsorpt" ).datepicker( 'setDate', '<?php echo date('d/m/Y')?>' );
	$( "#enddate_b2bsorpt" ).datepicker( 'setDate', '<?php echo date('d/m/Y')?>' );
	
	
	function gridReloadb2bsoRpt(){
		var startdate_b2bsorpt = $("#startdate_b2bsorpt").val();
		var enddate_b2bsorpt = $("#enddate_b2bsorpt").val();
		var filter = $("#filterb2bso_rpt").val();
		var v_url ='<?php echo BASE_URL?>pages/summary_b2b/b2bso_reportidx.php?action=json&startdate_b2bsorpt='+startdate_b2bsorpt+'&enddate_b2bsorpt='+enddate_b2bsorpt+'&filter='+filter ;
		jQuery("#table_b2bdorpt").setGridParam({url:v_url,page:1}).trigger("reloadGrid");
	}

    function printb2bsoRpt() {
		var startdate_b2bsorpt = $("#startdate_b2bsorpt").val();
		var enddate_b2bsorpt = $("#enddate_b2bsorpt").val();
		var filter = $("#filterb2bso_rpt").val();
		// console.log(filter+' '+lokasi_list);

		window_open('<?php echo BASE_URL ?>pages/summary_b2b/b2bso_report.php?action=preview&filter='+filter+'&start='+startdate_b2bsorpt+'&end='+enddate_b2bsorpt);
	...
	$("#btn-print").on('click',function(){
	 ...
	 function getSelectedRows() {
        ...
        */
		...
		//alert('kdie');
		$("#table_b2bdorpt").jqGrid({
			url:'<?php echo BASE_URL.'pages/summary_b2b/b2bso_reportidx.php?action=json'; ?>',
			datatype: "json",
			colNames:['ID','Kode Order','Customer','Date','Salesman','Address','Category','Qty Order','Qty Kirim','Status'],
			colModel:[
			...
			{name:'totalqtykirim',index:'totalqtykirim', align:'right', width:20, searchoptions: {sopt:['cn']}},
			{name:'statusnya',index:'statusnya', align:'center', width:50, searchoptions: {sopt:['cn']}},
			...
			rownumbers: true,
			sortorder: "desc",
			caption:"B2B Order Input Report",
		...
		$("#table_b2bdorpt").jqGrid('navGrid','#pager_table_b2bdorpt',{edit:false,add:false,del:false,search:false});
