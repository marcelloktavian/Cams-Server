<div class="ui-widget ui-form">
    <div class="ui-widget-header ui-corner-top padding5">
        <?php
			...
			echo $action .' DATA GUDANG';
        ?>
    </div>
    <div class="ui-widget-content ui-corner-bottom">
        <form id="supplier_form" method="post" action="<?php echo BASE_URL ?>pages/master/mstgudang.php?action=process" class="ui-helper-clearfix">
        	<?php
	        	if(strtolower($_GET['action']) == 'edit') {
					echo '<input value="'.$_GET['id'].'" type="hidden" class="required" id="id" name="id">';
					$select = $db->prepare('SELECT * FROM mst_gudang WHERE id = :id');
					$select->execute(array(':id' => $_GET['id']));
					$row = $select->fetch(PDO::FETCH_ASSOC);
				}
	        ?>
            <label for="supplier_name" class="ui-helper-reset label-control">Nama Gudang (*)</label>
            ...
            <div class="ui-corner-all form-control">
                <input value="<?php echo isset($row['telp1']) ? $row['telp1'] : ''; ?>" type="text" class="required" id="telp1" name="telp1">
            </div>
