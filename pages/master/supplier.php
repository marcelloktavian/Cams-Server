...
//--------------end of searching--------------------		
        ...
        $q = $db->query("SELECT p.id,p.id_cust, p.namaperusahaan,p.alamat,p.telp1,p.HP, p.contactperson,p.keterangan  
							 FROM `tblsupplier` p
							 ".$where."
							 ORDER BY `".$sidx."` ".$sord."
							 LIMIT ".$start.", ".$limit);
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'process') {
		if(isset($_POST['id'])) {
			$stmt = $db->prepare("UPDATE tblsupplier SET id_cust=?,namaperusahaan=?,keterangan=?,alamat=?,telp1=?,telp2=?,fax=?,contactperson=?,HP=?,email=?,user=?, lastmodified = NOW() WHERE id=?");
			$stmt->execute(array($_POST['idsupp'],strtoupper($_POST['nama']),strtoupper($_POST['keterangan']),$_POST['alamat'],$_POST['telp1'],$_POST['telp2'],$_POST['fax'],$_POST['contactperson'],$_POST['hp'],$_POST['email'],$_SESSION['user']['username'], $_POST['id']));
			$affected_rows = $stmt->rowCount();
			if($affected_rows > 0) {
				$r['stat'] = 1;
				$r['message'] = 'Success';
			...
			$stmt = $db->prepare("INSERT INTO tblsupplier(`id_cust`,`namaperusahaan`,`keterangan`,`alamat`,`telp1`,`telp2`,`fax`,`contactperson`,`HP`,`email`,`user`,`type`,`lastmodified`) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,2, NOW())");
			if($stmt->execute(array($_POST['idsupp'],strtoupper($_POST['nama']),strtoupper($_POST['keterangan']),$_POST['alamat'],$_POST['telp1'],$_POST['telp2'],$_POST['fax'],$_POST['contactperson'],$_POST['hp'],$_POST['email'],$_SESSION['user']['username']))) {
				$r['stat'] = 1;
				$r['message'] = 'Success';
			}
			else {
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#table_supplier").jqGrid({
            ...
            datatype: "json",
            colNames:['Kode','Nama','Alamat','Telepon','Contact Person','NPWP/NIK','Edit','Delete'],
            colModel:[
                {name:'id_cust',index:'id_cust', width:20, searchoptions: {sopt:['cn']}},
                
				{name:'namaperusahaan',index:'namaperusahaan', width:100, searchoptions: {sopt:['cn']}},                
                {name:'alamat',index:'alamat', width:140, searchoptions: {sopt:['cn']}},                
                {name:'telp1',index:'telp1', width:60, searchoptions: {sopt:['cn']}},                
                {name:'contactperson',index:'contactperson', width:70, searchoptions: {sopt:['cn']}},                
                
				{name:'keterangan',index:'keterangan', width:70, searchoptions: {sopt:['cn']}},                
