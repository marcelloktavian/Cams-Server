...
<?php
	if(isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
       ...
       $where = "WHERE TRUE AND (p.piutang>0) AND p.deleted = 0 ";
       } else {
       $operations = array(
        'eq' => "= '%s'",            // Equal
        'ne' => "<> '%s'",           // Not equal
	  ...
	  $where = sprintf(" where (p.deleted=0) AND (p.piutang>0) AND %s ".$operations[$_REQUEST["searchOper"]], $_REQUEST["searchField"], $value);
