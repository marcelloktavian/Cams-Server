...
//--------------end of searching--------------------		
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'process') {
		if(isset($_POST['id'])) {
			$stmt = $db->prepare("UPDATE mst_b2bexpedition SET kode=?,nama=?,operator=?,kode_warna=?,user=?, lastmodified = NOW() WHERE id=?");
			$stmt->execute(array($_POST['kode'],strtoupper($_POST['nama']),$_POST['operator'],$_POST['kode_warna'], $_SESSION['user']['username'], $_POST['id']));
			$affected_rows = $stmt->rowCount();
			if($affected_rows > 0) {
				$r['stat'] = 1;
				$r['message'] = 'Success';
			...
			$stmt = $db->prepare("INSERT INTO  mst_b2bexpedition(`kode`,`nama`,`operator`,`kode_warna`,`user`,`lastmodified`) VALUES( ?, ?, ?,  ?,  ?, NOW())");
			//var_dump($stmt);die;
			if($stmt->execute(array($_POST['kode'],strtoupper($_POST['nama']),$_POST['operator'],$_POST['kode_warna'],$_SESSION['user']['username']))) {
				$r['stat'] = 1;
				$r['message'] = 'Success';
			}
			else {
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#table_b2bexpedition").jqGrid({
            ...
            datatype: "json",
            colNames:['ID','No Kendaraan','Nama','Operator','Kode_Warna','Edit','Delete'],
            colModel:[
                ...
                {name:'Operator',index:'nama', width:170, searchoptions: {sopt:['cn']}},                
