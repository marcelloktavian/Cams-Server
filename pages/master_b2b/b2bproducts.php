...
//--------------end of searching--------------------		
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'json_sub') {
		...
		$sql_sub="SELECT pd.products_detail_id,pd.products_id,pd.composition_id,pd.qty,c.nama AS composition, pd.qty FROM mst_b2bproducts_detail pd LEFT JOIN mst_composition c ON pd.composition_id=c.id  WHERE  pd.products_id ='".$id."' ORDER BY c.nama ASC";
		$q = $db->query($sql_sub);
