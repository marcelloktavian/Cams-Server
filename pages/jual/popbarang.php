...
{
	$cari=$_GET['cari'];
	$sql=mysql_query("Select sb.id_barang,b.nm_barang,b.hrg_jual,sum(sb.stok) as stok from stok_barang sb left join barang b on sb.id_barang = b.id_barang where b.nm_barang LIKE '%".$cari."%' group by sb.id_barang order by b.nm_barang asc limit $offset, $jmlperhalaman") or die (mysql_error());
	//$sql=mysql_query("select * from barang where nm_barang LIKE '%".$cari."%' limit $offset, $jmlperhalaman") or die (mysql_error());
...
?>
        ...
        <td width="89" id="Harga<?=$rs[0];?>" align=right><?=$rs[hrg_jual];?>
        </td> 
        <td width="89" id="Stok<?=$rs[0];?>" align=right><?=$rs[stok];?>
        </td> 
        </tr>
      <?php
	$i++;
...
*/
    ...
    where b.nm_barang LIKE '%".$cari."%' group by sb.id_barang order by b.nm_barang asc "),0);
