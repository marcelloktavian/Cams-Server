...
<?php
	if(isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
       ...
       $where = "WHERE TRUE AND ((p.piutang-p.pelunasan)>0) AND p.deleted = 0 ";
       } else {
       $operations = array(
        'eq' => "= '%s'",            // Equal
        'ne' => "<> '%s'",           // Not equal
	  ...
	  $where = sprintf(" where (p.deleted=0) AND ((p.piutang-p.pelunasan)>0) AND %s ".$operations[$_REQUEST["searchOper"]], $_REQUEST["searchField"], $value);
        ...
        foreach($data1 as $line) {
			...
			if(in_array($_SESSION['user']['access'], $allowEdit))
				if($line['is_start'] == 0) {
					...
					$edit = '<a onclick="window.open(\''.BASE_URL.'pages/piutang/piutang_detail_lunasi.php?ids='.$line['id_trans'].'\',\'table_jual_blmbyr\')" href="javascript:;">lunasi</a>';
			...
			if(in_array($_SESSION['user']['access'], $allowDelete))
				if($line['is_start'] == 0) {
					$delete = '<a onclick="javascript:link_ajax(\''.BASE_URL.'pages/jual/jual.php?action=delete&id='.$line['id_trans'].'\',\'table_jual_blmbyr\')" href="javascript:;">Delete</a>';
				}
				else {
	...
	*/
?>
<table id="table_jual_blmbyr"></table>
<div id="pager_table_jual_blmbyr"></div>

<script type="text/javascript">
    $(document).ready(function(){

        $("#table_jual_blmbyr").jqGrid({
            url:'<?php echo BASE_URL.'pages/jual/jualblmbyr.php?action=json'; ?>',
            /*postData: {
            ...
            datatype: "json",
            colNames:['ID','Customer','Tanggal Transaksi','Qty','Faktur','Ongkir','Total Faktur','Tunai','Transfer','Piutang','View','Delete'],
            colModel:[
            ...
            subGrid : true,
            subGridUrl : '<?php echo BASE_URL.'pages/jual/jualblmbyr.php?action=json_sub'; ?>',
            subGridModel: [
			            	{ 
        ...
        $("#table_jual_blmbyr").jqGrid('navGrid','#pager_table_jual_blmbyr',{edit:false,add:false,del:false});
    })
</script>