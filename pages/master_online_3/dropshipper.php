...
//--------------end of searching--------------------		
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'process') {
		...
		if(isset($_POST['id'])) {
            ...
            if ($query->rowCount() == 0) {
                $stmt = $db->prepare("UPDATE mst_dropshipper SET nama=?,oln_customer_id=?,alamat=?,no_telp=?,hp=?,disc=?,type=?,user=?,no_akun=?, lastmodified = NOW() WHERE id=?");
                $stmt->execute(array($_POST['nama'],$_POST['oln_customer_id'],$_POST['alamat'],$telp,$_POST['hp'],$_POST['disc'],$_POST['tipe'], $_SESSION['user']['username'], $_POST['noakun'], $_POST['id']));
                //var_dump($stmt);die;
                $affected_rows = $stmt->rowCount();
                if($affected_rows > 0) {
                    $r['stat'] = 1;
            ...
            if ($query->rowCount() == 0) {
			    $stmt = $db->prepare("INSERT INTO  mst_dropshipper(`nama`,`oln_customer_id`,`alamat`,`no_telp`,`hp`,`disc`,`type`,`user`,`no_akun`,`lastmodified`) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?,NOW())");
			    if($stmt->execute(array($_POST['nama'],$_POST['oln_customer_id'],$_POST['alamat'], $telp, $_POST['hp'], $_POST['disc'], $_POST['tipe'],$_SESSION['user']['username'],$_POST['noakun']))) {
				    $r['stat'] = 1;
				    $r['message'] = 'Success';
			    }
			    else {
