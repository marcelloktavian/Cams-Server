...
<form id="form2" name="form2" action="" method="post"  onSubmit="return validasi(this)">
  ...
  <?
    ...
    $sql_exp = "SELECT e.`nama`,e.kode_warna,jual.totalexp FROM mst_expedition e LEFT JOIN (SELECT so.id_expedition,COUNT(so.id_expedition)AS totalexp FROM olnso so LEFT JOIN olnso_id i ON so.id_trans=i.id_trans LEFT JOIN `mst_expedition` e on (so.id_expedition=e.id) WHERE (so.state='1') $filter_title GROUP BY so.id_expedition) AS jual ON e.id=jual.id_expedition where e.deleted=0";
...
?>
  ...
  <table class="cut-off "width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
      ...
      <th width="20%" class="style_title"><div align="left">Item</div></td>
	...
	*/
	...
	echo"<td class='style_detail' colspan='3'><div align='left'></div></td></tr>";
