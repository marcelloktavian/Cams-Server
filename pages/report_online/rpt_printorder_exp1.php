...
error_reporting(0);
	...
	*/
	...
	if ($id_exp != null)
	{
	$filter_title.=" AND so.id_expedition=$id_exp ";
	}
	
	$sql_title ="SELECT (SELECT i.id as id_kirim FROM olnso so LEFT JOIN `olnso_id` i on (so.id_trans=i.id_trans)  WHERE (so.state='1') and (so.deleted=0) $filter_title ORDER BY i.id ASC LIMIT 1) AS first_ship,(SELECT i.id as id_kirim FROM olnso so LEFT JOIN `olnso_id` i on (so.id_trans=i.id_trans)  WHERE (so.state='1') and (so.deleted=0) $filter_title ORDER BY i.id DESC LIMIT 1) AS last_ship,(SELECT so.id_trans FROM olnso so LEFT JOIN `olnso_id` i on (so.id_trans=i.id_trans)  WHERE (so.state='1') and (so.deleted=0) $filter_title ORDER BY so.id_trans ASC LIMIT 1) AS first_order,
    (SELECT so.id_trans FROM olnso so LEFT JOIN `olnso_id` i on (so.id_trans=i.id_trans) WHERE (so.state='1') and (so.deleted=0) $filter_title ORDER BY so.id_trans DESC LIMIT 1) AS last_order ,
    (SELECT COUNT(so.id_trans) FROM olnso so LEFT JOIN `olnso_id` i on (so.id_trans=i.id_trans) WHERE (so.state='1') and (so.deleted=0) $filter_title) AS jumlah_order,
	(SELECT SUM(so.totalqty) FROM olnso so LEFT JOIN `olnso_id` i on (so.id_trans=i.id_trans) WHERE (so.state='1') and (so.deleted=0) $filter_title) AS jumlah_qty";
...
<form id="form2" name="form2" action="" method="post"  onSubmit="return validasi(this)">
  ...
  <tr>
  <?
    //$sql_exp = "SELECT e.`nama`,e.kode_warna,jual.totalexp FROM mst_expedition e LEFT JOIN (SELECT id_expedition,COUNT(id_expedition)AS totalexp FROM olnso WHERE (state='1') AND DATE(tgl_trans) BETWEEN STR_TO_DATE('$tglstart','%d/%m/%Y') AND STR_TO_DATE('$tglend','%d/%m/%Y') GROUP BY id_expedition) AS jual ON e.id=jual.id_expedition";
	
    $sql_exp = "SELECT e.`nama`,e.kode_warna,jual.totalexp FROM mst_expedition e LEFT JOIN (SELECT so.id_expedition,COUNT(so.id_expedition)AS totalexp FROM olnso so LEFT JOIN olnso_id i ON so.id_trans=i.id_trans WHERE (so.state='1') $filter_title GROUP BY so.id_expedition) AS jual ON e.id=jual.id_expedition where e.deleted=0";
...
?>
  ...
  <table class="cut-off "width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
	...
	$where = "WHERE m.state='1' and m.deleted=0 AND DATE(m.lastmodified) BETWEEN STR_TO_DATE('$tglstart','%d/%m/%Y') AND STR_TO_DATE('$tglend','%d/%m/%Y') ";
	/*
	if(($id_start != null) and ($id_end != null))
	{
	...
	*/
	...
	if ($id_exp != null)
	{
	$where .=" AND m.id_expedition=$id_exp ";
	}
    ...
    LEFT JOIN mst_expedition e ON m.id_expedition = e.id ".$where." order by (e.id),(i.id),m.id_trans,dt.id_so_d asc";
	...
	if (($expedition!=$rs2['expedition'])&&($expedition!='')){
	echo"<tr><td class='style_detail_left' colspan='5'>SUMMARY of EXPEDITION<div align='center'></div></td>";
	echo"<td class='style_detail'><div align='center'>".$qtyexpedition."</div></td>";
	echo"<td class='style_detail'><div align='left'></div></td>";
	echo"<td class='style_detail'><div align='left'></div></td></tr>";
  ...
  ?>
    <tr>
	  ...
	  {
	    $nomer++;
	    if ($expedition!=$rs2['expedition']){
		$nomer=1;
		$qtyexpedition=0; 	
		}
		echo"<td class='style_detail_left'><div align='center'>".$rs2['id_kirim']."(".$nomer.")</div></td>";
