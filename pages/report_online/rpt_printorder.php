...
error_reporting(0);
	...
	*/
	...
	{
	$filter_title=" AND DATE(so.lastmodified) BETWEEN STR_TO_DATE('$tglstart','%d/%m/%Y') AND STR_TO_DATE('$tglend','%d/%m/%Y') AND i.id_ship BETWEEN '$ship_start' AND '$ship_end'";
	}
	else
	{
...
?>
  ...
  <table class="cut-off "width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
	...
	$where = "WHERE m.state='1' and m.deleted=0 ";
	/*
	if(($id_start != null) and ($id_end != null))
	{
	...
	*/
	if(($ship_start != null) and ($ship_end != null))
	{
	$where .=" AND DATE(m.lastmodified) BETWEEN STR_TO_DATE('$tglstart','%d/%m/%Y') AND STR_TO_DATE('$tglend','%d/%m/%Y') AND i.id BETWEEN '$ship_start' AND '$ship_end'";
	}
	else
	{
  ...
  ?>
    <tr>
	<!--
      <td class="style_detail_left"><div align="center"><?=$rs2['alias_id'];?></div></td>
