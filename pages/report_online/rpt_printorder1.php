...
error_reporting(0);
	...
	{
	$filter_title=" AND DATE(tgl_trans) BETWEEN STR_TO_DATE('$tglstart','%d/%m/%Y') AND STR_TO_DATE('$tglend','%d/%m/%Y') AND SUBSTRING(id_trans,8,6) BETWEEN '$id_start' AND '$id_end'";
	}
	else
	{
	$filter_title=" AND DATE(tgl_trans) BETWEEN STR_TO_DATE('$tglstart','%d/%m/%Y') AND STR_TO_DATE('$tglend','%d/%m/%Y') ";
	}
	
	$sql_title ="SELECT (SELECT id_trans FROM olnso WHERE (state='1') and (deleted=0) $filter_title ORDER BY id_trans ASC LIMIT 1) AS first_order,
    (SELECT id_trans FROM olnso WHERE (state='1') and (deleted=0) $filter_title ORDER BY id_trans DESC LIMIT 1) AS last_order ,
    (SELECT COUNT(id_trans) FROM olnso WHERE (state='1') and (deleted=0) $filter_title) AS jumlah_order,
	(SELECT SUM(totalqty) FROM olnso WHERE (state='1') and (deleted=0) $filter_title) AS jumlah_qty";
...
<form id="form2" name="form2" action="" method="post"  onSubmit="return validasi(this)">
    ...
    <tr>
      <td height="123" valign="top"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
          ...
          <tr>
            ...
            <?php echo"".$tglstart;?>
            &nbsp;-&nbsp;<?php echo"".$tglend;?> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Order:&nbsp;<?php echo"".$rs_title['jumlah_order'];?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Qty:&nbsp;<?php echo"".$rs_title['jumlah_qty'];?> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Nomor_awal:&nbsp;<?php echo"".$rs_title['first_order'];?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Nomor_akhir:&nbsp;<?php echo"".$rs_title['last_order'];?></td>           
		  </tr>
  ...
  <?
    ...
    $sql_exp = "SELECT e.`nama`,e.kode_warna,jual.totalexp FROM mst_expedition e LEFT JOIN (SELECT id_expedition,COUNT(id_expedition)AS totalexp FROM olnso WHERE (state='1') $filter_title GROUP BY id_expedition) AS jual ON e.id=jual.id_expedition where e.deleted=0";
...
?>
  ...
  <table class="cut-off "width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
	...
	if(($id_start != null) and ($id_end != null))
	{
	$where .= " AND DATE(m.tgl_trans) BETWEEN STR_TO_DATE('$tglstart','%d/%m/%Y')  AND STR_TO_DATE('$tglend','%d/%m/%Y') AND SUBSTRING(m.id_trans,8,6) BETWEEN '$id_start' AND '$id_end'";
	}
	else
	{
    ...
    LEFT JOIN mst_expedition e ON m.id_expedition = e.id ".$where." order by m.id_trans,dt.id_so_d asc";
