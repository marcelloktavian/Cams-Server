...
$allow_add = is_show_menu(ADD_POLICY, Supplier, $group_acess);
$allow_edit = is_show_menu(EDIT_POLICY, Supplier, $group_acess);
$allow_delete = is_show_menu(DELETE_POLICY, Supplier, $group_acess);
...
//--------------end of searching--------------------		
     //   $where = "WHERE deleted=0 ";
        $q = $db->query("SELECT * FROM `mst_supplier` a ".$where);
		$count = $q->rowCount();
        ...
        foreach($data1 as $line) {
			...
			if($allow_edit)
				$edit = '<a onclick="javascript:popup_form(\''.BASE_URL.'pages/master_biaya/supplier.php?action=edit&id='.$line['id'].'\',\'table_supplier\')" href="javascript:;">Edit</a>';
			else
				$edit = '<a onclick="javascript:custom_alert(\'Not Allowed\')" href="javascript:;">Edit</a>';
        	
			if($allow_delete)
				$delete = '<a onclick="javascript:link_ajax(\''.BASE_URL.'pages/master_biaya/supplier.php?action=delete&id='.$line['id'].'\',\'table_supplier\')" href="javascript:;">Delete</a>';
			else
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'process') {
		...
		if(isset($_POST['id'])) {
            $query = $db->prepare("SELECT * FROM `mst_supplier` WHERE hp=? AND id<>? AND hp<>'0'");
            $query->execute(array($_POST['hp'],$_POST['id']));
            if ($query->rowCount() == 0) {
                $stmt = $db->prepare("UPDATE mst_supplier SET nama=?,alamat=?,no_telp=?,hp=?,user=?, lastmodified = NOW() WHERE id=?");
                $stmt->execute(array($_POST['nama'],$_POST['alamat'],$telp,$_POST['hp'], $_SESSION['user']['username'], $_POST['id']));
                //var_dump($stmt);die;
                $affected_rows = $stmt->rowCount();
                if($affected_rows > 0) {
                    $r['stat'] = 1;
            ...
            $query = $db->prepare("SELECT * FROM `mst_supplier` WHERE hp=? AND hp<>'0'");
            $query->execute(array($_POST['hp']));
            if ($query->rowCount() == 0) {
			    $stmt = $db->prepare("INSERT INTO  mst_supplier(`nama`,`alamat`,`no_telp`,`hp`,`user`,`lastmodified`) VALUES(?, ?, ?, ?, ?, NOW())");
			    if($stmt->execute(array($_POST['nama'],$_POST['alamat'], $telp, $_POST['hp'],$_SESSION['user']['username']))) {
				    $r['stat'] = 1;
				    $r['message'] = 'Success';
			    }
			    else {
...
<?php
	...
	if($allow_add) {
		echo '<button type="button" onclick="javascript:popup_form(\''.BASE_URL.'pages/master_biaya/supplier.php?action=add\',\'table_supplier\')" class="btn">Tambah</button>';
	}}
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#table_supplier").jqGrid({
            url:'<?php echo BASE_URL.'pages/master_biaya/supplier.php?action=json'; ?>',
            /*postData: {
            ...
            datatype: "json",
            colNames:['ID','Name','Address','Phone','Edit','Delete'],
            colModel:[
