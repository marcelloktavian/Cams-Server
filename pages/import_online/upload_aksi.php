...
for ($i=2; $i<=$jumlah_baris; $i++){
	...
	if($oln_order_id != ""){
		// input data ke database (table oln_xlscamou)
		
		$sql_insert="INSERT into oln_xlscamou values(
		''
		,'$oln_order_id'  
		,'$oln_productid' 
		,'$oln_productname' 
		...
		$sql_update_product="";
		$sql_update_product="UPDATE oln_xlscamou xls SET xls.product_id=(SELECT id FROM mst_products p WHERE (p.oln_product_id=xls.oln_productid) AND(p.size=xls.oln_size)),xls.dropshipper_id = (SELECT id FROM mst_dropshipper d WHERE (d.oln_customer_id=xls.oln_customerid))";
...
header("location:importcamou.php?berhasil=$berhasil");
?>