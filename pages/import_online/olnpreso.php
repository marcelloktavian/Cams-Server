...
<?php
	if(isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
       ...
       $operations = array(
        ...
        foreach($data1 as $line) {
			...
			$edit = '<a onclick="window.open(\''.BASE_URL.'pages/import_online/olnpreso_detail_edit.php?ids='.$line['oln_order_id'].'\',\'table_olnpreso\')" href="javascript:;">Posting</a>';
			}
			else
			...
			if(in_array($_SESSION['user']['access'], $allowDelete))
				$delete = '<a onclick="javascript:link_ajax(\''.BASE_URL.'pages/import_online/olnpreso.php?action=delete&id='.$line['oln_order_id'].'\',\'table_preso\')" href="javascript:;">Cancel</a>';
			else
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'delete') {
		...
		$stmt = $db->prepare("DELETE olnpreso WHERE oln_order_id=?");
		$stmt->execute(array($_GET['id']));
		//var_dump($stmt);die;
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $("#table_olnpreso").jqGrid({
            url:'<?php echo BASE_URL.'pages/import_online/olnpreso.php?action=json'; ?>',
            datatype: "json",
            ...
            subGrid : true,
            subGridUrl : '<?php echo BASE_URL.'pages/import_online/olnpreso.php?action=json_sub'; ?>',
            subGridModel: [
			            	{ 
