...
$id=$_GET['ids'];
$sql_init="SELECT p.deposit,sum(p.harga_satuan) as harga_satuan,sum(p.tax) as tax,p.total as grandtotal,p.oln_customer_telp,p.oln_order_id,p.oln_tgl,p.oln_customerid,p.oln_customer,p.id_dropshipper,p.oln_tgl,p.oln_penerima,p.oln_address,p.oln_provinsi,p.oln_postalcode,p.oln_kotakab,p.oln_kecamatan,p.oln_telp,p.oln_expnote,sum(p.jumlah_beli) as qty, sum(p.subtotal) as total,sum(p.deposit) as deposit,p.oln_note,p.oln_expnote,d.disc as discdp FROM `olnpreso` p LEFT JOIN mst_dropshipper d on p.id_dropshipper=d.id  WHERE p.oln_order_id ='".$id."' GROUP by p.oln_order_id ";
...
<table>
    <tr>
		...
		<input type='text' name='id_expedition' id='id_expedition'/></td>
	 ...
	 <tr>
        ...
        <td><input type='text' class='inputform' name='exp_fee' id='exp_fee' placeholder='Biaya Ekspedisi' value='$expedisifee' onkeyup='hitungtotaldisable();'/></td>
...
<td><input type='text' class='inputform' name='transfer' id='transfer' style='text-align:right;'onkeyup='hitungpiutangdisable();' value='$grandtotal'></td>
<td class='fonttext' >&nbsp;</td>
</tr>
<tr>
