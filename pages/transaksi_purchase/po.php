...
if(isset($_GET['action']) && strtolower($_GET['action'])=='json'){
  ...
  if($_REQUEST['_search']=='false'){
    $where = ' WHERE a.approval=0 AND a.proforma=0 AND a.deleted=0 ';
  }else {
    $operations = array (
      'eq' => "= '%s'",            // Equal
      'ne' => "<> '%s'",           // Not equal
    ...
    $where = sprintf(" WHERE a.approval=0 AND a.proforma=0 AND a.deleted=0 AND %s ". $operations[$_REQUEST['searchOper']], $_REQUEST['searchField'], $value);
  }
  ...
  foreach($data1 as $line){
    if($allow_edit)
      $edit = '<a onclick="javascript:window_open(\''.BASE_URL.'pages/transaksi_purchase/po_edit.php?id='.$line['id'].'\',\'table_po\')" href="javascript:void(0);">Edit</a>';
    else
...
<script type="text/javascript">
  $(document).ready(function(){
    $('#table_po').jqGrid({
      ...
      datatype  : 'json',
      colNames  : ['ID','Dokumen','Pemohon','Supplier','Tanggal PO','Estimasi Pengiriman','Total Qty','Total DPP','PPN','Grand Total','Note','Approval','Edit','Delete'],
      colModel  : [
