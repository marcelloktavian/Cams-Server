...
if (isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
  ...
  $q_get_product = $db->query("SELECT di.qty,di.price,mp.kode,mp.durasi,mp.ppn,mp.tipe,di.nama_produk AS produk_jasa,i.tanggal_invoice AS lastmodified,di.id_akun,mp.id_akun AS akun_cr,mp.nomor_akun,mp.nama_akun FROM det_invoice di JOIN mst_produk mp ON di.id_produk = mp.id JOIN mst_invoice i ON di.id_invoice = i.id WHERE mp.kode > 0 AND di.id_invoice = $_GET[id] AND di.deleted = 0 AND mp.kode > 0;")->fetchAll(PDO::FETCH_ASSOC);
  $find_by_ref = $db->query("SELECT COUNT(nama) as count FROM `penyusutan` WHERE ref_id = $_GET[id]")->fetchAll(PDO::FETCH_ASSOC);


  if ($_GET['val'] == 1) {
    if (!$find_by_ref[0]['count']) {
      for ($i = 0; $i < count($q_get_product); $i++) {
        $item = $q_get_product[$i];
        for ($j = 1; $j <= $item['qty']; $j++) {
          $db->prepare("INSERT INTO penyusutan SET nama = ?, tgl_pembelian = ?,durasi = ?, dpp = ?,kode = ?,tipe = ?,is_add = 0,id_akun = ?,ref_id = ?,deleted = 0,akun_cr = ?,nomor_akun = ?,nama_akun = ?")
            ->execute([$item['qty'] == 1 ? $item['produk_jasa'] : "$item[produk_jasa] Ke $j", $item['lastmodified'], $item['durasi'], $item['price'], $item['kode'], $item['tipe'], $item['id_akun'], $_GET['id'], $item['akun_cr'], $item['nomor_akun'], $item['nama_akun']]);
        }
      }
    } else {
      $db->query("UPDATE penyusutan SET hide = 0 WHERE ref_id = $_GET[id]");
    }
  } else {
    if ($find_by_ref[0]['count'] > 0) {
      $db->query("UPDATE penyusutan SET hide = 1 WHERE ref_id = $_GET[id]");
    }
  }
  ...
  $db->query("DELETE FROM penyusutan WHERE ref_id = $_GET[id]");
...
<script type="text/javascript">
  ...
  function gridReloadInvoice() {
    ...
    jQuery("#table_invoice").setGridParam({
      url: v_url,
      page: 1
    }).trigger("reloadGrid");
  }
  ...
  $(document).ready(function() {
    ...
    $('#table_invoice').jqGrid('navGrid', '#pager_table_invoice', {
      edit: false,
      add: false,
      del: false,
      search: false
