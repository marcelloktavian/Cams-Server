...
<form id="form2" name="form2" action="" method="post"  onSubmit="return validasi(this)">
  ...
  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
    <tr>
      <td width="5%" class="style21"><div align="left">No.</div></td>
      <td width="35%" class="style21"><div align="left">Jenis Barang</div></td>
	  ...
	  <td width="30%" class="style21"><div align="right">Subtotal(yard)</div></td>
      </div></td>
	  </div></td>
    </tr>
    <tr>
	...
	$sql_data = "Select a.id_jenis,j.hrg_yard,a.harga_yard,j.nm_jenis, sum(a.qty) as pcs, sum(a.qty*30) as yard,sum(a.qty * j.hrg_yard) as subtotalpcs,sum(a.qty * a.harga_yard*30) as subtotalyard from trbeli_detail a left join barang b on b.id_barang = a.id_barang left join jenis_barang j on j.id = a.id_jenis where a.id_trans ='".$_GET['id_trans']."' group by j.hrg_yard order by a.qty asc";
  ...
  ?>
    <tr>
	  ...
	  <td class="style9"><div align="right">
          <?=number_format($rs2['pcs'],0);?>
      </div></td>
	  
	  <td class="style9"><div align="right">
          <?=number_format($rs2['yard']*$konversi_yard,0);?>
      </div></td>
	  ...
	  <td class="style9"><div align="right">
          <?=number_format($rs2['subtotalyard'],0);?>
      </div></td>
    </tr>  <?
	$total2+=$rs2['pcs'];
	$totalyard+=($rs2['yard']);
	$grandtotal+=($rs2['subtotalyard']);
  }
  ?>
    <tr>
 ...
 -->
  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
	...
	<tr>
            <td colspan="5" width="90%"><div align="right"><strong>PPN (10%)</strong></div></td>
			<td width="10%" class="style10"><div id="ppn" align="right" class="style21"><? echo number_format(0.11*$grandtotal,0);?></div></td>
    </tr>
    <tr>
            <td colspan="5" width="90%"><div align="right"><strong>Grand Total</strong></div></td>
