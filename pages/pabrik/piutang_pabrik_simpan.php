...
include("../../include/koneksi.php");
	...
	for ($i=1; $i<$row; $i++)
	{
		...
		$Deposit = str_replace(",","", ($_POST['Deposit'.$i]));
		$totalfaktur += $Bayar+$Bank+$Deposit ;
		$Piutang_update=$Sisa-($Bayar+$Bank+$Deposit);
		/*
	...
	mysql_query("insert into trbelipiutang(id_trans,id_transjual,tgl_trans,id_supplier,totalfaktur,totalqty,faktur,tunai,transfer,piutang,deposit,simpan_deposit,keterangan,info,id_user)values('".$id_pkb."','".$Id_Part."','".$tgl."','".$id_supplier."','".$totalfaktur."','".$totalqty."','".$faktur."','".$tunai."','".$transfer."','".$sisapiutang."','".$Deposit."','".$simpan_deposit."','".$keterangan."','".$info."','".$id_user."')") or die (mysql_error());
	//---akhir simpan master----------------------
	//---simpan deposit pelanggan--------------------------------
		if ($Deposit > 0)//berlaku bila bayar pakai deposit
		{
		$sql_deposit="insert into trdepositpb(id_trans,kode,tgl_trans,id_customer,totalfaktur,tunai,deposit,catatan) values('".$id_pkb."','".$id_pkb."',NOW(),'".$id_supplier."','-".$Deposit."','-".$Deposit."','-".$Deposit."','PIUTANG PAKAI DEPOSIT')";
		mysql_query($sql_deposit)or die (mysql_error());
		}
        else if($simpan_deposit > 0)//berlaku bila simpan_deposit
	    {
		$sql_deposit="insert into trdepositpb(id_trans,kode,tgl_trans,id_customer,totalfaktur,tunai,deposit,catatan) values('".$id_pkb."','".$id_pkb."',NOW(),'".$id_supplier."','".$simpan_deposit."','".$simpan_deposit."','".$simpan_deposit."','PIUTANG SIMPAN DEPOSIT')";
		//var_dump($sql_deposit);die;
		mysql_query($sql_deposit)or die (mysql_error());
		}
	//---end of simpan deposit pelanggan--------------------------------
