...
error_reporting(0);
	...
	$id_faktur=$_GET['id_trans'];
	$sql_note="SELECT a.*,b.namaperusahaan as pelanggan,b.alamat FROM trbelipiutang a left join tblsupplier b on a.id_supplier = b.id where a.id_trans='".$id_faktur."'";
	//var_dump($sql_note);die;
	
	$sql = mysql_query($sql_note);
	
	//$sql = mysql_query("SELECT a.*,b.namaperusahaan as pelanggan,b.alamat FROM trbelipiutang a left join tblsupplier b on a.id_ = a.id_customer where a.id_trans='".$id_faktur."'");
...
<form id="form2" name="form2" action="" method="post"  onSubmit="return validasi(this)">
    ...
    <tr>
      <td height="123" valign="top"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
          ...
          <tr>
            ...
            <td class="style9" width="1%"><div align="center">:</div></td>           
            <td class="style9" width="30%"class="style9"><?=$rs['pelanggan'];?></td>           
		  </tr>
          <tr>
  ...
  ?>
	...
	$bank_byr+=$rs2['bank'];
	$tunai_byr+=$rs2['bayar'];
	$deposit_byr=$rs['deposit'];
	$jumlahbyr=($rs2['bayar'] + $rs2['bank']+ $rs['deposit']);
	$piutangdt+=$rs2['piutang'];
	$totalbyrdt+=$rs2['totalbyr']-$rs['deposit'];
		
	if ($rs2['piutang_update'] > 0){ 
	$sisapiutangdt+=$rs2['piutang_update']+$rs['deposit'];
	$simpan_deposit= 0;
	}
	else if ($rs2['piutang_update'] < 0)
	{
	 $sisapiutangdt = 0;
	 $simpan_deposit=$rs['simpan_deposit'];
	}
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
      ...
      <td width="10%" class="style9" align="right"><strong>DEPOSIT</strong></td>
      <td width="10%">
		<div id="total" align="right" class="style20">
		<? echo number_format($deposit_byr,0);?>
		</div>
	  </td>	 
    </tr>
  </table> 
