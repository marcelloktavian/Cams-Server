...
<table border ="1px">
			<thead>
                <tr>
                    <th colspan='8'>DATA PENJUALAN RETURN CASH</th>
                </tr>
				<tr>
			...
			<?php 		
			$no=1;
				$data = mysql_query("SELECT *,jurnal.total_kredit, ROUND(jurnal.total_kredit/1.11) AS dpp, ROUND(jurnal.total_kredit/1.11*0.11) AS ppn,jurnal.keterangan as ket FROM jurnal 
				WHERE jurnal.keterangan LIKE '%Retur OLN Cash -%'");
			while($d = mysql_fetch_array($data)){
                ...
                WHERE nama_akun LIKE '%Penjualan OLN%' AND id_parent='".$d['id']."' ");
	...
	<table border ="1px">
			<thead>
                <tr>
                    <th colspan='8'>DATA PENJUALAN RETURN CREDIT</th>
                </tr>
				<tr>
					<th class="text-left">No.</th>
					<th class="text-left">ID</th>
			...
			<?php 		
			$no=1;
				$data = mysql_query("SELECT *,jurnal.total_debet, ROUND(jurnal.total_debet/1.11) AS dpp, ROUND(jurnal.total_debet/1.11*0.11) AS ppn,jurnal.keterangan as ket FROM jurnal 
				WHERE jurnal.keterangan LIKE '%Retur OLN Kredit -%'");
			while($d = mysql_fetch_array($data)){
				?>
				<tr>
					<td class="text-left"><?php echo $no; ?></td>
