...
require "../../include/config.php";
    $id = $_GET["id"];
    $res = $db->prepare(" SELECT * FROM biayaoperasional where id=? ");
...
<form id='form2' name='form2' action='' method='post'>
    <table width='100%'>
  	<tr>
		...
		<input type='text' class='' name='subtotal_m' id='subtotal_m' value='<?php echo "IDR ".number_format($data['subtotal'],2,'.',',') ?>' style='text-align:right;font-size: 30px;background-color:white;width: 300px;height:40px;border:1px dotted #f30; border-radius:4px; -moz-border-radius:4px;' />
		<!-- subtotal hidden -->
		<input type="hidden" name="subtotal" id="subtotal" value="<?php echo $data['subtotal'] ?>">
		<!--  ppn -->
        <td class='fontjudul'> PPN
		<input type='text' class='' name='PPN_m' id='PPN_m' value='<?php echo "IDR ".number_format($data['ppn'],2,'.',',') ?>' style='text-align:right;font-size: 30px;background-color:white;width: 300px;height:40px;border:1px dotted #f30; border-radius:4px; -moz-border-radius:4px;' />
		<!-- ppn hidden -->
		<input type="hidden" name="PPN" id="PPN" value="<?php echo $data['ppn'] ?>">
		 <!-- grandtotal -->
        <td class='fontjudul'> TOTAL <input type='text' class='' name='grandtotal_m' id='grandtotal_m' value='<?php echo "IDR ".number_format($data['total'],2,'.',',') ?>' style='text-align:right;font-size: 30px;background-color:white;width: 300px;height:40px;border:1px dotted #f30; border-radius:4px; -moz-border-radius:4px;' />
		<!-- grandtotal hidden -->
		<input type="hidden" name="grandtotal" id="grandtotal" value="<?php echo $data['total'] ?>">
		</td>
    </tr>
</table>
    <hr />
...
<tr>
        ...
        <td><input type='text' class='inputform' name='kode' id='kode' placeholder='Kode' value='<?php echo $data['kode'] ?>' />
        </td>
    </tr>
    <tr>
		<td class='fonttext'>Tanggal</td>
        <td><input type='date' class='inputform' name='tanggal' id='tanggal' value='<?php echo $data['tanggal'] ?>' />
        </td>
    </tr>
     <tr height='5'>
     <td colspan='6'></td>
...
function cetak(){
	...
	{	var answer = confirm("Mau Simpan datanya????")
		...
		hitungrow();
		// hitung() ;
		document.form2.action="simpanOperasional.php?trans=EDIT";
...
<?php 
$sqldetail = $db->prepare("SELECT det.*, jnmst.nama_jenis FROM biayaoperasional_det det LEFT JOIN det_jenisbiaya jndet ON jndet.id=det.id_det_jenisbiaya LEFT JOIN mst_jenisbiaya jnmst ON jnmst.id=jndet.id_parent WHERE det.id_parent=?");
...
document.getElementById('idbiaya'+<?=$i;?>+'').value = '<?=$result['id_det_jenisbiaya'];?>';
document.getElementById('namabiaya'+<?=$i;?>+'').value = '<?=$result['nama_biaya'];?>';
document.getElementById('namajenis'+<?=$i;?>+'').value = '<?=$result['nama_jenis'];?>';
document.getElementById('satuan'+<?=$i;?>+'').value = '<?=$result['satuan'];?>';
document.getElementById('Qty'+<?=$i;?>+'').value = '<?=$result['qty'];?>';
document.getElementById('hargaSatuan'+<?=$i;?>+'').value = '<?=$result['harga_satuan'];?>';
document.getElementById('Subtotal'+<?=$i;?>+'').value = '<?=$result['subtotal'];?>';
document.getElementById('ppn'+<?=$i;?>+'').value = '<?=$result['ppn'];?>';
<?php 
$i++;
}
?>
