<?php
    ...
    if($trigger == 'INSERT'){
        ...
        // execute for master
        $sql_master="INSERT INTO `cashreceipt`(`kode`, `tanggal`, `total_debet`, `total_kredit`, `keterangan`, `deleted`, `user`, `lastmodified`) VALUES ('$masterkode','$masterTanggal','$masterDebet','$masterKredit','$keterangan','0','$id_user',NOW()) ";
        ...
        // execute for master
        $sql_master="UPDATE `cashreceipt` SET `kode`='$masterkode',`tanggal`='$masterTanggal',`total_debet`='$masterDebet',`total_kredit`='$masterKredit',`keterangan`='$keterangan',`user`='$id_user',`lastmodified`=NOW() WHERE `id`='$masterid' ";
	    mysql_query($sql_master) or die (mysql_error());
