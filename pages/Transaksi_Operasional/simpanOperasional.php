...
//    echo $idbo;
    ...
    if($trigger == 'INSERT'){
        ...
        // execute for master
        $sql_master="INSERT INTO `biayaoperasional`(`kode`,`tanggal`,`subtotal`, `ppn`, `total`, `deleted`, `user`, `lastmodified`) VALUES ('$masterkode','$masterTanggal','$masterSubtotal','$masterPPN','$masterGrandtotal','0','$id_user',NOW()) ";
        ...
        for ($i=1; $i<$count; $i++)
	    {
            ...
            $sql_detail="INSERT INTO biayaoperasional_det VALUES(NULL,'$idparent','$idbiaya','$namabiaya','$qty','$satuan','$harga_satuan','$ppn','$jumlah',NOW()) ";
        ...
        // execute for master
        $sql_master="UPDATE biayaoperasional SET kode='$masterkode',tanggal='$masterTanggal',subtotal=$masterSubtotal,ppn=$masterPPN,total=$masterGrandtotal, user='$id_user', lastmodified=NOW() WHERE id='$masterid' ";
        ...
        for ($i=1; $i<$count; $i++)
	    {
            ...
            if ($iddetail == '' && $idbiaya == '' && $delete == ''){
                ...
                if($delete == '' && $iddetail== ''){
                    // insert ke detail
                    $sql_detail="INSERT INTO biayaoperasional_det VALUES(NULL,'$masterid','$idbiaya','$namabiaya','$qty','$satuan','$harga_satuan','$ppn','$jumlah',NOW()) ";
                    // var_dump($sql_detail.'   s');
	                mysql_query($sql_detail) or die (mysql_error());
                } else if($delete == '' && $iddetail != ''){
                    // update detail
                    $detailQuery="UPDATE biayaoperasional_det SET id_det_jenisbiaya='$idbiaya',nama_biaya='$namabiaya',satuan='$satuan',qty='$qty',harga_satuan='$harga_satuan',ppn='$ppn',subtotal='$jumlah' WHERE id='$iddetail' ";
