...
// 
 ...
 if ($filter == '') {
  ...
  ".$where." AND kat.id<>9 AND mstbiaya.id<>27 AND (detbiaya.id <> 163 AND detbiaya.id <> 164 AND detbiaya.id <> 186 AND detbiaya.id <> 187 AND detbiaya.id <> 269 AND detbiaya.id <> 490)
...
<tr>
  ...
  <div id="right" align='right'><?= rupiah($data['subtotal']) ?></div>
    ...
    <?php
      ...
      ".$where." AND kat.id<>9 AND mstbiaya.id<>27 AND (detbiaya.id <> 163 AND detbiaya.id <> 164 AND detbiaya.id <> 186 AND detbiaya.id <> 187 AND detbiaya.id <> 269 AND detbiaya.id <> 490)
      GROUP BY detbiaya.id
      ORDER BY kat.id ASC, mstbiaya.id ASC, detbiaya.nama_biaya ASC LIMIT ".($jumlah).",1";

      $next = '';
      $result2= mysql_query($sql2);
      while ($data2 = mysql_fetch_array($result2)){
        $next = $data2['nama_jenis'];
      }

      if($next != $jenis2){
    ?>
      <td class='style_detail2'><div id='subtotal_<?= $nomorurut2 ?>'><input type='text' id='subtotalinput_<?= $nomorurut2 ?>' value='<?=$kondisi?>'></div></td>
    <?php
      }else{
        ?>
          <td class='style_detail2'></td>
        <?php
      }
    ?>
    ...
    <script>
      ...
      document.getElementById("subtotal_<?= $nomorurut2 ?>").innerHTML = '<div id="container"><u><?= $data['nama_jenis'] ?></u><br><div id="left2">SUBTOTAL : RP.</div><div id="right2" align="right"><?= rupiah($sub) ?></div></div>';
...
<?php
 ...
 if ($filter == '') {
  ...
  LEFT JOIN mst_jenisbiaya mstbiaya ON mstbiaya.id=detbiaya.id_parent
  LEFT JOIN mst_kategori_biaya kat ON kat.id=mstbiaya.id_kategori
  ".$where." AND (kat.id=9 OR (kat.id=3 AND (detbiaya.id = 163 OR detbiaya.id = 164 OR detbiaya.id =  186 OR detbiaya.id = 187 OR detbiaya.id = 269 OR detbiaya.id = 490)) OR (kat.id=7 AND mstbiaya.id=27))
  GROUP BY detbiaya.id
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <div id="right" align='right'><?= rupiah($data['subtotal']) ?></div>
    ...
    if ($data['nama_jenis'] == 'ANGKUTAN') {
      $where2 = $where." AND (detbiaya.id = 163 OR detbiaya.id = 164 OR detbiaya.id = 186 OR detbiaya.id = 187 OR detbiaya.id = 269 OR detbiaya.id = 490) ";
    }else{
      $where2 = $where;
    }
      $sql2="SELECT kat.nama_kategori, mstbiaya.nama_jenis, detbiaya.nama_biaya ,CONCAT(ROUND(SUM(detop.qty)),' ',detop.satuan) AS jml_satuan, 
