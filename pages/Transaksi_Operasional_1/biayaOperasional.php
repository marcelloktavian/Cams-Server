...
$allow_delete = is_show_menu(DELETE_POLICY, BiayaOperasional, $group_acess);
	...
	if(isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
		...
		$startdate = isset($_GET['startdate_trjual'])?$_GET['startdate_trjual']:date('Y-m-d');
		$enddate = isset($_GET['enddate_trjual'])?$_GET['enddate_trjual']:date('Y-m-d'); 
        $filter=$_GET['filter'];
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'json_sub') {
        ...
        $q = $db->query("SELECT id,namabiaya,satuan,qty,harga_satuan,jumlah FROM biayaoperasional_det  ".$where);
    ...
    <div class="ui-widget-content ui-corner-bottom">
        <form id="report_project_form" method="" action="" class="ui-helper-clearfix">
            ...
            <div class="ui-corner-all form-control">
				...
				<tr>
				<td>
				 <input value="" type="text" class="required datepicker"   id="startdate_trjual" name="startdate_trjual">
				</td>
				<td> s.d.  
				<input value="" type="text" class="required datepicker"  id="enddate_trjual" name="enddate_trjual">
				</td>
				<td> Filter
				 <input value="" type="text" id="filteroperasional" name="filteroperasional">(Nama Biaya,Tanggal,Subtotal,PPN,Total)
				</td>
				</tr>
				</table>
            </div>
...
<script type="text/javascript">
	...
	$('#startdate_trjual').datepicker({
		dateFormat: "dd/mm/yy"
	});
	$('#enddate_trjual').datepicker({
		dateFormat: "dd/mm/yy"
	});
	$( "#startdate_trjual" ).datepicker( 'setDate', '<?php echo date('d/m/Y')?>' );
	$( "#enddate_trjual" ).datepicker( 'setDate', '<?php echo date('d/m/Y')?>' );
	
	
	function gridOperasional(){
		var startdate_trjual = $("#startdate_trjual").val();
		var enddate_trjual = $("#enddate_trjual").val();
		var filter_do = $("#filteroperasional").val();
		var v_url ='<?php echo BASE_URL?>pages/Transaksi_Operasional/biayaOperasional.php?action=json&startdate_trjual='+startdate_trjual+'&enddate_trjual='+enddate_trjual+'&filter='+filter_do;
    ...
    $(document).ready(function(){
        $("#tableoperasionals").jqGrid({
            ...
            datatype: "json",
            colNames:['Tanggal','Total Qty','Subtotal Biaya','PPN','Total','Edit','Delete'],
            colModel:[
            ...
            rownumbers: true,
            sortorder: "desc",
            caption:"Data Biaya Operasional",
            ...
            subGridModel: [
			            	{ 
			            		name : ['No','Biaya','Packing','Qty','Harga','Jumlah'], 
			            		width : [40,100,20,50,50,50],
			            		align : ['right','center','left','left','left','left'],
			            	} 
			            ],
