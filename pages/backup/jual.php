...
<?php
	if(isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
        ...
        //$where = "WHERE TRUE ";
        $q = $db->query("SELECT * FROM `trjual` p ".$where);
		$count = $q->rowCount();
        ...
        foreach($data1 as $line) {
			...
			if(in_array($_SESSION['user']['access'], $allowEdit))
				if($line['is_start'] == 0) {
					...
					$edit = '<a onclick="window.open(\''.BASE_URL.'pages/jual_detail_edit.php?ids='.$line['id_trans'].'\',\'table_jual\')" href="javascript:;">Edit</a>';
			...
			if(in_array($_SESSION['user']['access'], $allowDelete))
				if($line['is_start'] == 0) {
					$delete = '<a onclick="javascript:link_ajax(\''.BASE_URL.'pages/jual.php?action=delete&id='.$line['id_trans'].'\',\'table_jual\')" href="javascript:;">Delete</a>';
				}
				else {
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'json_sub') {
        ...
        $q = $db->query("SELECT pd.id_detail,pd.id_barang,b.nm_barang,pd.id_trans,pd.qty,pd.harga,(pd.qty * pd.harga) as subtotal FROM `trjual_detail` pd INNER JOIN `barang` b ON (pd.id_barang=b.id_barang) ".$where);
...
<a href="javascript: void(0)" 
   onclick="window.open('pages/jual_detail.php', 
  'windowname1', 
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#table_jual").jqGrid({
            url:'<?php echo BASE_URL.'pages/jual.php?action=json'; ?>',
            /*postData: {
            ...
            datatype: "json",
            colNames:['ID','Customer','Tanggal Transaksi','Total Qty','Total Faktur','Edit','Delete'],
            colModel:[
            ...
            subGrid : true,
            subGridUrl : '<?php echo BASE_URL.'pages/jual.php?action=json_sub'; ?>',
            subGridModel: [
			            	{ 
