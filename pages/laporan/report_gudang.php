...
<?php error_reporting(0);
	if(isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
       ...
       $where = "WHERE TRUE AND (p.deleted = 0) AND (p.id_jenis=1 or p.id_jenis=2) ";
       } else {
       $operations = array(
        'eq' => "= '%s'",            // Equal
        'ne' => "<> '%s'",           // Not equal
	  ...
	  $where = sprintf(" where (p.deleted=0) AND (p.id_jenis=1 or p.id_jenis=2) AND %s ".$operations[$_REQUEST["searchOper"]], $_REQUEST["searchField"], $value);
	 //echo"<script>alert('where=$where')</script>";
     }*/
	    
		$startdate = isset($_GET['startdategudang'])?$_GET['startdategudang']:date('Y-m-d');
		...
		//var_dump("respons=".$responce['records']);die;
        foreach($data1 as $line) {
			...
			if(in_array($_SESSION['user']['access'], $allowEdit))
				//if($line['is_start'] == 0) {
					$edit = '<a onclick="window.open(\''.BASE_URL.'pages/laporan/report_gudang.php?ids='.$line['id_trans'].'\',\'table_stokgudang\')" href="javascript:;">Nota</a>';
				//}
				/*else {
        	...
        	//$responce['rows'][$i]['id']   = $line['tgl_trans'];
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'json_sub') {
		...
		$where = "WHERE (sb.id_jenis=1 or sb.id_jenis=2) AND sb.id_barang = '".$id."' order by sb.id desc";
    ...
    <div class="ui-widget-content ui-corner-bottom">
        <form id="report_project_form" method="" action="" class="ui-helper-clearfix">
            ...
            <div class="ui-corner-all form-control">
				...
				<tr> s.d.
				<td>
				 <input value="" type="text" class="required datepicker"   id="startdategudang" name="startdategudang">
				</td>
				</tr>
				</table>
            </div>
...
<div class="btn_box">
<button onclick="javascript:window_open('<?php echo BASE_URL ?>pages/laporan/report_gudang_print.php?start='+$('#startdategudang').val())" class="btn" type="button">CETAK</button>
<!--
<a href="javascript: void(0)" 
   onclick="window.open('pages/laporan/report_gudang_print.php', 
  'windowname1'); 
   return false;">
...
<script type="text/javascript">
    $('#startdategudang').datepicker({
		dateFormat: "dd/mm/yy"
		//dateFormat: "yy-mm-dd"
	});
	
	$( "#startdategudang" ).datepicker( 'setDate', '<?php echo date('d/m/Y')?>' );
	
	
	function gridReloadJual(){
		var startdategudang = $("#startdategudang").val();
		var v_url ='<?php echo BASE_URL?>pages/laporan/report_gudang.php?action=json&startdategudang='+startdategudang;
		jQuery("#table_stokgudang").setGridParam({url:v_url,page:1}).trigger("reloadGrid");
	}
    ...
    $(document).ready(function(){
        ...
        $("#table_stokgudang").jqGrid({
            ...
            rownumbers: true,
            sortorder: "asc",
            caption:"Laporan Stok GUDANG ARROW KELIR dan ARROW PUTIH",
        ...
        $("#table_stokgudang").jqGrid('navGrid','#pager_table_stokgudang',{edit:false,add:false,del:false,search:false});
    })
</script>