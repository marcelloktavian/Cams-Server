...
<form id="form2" name="form2" action="" method="post"  onSubmit="return validasi(this)">
  ...
  <table width="100%" id="tbd" border="0" align="center" cellpadding="0" cellspacing="0">
	...
	/*
	$sql_union="
	Select 'JUAL' as trans,'' as id_transjual,tgl_trans,date_format(tgl_trans,'%d-%m-%Y')as tgl_edit,id_customer, id_trans,kode,faktur,biaya,tunai,transfer,totalfaktur,(totalfaktur-piutang) as bayar_faktur,piutang as piutang_faktur,'' as jualdeposit,'' as simpan_deposit,'' as piutang_tunai ,'' as piutang_transfer,'' as piutang_bayar from trjual ".$where."   
    union all
	Select 'JUALBAYAR' as trans,'' as id_transjual,tgl_trans,date_format(tgl_trans,'%d-%m-%Y')as tgl_edit,id_customer,id_trans,kode,faktur,biaya,tunai,transfer,totalfaktur,(totalfaktur-piutang) as bayar_faktur,piutang as piutang_faktur,'' as jualdeposit,'' as simpan_deposit,'' as piutang_tunai ,'' as piutang_transfer,'' as piutang_bayar from trjual ".$where." and ((tunai+transfer)>0)
    union all
    Select 'SIMPANDEPOSIT' as trans,'' as id_transjual,tgl_trans,date_format(tgl_trans,'%d-%m-%Y')as tgl_edit,id_customer,id_trans,kode,faktur,biaya,tunai,transfer,totalfaktur,(totalfaktur-piutang) as bayar_faktur,piutang as piutang_faktur,'' as jualdeposit,simpan_deposit as simpan_deposit,'' as piutang_tunai ,'' as piutang_transfer,'' as piutang_bayar from trjual ".$where." and ((simpan_deposit)>0)
    union all
    Select 'JUALBYRDEPOSIT' as trans,'' as id_transjual,tgl_trans,date_format(tgl_trans,'%d-%m-%Y')as tgl_edit,id_customer,id_trans,kode,faktur,biaya,tunai,transfer,totalfaktur,(totalfaktur-piutang) as bayar_faktur,piutang as piutang_faktur,deposit as jualdeposit,'' as simpan_deposit,'' as piutang_tunai ,'' as piutang_transfer,'' as piutang_bayar from trjual ".$where." and (deposit > 0)
    union all
	Select 'PIUTANG' as trans,id_transjual,tgl_trans,date_format(tgl_trans,'%d-%m-%Y')as tgl_edit,id_customer,id_trans,kode,'','','','','','','','','',tunai,transfer,totalfaktur from trpiutang ".$where."  
    order by tgl_trans asc,trans asc";
	*/
	$sql_union="
	Select 'DEPOSIT' as trans,'' as id_transjual,tgl_trans,date_format(tgl_trans,'%d-%m-%Y')as tgl_edit,id_customer, id_trans,kode,faktur,biaya,tunai,transfer,totalfaktur,(totalfaktur-piutang) as bayar_faktur,piutang as piutang_faktur,'' as jualdeposit,'' as simpan_deposit,'' as piutang_tunai ,'' as piutang_transfer,'' as piutang_bayar from trdeposit ".$where." and id_trans like 'TD%'   
    union all
	Select 'JUAL' as trans,'' as id_transjual,tgl_trans,date_format(tgl_trans,'%d-%m-%Y')as tgl_edit,id_customer, id_trans,kode,faktur,biaya,tunai,transfer,totalfaktur,(totalfaktur-piutang) as bayar_faktur,piutang as piutang_faktur,'' as jualdeposit,'' as simpan_deposit,'' as piutang_tunai ,'' as piutang_transfer,'' as piutang_bayar from trjual ".$where."   
    union all
	Select 'JUALBAYAR' as trans,'' as id_transjual,tgl_trans,date_format(tgl_trans,'%d-%m-%Y')as tgl_edit,id_customer,id_trans,kode,faktur,biaya,tunai,transfer,totalfaktur,(totalfaktur-piutang) as bayar_faktur,piutang as piutang_faktur,'' as jualdeposit,'' as simpan_deposit,'' as piutang_tunai ,'' as piutang_transfer,'' as piutang_bayar from trjual ".$where." and ((tunai+transfer)>0)
	...
	{ 
	  ...
	  $jumlah_piutang="";
	  $kas=$rs2['tunai'];//tunai
	  $bca=$rs2['transfer'];//transfer
	  $total_bayar=$rs2['totalfaktur'];
      $tmp= -($rs2['totalfaktur']);	  
	  }	
	  //bila penjualan
	  //if (substr($rs2['id_trans'],0,3) != 'TPB' )
	  //if (substr($rs2['id_trans'],0,3) != 'BYR' )
	  else if ($rs2['trans']== 'JUAL' )
	  {
	  $kode=$rs2['kode'];
	  $keterangan="JUAL";
	  $piutang_awal=$rs2['faktur'];
	  ...
	  */
	  ...
	  //else if (substr($rs2['id_trans'],0,3) == 'BYR' )
	  //transaksi pembayaran
	  else if ($rs2['trans']== 'JUALBAYAR' )
	  {
	  $kode=$rs2['kode'];
	       if ($rs2['transfer'] > 0) {
		   $keterangan="TF BANK BCA";}
		   else
		   {$keterangan="SETORAN TUNAI BCA";}
	  $piutang_awal="";
	  $ongkos_kuli="";
	  $jumlah_piutang="";
	  $kas="";//dianggap ke bank semua
	  //$bca=$rs2['tunai']+$rs2['transfer']-$rs2['simpan_deposit'];
	  $bca=$rs2['tunai']+$rs2['transfer'];
	  //$total_bayar=$rs2['tunai']+$rs2['transfer']-$rs2['simpan_deposit'];
	  $total_bayar=$rs2['tunai']+$rs2['transfer'];
	  $tmp= -($total_bayar);	  	  
	  }
	  //transaksi penjualan kelebihan (deposit)
	  /*
	  else if ($rs2['trans']== 'SIMPANDEPOSIT' )
	  {
	  $kode=$rs2['kode'];
	  $keterangan="SIMPAN DEPOSIT";
	  $piutang_awal="";
	  $ongkos_kuli="";
	  $jumlah_piutang="";
	  $kas="";//dianggap ke bank semua
	  $bca= -$rs2['simpan_deposit'];
	  $total_bayar= -$rs2['simpan_deposit'];
	  $tmp= -($total_bayar);	  	  
	  }
	  */
	  //transaksi penjualan membayar dengan deposit
	  /*
	  else if ($rs2['trans']== 'JUALBYRDEPOSIT' )
	  {
	  $kode=$rs2['kode'];
	  $keterangan="BAYAR dg DEPOSIT";
	  $piutang_awal="";
	  $ongkos_kuli="";
	  $jumlah_piutang="";
	  $kas="";//dianggap ke bank semua
	  $bca=$rs2['jualdeposit'];
	  $total_bayar=$rs2['jualdeposit'];
	  $tmp= -($total_bayar);	  	  
	  }
	  */
	  //bila pelunasan piutang
	  //else if (substr($rs2['id_trans'],0,3) == 'TPB' ){
	  else if ($rs2['trans'] == 'PIUTANG' ){
