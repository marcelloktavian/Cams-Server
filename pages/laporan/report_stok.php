...
<?php
	if(isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
	 ...
	 //echo"<script>alert('where=$where')</script>";
        ...
        $q = $db->query("SELECT * FROM `stok_barang` p Left Join `barang` j on (p.kode_brg=j.kode_brg) ".$where);
		$count = $q->rowCount();
        ...
        $q = $db->query("SELECT p.kode_brg,j.id_barang,j.id as nomor,j.nm_barang, j.hrg_jual,sum(p.stok) as stok FROM `stok_barang` p Left Join `barang` j on (p.kode_brg=j.kode_brg) ".$where." GROUP BY p.kode_brg ORDER BY `".$sidx."` ".$sord." LIMIT ".$start.", ".$limit);
        ...
        foreach($data1 as $line) {
			...
			if(in_array($_SESSION['user']['access'], $allowEdit))
				if($line['is_start'] == 0) {
					...
					$edit = '<a onclick="window.open(\''.BASE_URL.'pages/laporan/report_stok.php?ids='.$line['id_trans'].'\',\'table_stok\')" href="javascript:;">Nota</a>';
			...
			if(in_array($_SESSION['user']['access'], $allowDelete))
				if($line['is_start'] == 0) {
					$delete = '<a onclick="javascript:link_ajax(\''.BASE_URL.'pages/laporan/report_stok.php?action=delete&id='.$line['id_trans'].'\',\'table_stok\')" href="javascript:;">Delete</a>';
				}
				else {
        	...
        	$responce['rows'][$i]['id']   = $line['kode_brg'];
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'json_sub') {
		...
		$where = "WHERE sb.kode_brg = '".$id."' order by sb.id desc";
        $q = $db->query("SELECT  sb.id_barang,b.nm_barang,DATE_FORMAT(sb.tgl_trans,'%d-%m-%Y') as tgl_trans,sb.stok,sb.id_trans FROM `stok_barang` sb INNER JOIN `barang` b ON (sb.kode_brg=b.kode_brg) ".$where);
	...
	*/
?>
<table id="table_stok"></table>
<div id="pager_table_stok"></div>
<div class="btn_box">
<a href="javascript: void(0)" 
   onclick="window.open('pages/laporan/report_stok.php', 
  'windowname1', 
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#table_stok").jqGrid({
            url:'<?php echo BASE_URL.'pages/laporan/report_stok.php?action=json'; ?>',
            /*postData: {
            ...
            datatype: "json",
            colNames:['Kode','ID','Nama Barang','Stok'],
            colModel:[
            ...
            subGrid : true,
            subGridUrl : '<?php echo BASE_URL.'pages/laporan/report_stok.php?action=json_sub'; ?>',
            subGridModel: [
			            	{ 
        ...
        $("#table_stok").jqGrid('navGrid','#pager_table_stok',{edit:false,add:false,del:false});
    })
</script>