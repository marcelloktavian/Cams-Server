...
<?php
	if(isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
	 ...
	 //echo"<script>alert('where=$where')</script>";
        ...
        $q = $db->query("SELECT * FROM `stok_barang` p Left Join `barang` j on (p.id_barang=j.id_barang) ".$where);
		$count = $q->rowCount();
        ...
        $q = $db->query("SELECT p.id_barang,j.id as nomor,j.nm_barang, j.hrg_jual,sum(p.stok) as stok FROM `stok_barang` p Left Join `barang` j on (p.id_barang=j.id_barang) ".$where." GROUP BY p.id_barang ORDER BY `".$sidx."` ".$sord." LIMIT ".$start.", ".$limit);
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'json_sub') {
		...
		$where = "WHERE sb.id_barang = '".$id."' ";
        $q = $db->query("SELECT  sb.id_barang,b.nm_barang,sb.tgl_trans,sb.stok,sb.id_trans FROM `stok_barang` sb INNER JOIN `barang` b ON (sb.id_barang=b.id_barang) ".$where);
