...
<form id="form2" name="form2" action="" method="post"  onSubmit="return validasi(this)">
    ...
    <tr>
      <td height="123" valign="top"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
          ...
          <tr>
            ...
            <td class="style9" width="19%" ><a href="accreport_kartupiutangrekapxl.php?start=<?php echo"".$tglstart;?>&end=<?php echo"".$tglend;?>&id=<?php echo"".$id_pelanggan;?>">Export ke Excell</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Dari</td>
  ...
  <table width="100%" id="tbd" border="0" align="center" cellpadding="0" cellspacing="0">
	...
	$sql_union="Select b.id,b.id_cust,b.namaperusahaan,b.keterangan,jual.faktur,jual.biaya,jual.tunai,jual.transfer,jual.deposit as jualdeposit,jual.totalfaktur,jual.bayar_faktur
	,jual.piutang_faktur,jual.piutang_tunai,jual.piutang_transfer,jual.piutang_bayar from tblpelanggan b 
	left join 
	(Select id_customer,sum(faktur) as faktur,sum(biaya) as biaya,sum(tunai) as tunai,sum(transfer) as transfer,sum(deposit) as deposit,sum(totalfaktur) as totalfaktur
	...
	from (Select tgl_trans,id_customer,id_trans,kode,faktur,biaya,tunai,transfer,deposit,totalfaktur,(totalfaktur-piutang) as bayar_faktur,piutang as piutang_faktur,'' as piutang_tunai ,'' as piutang_transfer,'' as piutang_bayar from trjual ".$where."
	union all
	...
	{ 
	  ...
	  $bca=$rs2['tunai']+$rs2['transfer']+$rs2['jualdeposit']+$rs2['piutang_tunai'] + $rs2['piutang_transfer'];
	  $total_bayar=$rs2['tunai']+$rs2['transfer']+$rs2['jualdeposit']+$rs2['piutang_tunai'] + $rs2['piutang_transfer'];
	  $saldo_akhir=$rs2['totalfaktur']-($rs2['tunai']+$rs2['transfer']+$rs2['jualdeposit']+$rs2['piutang_tunai'] + $rs2['piutang_transfer']);
