...
<form id="form2" name="form2" action="" method="post"  onSubmit="return validasi(this)">
    ...
    <tr>
      <td height="123" valign="top"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
          ...
          <tr>
			...
			<a href="javascript:window.print()">CETAK</a> REKAP BUKU PENJUALAN BULAN <?php echo DateTime::createFromFormat('d/m/Y', $tglstart)->format('F Y');?></strong></td>
            
          </tr>
          <tr>
            <td width="19%" class="style9"><a href="rekap_buku_penjualanxl.php?start=<?php echo"".$_GET['start'];?>&end=<?php echo"".$tglend;?>&id=<?php echo"".$id_pelanggan;?>">Export ke Excell</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
  ...
  <table width="100%" border="0" align="center" cellpadding="2" cellspacing="2">
    ...
    <tr>
      ...
      <td width="5%" class="style9b"><div align="center">Qty(pcs)</div></td>
...
//	$where = "WHERE TRUE ";
	...
	$sql = "SELECT DISTINCT cal.tanggal, DATE_FORMAT(cal.tanggal,'%d') AS tgl_trans,SUM( a.totalqty) AS totalqty,'-' AS harga_satuan,SUM(a.faktur)AS faktur, SUM(a.totalfaktur) AS totalfaktur
,ROUND((SUM(a.faktur)/1.11),0) AS DPP,ROUND((SUM(a.faktur)/1.11)*0.11,0) AS PPN,
...
trjual a ON DATE(a.tgl_trans) = DATE(cal.tanggal)  LEFT JOIN tblpelanggan b ON b.id = a.id_customer  $where
...
//var_dump($sql);
    ...
    $sq2 = mysql_query($sql);
	//$sq2 ="select b.namaperusahaan,a.id_trans,a.kode, date_format(a.tgl_trans,'%d-%m-%Y') as tgl_trans, a.totalqty,a.faktur, a.totalfaktur,b.id_cust,b.keterangan,round((a.faktur/a.totalqty),0) as harga_satuan,round((a.faktur/1.11),0) as DPP,round((a.faktur/1.11)*0.11,0) as PPN,
  ...
  ?>
    <tr>
      ...
      <td class="style9"><div align="center">
        <?=number_format($rs2['totalqty']*30,0);?>
      </div></td>
      <td class="style9"><div align="right">
	  <?=$rs2['harga_satuan'];?>
      </div></td>
      <td class="style9"><div align="right">
	  <?=number_format($rs2['faktur']);?>
      </div></td>
      <td class="style9"><div align="center">&nbsp;</div></td>
      <td class="style9"><div align="center">&nbsp;</div></td>
      <td class="style9"><div align="right">
	  <?=number_format($rs2['DPP']);?>
      </div></td>
	  <td class="style9"><div align="right">
	  <?=number_format($rs2['PPN']);?>
      </div></td>
	  <td class="style9"><div align="right">
	  <?=number_format($rs2['faktur']);//DPP+PPN?>
      </div></td>
	  <td class="style9"><div align="right">
	  <?=number_format($rs2['biaya']);?>
      </div></td>
	  <td class="style9"><div align="right">
          <?=number_format($rs2['totalfaktur']);?>
      </div></td>
	  <td class="style9"><div align="right">
          <?=number_format($rs2['bayar']);?>
      </div></td>
  ...
  ?>
	...
	<td class="style9"><div align="right">
          <?=number_format($grand_faktur);//dpp+ppn?>
    </div>
	</td>
	...
	<td class="style9"><div align="right">
          <?=number_format($grand_totalfaktur);?>
    </div>
	</td>
	
	<td class="style9"><div align="right">
          <?=number_format($grand_bayar);?>
    </div>
	</td>
	<td class="style9">&nbsp;</td>
