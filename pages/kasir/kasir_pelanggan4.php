...
<?php
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'get_pelanggan') {
		...
		$p = $db->query("SELECT * FROM tblpelanggan p where type=2 ORDER BY p.namaperusahaan");
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'process') {
		...
		if(isset($_POST['id'])) {
			$stmt = $db->prepare("UPDATE tblpelanggan SET namaperusahaan=?,alamat=?,keterangan=?,telp1=?,telp2=?,fax=?,contactperson=?,HP=?,email=?,user=?, lastmodified = NOW() WHERE id=?");
			$stmt->execute(array($_POST['nama'],$_POST['alamat'],strtoupper($_POST['keterangan']),$_POST['telp1'],$_POST['telp2'],$_POST['fax'],$_POST['contactperson'],$_POST['hp'],$_POST['email'],$_SESSION['user']['username'], $_POST['id']));
		  ...
		  else 
		  {
			...
			$stmt = $db->prepare("INSERT INTO tblpelanggan(`namaperusahaan`,`alamat`,`keterangan`,`telp1`,`telp2`,`fax`,`contactperson`,`HP`,`email`,`user`,`lastmodified`,`type`) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?)");
			//di kasir diinput pelanggan type=2
			if($stmt->execute(array(strtoupper($_POST['nama']),$_POST['alamat'],strtoupper($_POST['keterangan']),$_POST['telp1'],$_POST['telp2'],$_POST['fax'],$_POST['contactperson'],$_POST['hp'],$_POST['email'],$_SESSION['user']['username'],2))) {
