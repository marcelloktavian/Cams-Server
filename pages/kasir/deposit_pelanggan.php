...
//searching _filter---------------------------------------------------------
       ...
       $where = "WHERE b.deleted=0 AND b.type=2 ";
       } else {
       $operations = array(
        'eq' => "= '%s'",            // Equal
        'ne' => "<> '%s'",           // Not equal
	  ...
	  $where = sprintf(" where b.deleted=0 AND b.type=2 AND p.%s ".$operations[$_REQUEST["searchOper"]], $_REQUEST["searchField"], $value);
	
     }
//--------------end of searching--------------------		

        
		$sql_deposit = "Select b.id, b.id_cust,b.namaperusahaan,b.keterangan,b.alamat,b.telp1,b.hp,b.contactperson,jual.deposit as trdeposit from tblpelanggan b left join (select id_customer,sum(ifnull(deposit,0)) as deposit from trdeposit td group by id_customer ) as jual on b.id = jual.id_customer ".$where;
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'json_sub') {
        ...
        $sql_sub="Select d.id_trans,d.kode,d.tgl_trans,d.id_customer,p.namaperusahaan,d.totalfaktur,d.tunai,d.transfer,d.deposit,d.keterangan,d.catatan from trdeposit d left join tblpelanggan p on d.id_customer=p.id".$where;
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#table_depositpelanggan").jqGrid({
            url:'<?php echo BASE_URL.'pages/kasir/deposit_pelanggan.php?action=json'; ?>',
            /*postData: {
            ...
            rownumbers: true,
            sortorder: "desc",
            caption:"Data Deposit Pelanggan Toko",
            ondblClickRow: function(rowid) {
                alert(rowid);
            },
			subGrid : true,
            subGridUrl : '<?php echo BASE_URL.'pages/kasir/deposit_pelanggan.php?action=json_sub'; ?>',
            subGridModel: [
			            	{ 
