...
<script type="text/javascript" src="myseparator.js"></script>
...
function max_id(){
$q=mysql_fetch_array(mysql_query("select max(id) as nomor from trjual_id"))
$temp="INV-".$q['nomor'];
return $temp;
}	
...
{   
	...
	//for(var i=0;i<tunai.length;i++){
	//var tunai_murni=parseInt(tunai.replace(".", ""));
	//tunai.split('.');
	//alert("tunai="+tunai)
	//}
	<?php   ?>
	var tunai_murni=parseInt(tunai);
    ...
    sisa= total-(tunai_murni+transfer_murni);
...
function cetak(){
    ...
    //var namaValid    = /^[a-zA-Z]+(([\'\,\.\- ][a-zA-Z ])?[a-zA-Z]*)*$/;
        ...
        var piutang      = parseInt(form2.piutang.value);
