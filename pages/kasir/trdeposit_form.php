<div class="ui-widget ui-form">
    <div class="ui-widget-header ui-corner-top padding5">
        <?php
			...
			echo $action .' INPUT DATA DEPOSIT PELANGGAN';
        ?>
    </div>
    <div class="ui-widget-content ui-corner-bottom">
        <form id="trdeposit_form" method="post" action="<?php echo BASE_URL ?>pages/kasir/trdeposit.php?action=process" class="ui-helper-clearfix">
        	<?php
	        	if(strtolower($_GET['action']) == 'edit') {
					echo '<input value="'.$_GET['id'].'" type="hidden" class="required" id="id" name="id">';
					$select = $db->prepare('SELECT * FROM trdeposit WHERE id_trans = :id');
				...
				//max id for kode pelanggan--------------------
					$select2 = $db->prepare("Select max(substring(kode,3,3)+1) as kode_id from trdeposit where kode like 'TD%'");
					$select2->execute();
            ...
            <div class="ui-corner-all form-control">
                <select class="required" name="id_cust" id="id_cust">
                	...
                	<?php
                		$query = $db->query("SELECT * FROM tblpelanggan where deleted=0 and type=2 ORDER BY namaperusahaan ASC");
