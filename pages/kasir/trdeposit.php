...
<?php
	if(isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
        ...
        $sql = "SELECT p.*,p.keterangan as info,j.* FROM `trdeposit` p Left Join `tblpelanggan` j on (p.id_customer=j.id) ".$where;
        $q = $db->query($sql);
		$count = $q->rowCount();
        ...
        foreach($data1 as $line) {
			...
			if(in_array($_SESSION['user']['access'], $allowEdit))
				$edit = '<a onclick="javascript:popup_form(\''.BASE_URL.'pages/kasir/trdeposit.php?action=edit&id='.$line['id_trans'].'\',\'table_trdeposit\')" href="javascript:;">Edit</a>';
			else
				$edit = '<a onclick="javascript:custom_alert(\'Not Allowed\')" href="javascript:;">Edit</a>';
        	
			if(in_array($_SESSION['user']['access'], $allowDelete))
				$delete = '<a onclick="javascript:link_ajax(\''.BASE_URL.'pages/kasir/trdeposit.php?action=delete&id='.$line['id_trans'].'\',\'table_trdeposit\')" href="javascript:;">Delete</a>';
			else
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'add') {
		include 'trdeposit_form.php';exit();
		exit;
	}
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'delete') {
		$stmt = $db->prepare("DELETE FROM trdeposit WHERE id_trans=?");
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'process') {
		if(isset($_POST['id'])) {
			$stmt = $db->prepare("UPDATE trdeposit SET id_trans=?,kode=?,id_customer=?,tgl_trans=?,totalfaktur=?,tunai=?,transfer=?,deposit=?,keterangan=?,catatan=?,user=?, lastmodified = NOW() WHERE id_trans=?");
			...
			$stmt = $db->prepare("INSERT INTO trdeposit(`id_trans`,`kode`,`id_customer`,`tgl_trans`,`totalfaktur`,`tunai`,`transfer`,`deposit`,`keterangan`,`catatan`,`user`,`lastmodified`) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
...
<?php
	...
	if(in_array($_SESSION['user']['access'], $allow)) {
		echo '<button type="button" onclick="javascript:popup_form(\''.BASE_URL.'pages/kasir/trdeposit.php?action=add\',\'table_trdeposit\')" class="btn">Tambah</button>';
	}
...
<script type="text/javascript">
	...
	function gridReloadJual(){
		...
		var v_url ='<?php echo BASE_URL?>pages/kasir/trdeposit.php?action=json&start_trdeposit='+start_trdeposit+'&end_trdeposit='+end_trdeposit ;
    ...
    $(document).ready(function(){
        ...
        $("#table_trdeposit").jqGrid({
            url:'<?php echo BASE_URL.'pages/kasir/trdeposit.php?action=json'; ?>',
