...
//include("koneksi/koneksi.php");
	...
	if ($byr_deposit > 0)
	{
	$sql_deposit="insert into trdeposit(id_trans,kode,tgl_trans,id_customer,totalfaktur,tunai,deposit,catatan) values('".$id_pkb."','".$kode."',NOW(),'".$id_customer."','-".$byr_deposit."','-".$byr_deposit."','-".$byr_deposit."','JUAL SIMPAN DEPOSIT')";
	mysql_query($sql_deposit)or die (mysql_error());
	}
	else if ($simpan_deposit > 0)
	{
	$sql_deposit="insert into trdeposit(id_trans,kode,tgl_trans,id_customer,totalfaktur,tunai,deposit,catatan) values('".$id_pkb."','".$kode."',NOW(),'".$id_customer."','".$simpan_deposit."','".$simpan_deposit."','".$simpan_deposit."','BYR JUAL DEPOSIT')";
	mysql_query($sql_deposit)or die (mysql_error());
	}
