...
<?php
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'process') {
		...
		if(isset($_POST['id'])) {
		  ...
		  if($check_rows > 0) {
				//$r['stat'] = 1;
				$r['message'] = 'Ada nama yang sama';
		  }
		  else 
		  {
			$stmt = $db->prepare("INSERT INTO tblpelanggan(`namaperusahaan`,`alamat`,`keterangan`,`telp1`,`telp2`,`fax`,`contactperson`,`HP`,`email`,`user`,`lastmodified`) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
			
			if($stmt->execute(array(strtoupper($_POST['nama']),$_POST['alamat'],strtoupper($_POST['keterangan']),$_POST['telp1'],$_POST['telp2'],$_POST['fax'],$_POST['contactperson'],$_POST['hp'],$_POST['email'],$_SESSION['user']['username']))) {
