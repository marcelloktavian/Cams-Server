...
//--------------end of searching--------------------		
     //   $where = "WHERE deleted=0 ";
        ...
        foreach($data1 as $line) {
			...
			if(in_array($_SESSION['user']['access'], $allowEdit))
				$edit = '<a onclick="javascript:popup_form(\''.BASE_URL.'pages/armada.php?action=edit&id='.$line['id_ar'].'\',\'table_armada\')" href="javascript:;">Edit</a>';
			else
				$edit = '<a onclick="javascript:custom_alert(\'Not Allowed\')" href="javascript:;">Edit</a>';
        	
			if(in_array($_SESSION['user']['access'], $allowDelete))
				$delete = '<a onclick="javascript:link_ajax(\''.BASE_URL.'pages/armada.php?action=delete&id='.$line['id_ar'].'\',\'table_armada\')" href="javascript:;">Delete</a>';
			else
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'process') {
		if(isset($_POST['id_ar'])) {
			...
			$stmt = $db->prepare("INSERT INTO  tblarmada(`Nama`,`NoPOL`,`Keterangan`,`user`,`lastmodified`) VALUES(?, ?, ?, ?,NOW())");
			if($stmt->execute(array($_POST['nama'], $_POST['nopol'], $_POST['keterangan'],$_SESSION['user']['username']))) {
				$r['stat'] = 1;
				$r['message'] = 'Success';
			}
			else {
...
<?php
	...
	if(in_array($_SESSION['user']['access'], $allow)) {
		echo '<button type="button" onclick="javascript:popup_form(\''.BASE_URL.'pages/armada.php?action=add\',\'table_armada\')" class="btn">Tambah</button>';
	}
?>
</div>
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#table_armada").jqGrid({
            url:'<?php echo BASE_URL.'pages/armada.php?action=json'; ?>',
            /*postData: {
