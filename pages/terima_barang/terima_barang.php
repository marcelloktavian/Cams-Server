...
//---------------------------------------------------------------------------------
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'json_sub') {
        ...
        $q = $db->query("SELECT pd.id_detail,pd.id_barang,b.id_barang,b.nm_barang,pd.id_trans,pd.qty,pd.harga,(pd.qty * pd.harga) as subtotal FROM `trbeli_detail` pd INNER JOIN `barang` b ON (pd.id_barang=b.id_barang) ".$where);
	...
	//dipake buat unpost kembali gudang, sehingga stok_gudang dan stok_barang harus didelete dan dikembalikan statusnya ke 0
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'delete') {
		$stmt = $db->prepare("DELETE FROM stok_gudang WHERE id_trans=?");
		$stmt->execute(array($_GET['id']));
		$stmt = $db->prepare("DELETE FROM stok_barang WHERE id_trans=?");
		$stmt->execute(array($_GET['id']));
		$stmt = $db->prepare("UPDATE trbeli set faktur=0 WHERE id_trans=?");
		$stmt->execute(array($_GET['id']));
		$affected_rows = $stmt->rowCount();
		if($affected_rows > 0) {
			$r['stat'] = 1;
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#table_terimabrg").jqGrid({
            ...
            subGridModel: [
			            	{ 
			            		name : ['No','ID_Barang','Barang','Harga','Qty(pcs)','Subtotal'], 
			            		width : [40,75,300,50,50,50],
