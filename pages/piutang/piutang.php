...
<?php
	if(isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
     ...
     // }
		...
		$sql = "SELECT * FROM `trpiutang` p Left Join `tblpelanggan` j on (p.id_customer=j.id) ".$where;
        $q = $db->query($sql);
		$count = $q->rowCount();
    //    var_dump($sql);
        ...
        foreach($data1 as $line) {
			...
			$edit = '<a onclick="window.open(\''.BASE_URL.'pages/piutang/piutang_detail_edit.php?ids='.$line['id_trans'].'\',\'table_piutang\')" href="javascript:;">Nota</a>';
			}
			else
...
<script type="text/javascript">
	...
	function gridReloadPiutang(){
		...
		var v_url ='<?php echo BASE_URL?>pages/piutang/piutang.php?action=json&startdate_piutang='+startdate_piutang+'&enddate_piutang='+enddate_piutang ;
    ...
    $(document).ready(function(){
        ...
        $("#table_piutang").jqGrid({
            url:'<?php echo BASE_URL.'pages/piutang/piutang.php?action=json'; ?>',
            /*postData: {
