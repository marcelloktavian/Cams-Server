...
<?php
	if(isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
	 ...
	 //echo"<script>alert('where=$where')</script>";
        ...
        //$where = "WHERE TRUE AND p.deleted = 0 ";
        //$where = "WHERE TRUE ";
        $q = $db->query("SELECT * FROM `trpiutang` p Left Join `tblpelanggan` j on (p.id_customer=j.id) ".$where);
		$count = $q->rowCount();
        ...
        $q = $db->query("SELECT p.*, j.namaperusahaan as customer 
							 FROM `trpiutang` p Left Join `tblpelanggan` j on (p.id_customer=j.id) 
							 ".$where."
							 ORDER BY `".$sidx."` ".$sord."
							 LIMIT ".$start.", ".$limit);
        ...
        foreach($data1 as $line) {
			...
			if(in_array($_SESSION['user']['access'], $allowEdit))
				if($line['is_start'] == 0) {
					...
					$edit = '<a onclick="window.open(\''.BASE_URL.'pages/piutang/piutang_detail_edit.php?ids='.$line['id_trans'].'\',\'table_piutang\')" href="javascript:;">Lihat</a>';
			...
			if(in_array($_SESSION['user']['access'], $allowDelete))
				if($line['is_start'] == 0) {
					$delete = '<a onclick="javascript:link_ajax(\''.BASE_URL.'pages/piutang/piutang.php?action=delete&id='.$line['id_trans'].'\',\'table_piutang\')" href="javascript:;">Delete</a>';
				}
				else {
...
<a href="javascript: void(0)" 
   onclick="window.open('pages/piutang/piutang_detail.php')">
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#table_piutang").jqGrid({
            ...
            datatype: "json",
            colNames:['ID','Customer','Tanggal Transaksi','Qty','Total Bayar','Tunai','Transfer','View'/*,'Delete'*/],
            colModel:[
            ...
            rownumbers: true,
            sortorder: "desc",
            caption:"Data Piutang Penjualan",
        ...
        $("#table_piutang").jqGrid('navGrid','#pager_table_piutang',{edit:false,add:false,del:false});
    })
</script>