...
$id_pkb = getnewnotrxwait2();
	...
	LEFT JOIN tblpelanggan b ON b.id =a.id_customer WHERE a.id_trans= '".$_GET['ids']."'")or die (mysql_error());
		$rs = mysql_fetch_array($sql);
$id_pelanggan = $rs['id_customer'];
...
$info="<td style='text-align:center;font-size: 15px;background-color:#FFE4B5;height:40px;border:1px dotted #f30; border-radius:4px; -moz-border-radius:4px;' />PELANGGAN INI MEMILIKI DEPOSIT</td>";
}
else
{
$info="<td></td>";
...
<script type="text/javascript">
	...
	document.form2.faktur_jual.value='<?=$rs['totalfaktur'];?>';
...
<?php 
	$sql1 = mysql_query("select a.id_trans,a.kode,a.tgl_trans,a.faktur,a.totalfaktur,a.piutang,a.pelunasan,(a.piutang-a.pelunasan) as sisapiutang,b.namaperusahaan from trjual a left join tblpelanggan b on (a.id_customer=b.id) where a.id_trans = '".$_GET['ids']."'");
    $i=1;
		...
		?>
			...
			document.getElementById('Invoice['+<?=$i;?>+']').value = '<?=$rs1['totalfaktur'];?>';
