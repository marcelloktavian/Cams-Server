<head>
	<title>EDIT JURNAL MANUAL</title>
...
$sqlmaster = $db->prepare("SELECT * FROM jurnal WHERE id=? AND deleted=0");
$sqlmaster->execute(array($_GET["id"]));
while($result = $sqlmaster->fetch(PDO::FETCH_ASSOC)){
    $tgl = $result['tgl'];
    $keterangan = $result['keterangan'];
}
?>
	<form id='form2' name='form2' action='' method='post'>
        <input type="hidden" name="idparent" id="idparent" value="<?=$_GET['id']?>">
		<table width='100%'>
			<tr>

				<td class='fontjudul'>EDIT JURNAL MANUAL</td>
				<!-- subtotal -->
		...
		<table width='100%' cellspacing='0' cellpadding='0'>
			<tr>
				...
				<td><input type='date' class='inputform' name='tanggal' id='tanggal' value='<?=$tgl?>' />
				</td>
			</tr>
			<tr height='5'>
				<td colspan='6'></td>
		...
		<table>
			<tbody>
				<tr>
					...
					<td colspan="6" align="left"><textarea name="txtbrg" id="txtbrg" cols="100" rows="3"><?=$keterangan?></textarea>
					</td>
				</tr>
			</tbody>
		</table>
	...
	<script type="text/javascript">
		...
		function delRow1(id) {
            ...
            // if (row>2) {
                    ...
                    document.getElementById('delete'+id+'').value = document.getElementById('iddetail'+id+'').value;
		...
		function cetak() {
			...
			if (pesan != '') {
				...
				if (answer) {
					hitungrow();
					// hitung() ;
					document.form2.action = "simpanJurnalmanual.php?action=edit";
