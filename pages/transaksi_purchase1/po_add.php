  ...
  <form id="po_add" name="po_add" action="" method="post">
    <table width="100%">
      <tr>
        ...
        <td class="fontjudul">TOTAL ORDER<input type="text" class="" name="total_order" id="total_order" style="text-align: right; font-size: 30px; background-color: white; height: 40px; border: 1px dotted #F30; border-radius: 4px; -moz-border-radius: 4px;" readonly /></td>
        <td class="fontjudul">PPN<input type="text" class="" name="ppn" id="ppn" style="text-align: right; font-size: 30px; background-color: white; height: 40px; border: 1px dotted #F30; border-radius: 4px; -moz-border-radius: 4px;" readonly /></td>
        <td class="fontjudul">TOTAL ORDER DPP<input type="text" class="" name="total_dpp" id="total_dpp" style="text-align: right; font-size: 30px; background-color: white; height: 40px; border: 1px dotted #F30; border-radius: 4px; -moz-border-radius: 4px;" readonly /></td>
      </tr>
    </table>
    ...
    <table width="100%" cellpadding="0" cellspacing="0">
      ...
      <tr>
        ...
        <td colspan="100%"><textarea type="text" class="inputForm" name="catatan" id="catatan" placeholder="Catatan" style="height: 40px;"></textarea></td>
      </tr>
      <tr height="1">
...
<script type="text/javascript">
  ...
  function hitungorder(){
    ...
    $('#total_order').val(parseFloat(totalorder)); hitungppn(); hitungdpp();
  }

  function hitungppn(){
    $('#ppn').val((parseFloat($('#total_order').val()))*11/100);
  }

  function hitungdpp(){
    $('#total_dpp').val((parseFloat($('#total_order').val())+parseFloat($('#ppn').val())))
  }
  ...
  function generateProdukJasa(index){
    var idx = document.createElement("input");
    idx.type="text"; idx.name="produk_jasa"+index; idx.id="produk_jasa"+index; idx.size="70"; idx.readOnly="readonly"; return idx;
  }

  function generateQuantity(index){
    var idx = document.createElement("input");
    idx.type="text"; idx.name="qty"+index; idx.id="qty"+index; idx.value="0"; return idx;
  }

  function generateDPPUnit(index){
    var idx = document.createElement("input");
    idx.type="text"; idx.name="dpp"+index; idx.id="dpp"+index; idx.readOnly="readonly"; return idx;
  }

  function generateSubTotal(index){
    var idx = document.createElement("input");
    idx.type="text"; idx.name="sub_total"+index; idx.id="sub_total"+index; idx.readOnly="readonly"; return idx;
  }
  ...
  function addNewRow1(){
    ...
    td3.appendChild(generateDPPUnit(baris1));
