...
$allow_add = is_show_menu(ADD_POLICY, Address, $group_acess);
$allow_edit = is_show_menu(EDIT_POLICY, Address, $group_acess);
$allow_delete = is_show_menu(DELETE_POLICY, Address, $group_acess);
...
//--------------end of searching--------------------		
     //   $where = "WHERE deleted=0 ";
        ...
        foreach($data1 as $line) {
            ...
            if ($statusToko == 'Tutup') {
                ...
                if($allow_edit)
                    $edit = '<a onclick="javascript:popup_form(\''.BASE_URL.'pages/master_online/alamat.php?action=edit&id='.$line['id'].'\',\'table_alamat\')" href="javascript:;">Edit</a>';
                else
                    $edit = '<a onclick="javascript:custom_alert(\'Not Allowed\')" href="javascript:;">Edit</a>';
            
                if($allow_delete)
                    $delete = '<a onclick="javascript:link_ajax(\''.BASE_URL.'pages/master_online/alamat.php?action=delete&id='.$line['id'].'\',\'table_alamat\')" href="javascript:;">Delete</a>';
                else
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'delete') {
		$stmt = $db->prepare("UPDATE mst_address SET deleted=? WHERE id=?");
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'process') {
		if(isset($_POST['id'])) {
			$stmt = $db->prepare("UPDATE mst_address SET kecamatan=?,kabupaten=?,provinsi=?,jumlah_desa=?,kode_pos=?,user=?, lastmodified = NOW() WHERE id=?");
			$stmt->execute(array($_POST['kecamatan'],$_POST['kabupaten'],$_POST['provinsi'],$_POST['jumlah_desa'],$_POST['kode_pos'], $_SESSION['user']['username'], $_POST['id']));
			//var_dump($stmt);die;
			$affected_rows = $stmt->rowCount();
			if($affected_rows > 0) {
				$r['stat'] = 1;
			...
			$stmt = $db->prepare("INSERT INTO  mst_address(`kecamatan`,`kabupaten`,`provinsi`,`jumlah_desa`,`kode_pos`,`user`,`lastmodified`) VALUES(?, ?, ?, ?, ?, ?,NOW())");
			if($stmt->execute(array($_POST['kecamatan'],$_POST['kabupaten'], $_POST['provinsi'], $_POST['jumlah_desa'], $_POST['kode_pos'],$_SESSION['user']['username']))) {
				$r['stat'] = 1;
				$r['message'] = 'Success';
			}
			else {
...
// $allow = array(1,2,3);
	...
	if($allow_add) {
		echo '<button type="button" onclick="javascript:popup_form(\''.BASE_URL.'pages/master_online/alamat.php?action=add\',\'table_alamat\')" class="btn">Tambah</button>';
	}
	}
?>
</div>
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#table_alamat").jqGrid({
            url:'<?php echo BASE_URL.'pages/master_online/alamat.php?action=json'; ?>',
            /*postData: {
