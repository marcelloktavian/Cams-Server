...
//--------------end of searching--------------------		
     //   $where = "WHERE deleted=0 ";
        $sql_products ="SELECT a.*,c.nama as kategori, br.nama as nmbrand FROM `mst_products` a left join mst_category c on a.id_category=c.id left join mst_brand br on br.id=a.brand ";
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'process') {
		if(isset($_POST['id'])) {
			...
			$stmt = $db->prepare("UPDATE mst_products SET kode=?,oln_product_id=?,nama=?,harga=?,size=?,brand=?,type=?,id_category=?,aktif=?,user=?, limit_stok=?, lastmodified = NOW() WHERE id=?");
			$stmt->execute(array(strtoupper($_POST['kode']),$_POST['oln_product_id'],strtoupper($_POST['nama']),$_POST['harga'],$_POST['size'],$_POST['brand'],strtoupper($_POST['type']),$_POST['id_category'],strtoupper($_POST['aktif']), $_SESSION['user']['username'],$_POST['limit'], $_POST['id']));


            $stmt = $db->prepare("UPDATE inventory_balance SET kode=?,oln_product_id=?,nama=?,harga=?,size=?,brand=?,type=?,id_category=?,user=?,limit_stok=?, lastmodified = NOW() WHERE id=?");
            $stmt->execute(array(strtoupper($_POST['kode']),$_POST['oln_product_id'],strtoupper($_POST['nama']),$_POST['harga'],$_POST['size'],$_POST['brand'],strtoupper($_POST['type']),$_POST['id_category'], $_SESSION['user']['username'],$_POST['limit'], $_POST['id']));
			...
			$stmt = $db->prepare("INSERT INTO  mst_products(`kode`,`oln_product_id`,`nama`,`harga`,`size`,`brand`,`type`,`id_category`,`aktif`,`user`,`lastmodified`,`limit_stok`) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?,NOW(),?)");
			if($stmt->execute(array(strtoupper($_POST['kode']),$_POST['oln_product_id'],strtoupper($_POST['nama']), $_POST['harga'],$_POST['size'],$_POST['brand'],strtoupper($_POST['type']),$_POST['id_category'],strtoupper($_POST['aktif']),$_SESSION['user']['username'],$_POST['limit']))) {

                //insert inventory_balance
                $stmt = $db->prepare("INSERT INTO  inventory_balance(`oln_product_id`,`kode`,`nama`,`harga`,`brand`,`type`,`size`,`id_category`,`user`,`lastmodified`,`limit_stok`) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?,NOW(),?)");

                $stmt->execute(array($_POST['oln_product_id'],strtoupper($_POST['kode']),strtoupper($_POST['nama']), $_POST['harga'],$_POST['brand'],strtoupper($_POST['type']),$_POST['size'],$_POST['id_category'],$_SESSION['user']['username'],$_POST['limit']));
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#table_products").jqGrid({
            ...
            datatype: "json",
            colNames:['ID','ID OLN','Name','Price (inc PPN)','Size','Brand','Type','Limit','Category','Active','Edit','Delete'],
            colModel:[
                ...
                {name:'nmbrand',index:'nmbrand', align:'center', width:30, searchoptions: {sopt:['cn']}},             
