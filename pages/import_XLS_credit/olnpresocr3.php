...
$allow_delete = is_show_menu(DELETE_POLICY, PreSALESCredit, $group_acess);
	...
	if(isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
       ...
       $operations = array(
		...
		$sql = "SELECT SUM(p.total) as grandtotal,sum(p.deposit) as deposit,p.oln_order_id,p.oln_customer,p.oln_tgl,p.oln_penerima,p.oln_address,sum(p.jumlah_beli) as qty, sum(p.subtotal) as total,p.oln_note,p.oln_expnote FROM `olnpreso_cr` p ".$where. " GROUP by p.oln_order_id";
        //var_dump($sql);die;
		$q = $db->query($sql);
		$count = $q->rowCount();
        //var_dump($sql);
