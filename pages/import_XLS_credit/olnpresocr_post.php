<?php
	...
	$grandtotal = $_POST['totalhidden'];
		...
		// var_dump($idolnso);die;
         // die;

		//posting tanpa save keterangan info web
		$queryMaster = " INSERT INTO olnso (id_trans,ref_kode,tgl_trans,nama,alamat,telp,tunai,transfer,deposit,faktur,total,piutang,totalqty,id_dropshipper,
        id_address,id_expedition,exp_code,exp_fee,discount,aktif,state,lastmodified,exp_note,note) SELECT '$idolnso','$ref',b.oln_tgl,b.oln_penerima, CONCAT('$alamat',' (kec ',b.oln_kecamatan,', ',b.oln_kotakab,', ',b.oln_provinsi,')') AS oln_address,
        b.oln_telp,'$tunai','$transfer','".$deposit."','".($totalakhir-$exp_fee)."','".($totalakhir)."','".$piutang."','".$totalqty."',b.id_dropshipper,'0','".$id_expedisi."','".$exp_code."','$exp_fee','$disc_dropshipper','Y','0',NOW(),'".$exp_note."','".$keterangan_bawah."' FROM olnpreso_cr b WHERE (b.oln_order_id = '$ref') limit 0,1";
        // var_dump($queryMaster);die;
        ...
        {  
            ...
            $Harga = $_POST['Harga'.$i];
            $disc = $_POST['Disc'.$i];
            $sub = $_POST['SUBTOTAL'.$i];

        // detail Insert to olnsodetail
            // if ($i==1) {
            //    array_push($brg, $idbrg);
            // }
            ...
            // }
            // if ($validasi=="T") {
                if ($iddelete==null && $idbrg!=null) {
            $idolnauto = mysql_insert_id();
               $queryDetil = " INSERT INTO olnsodetail (id_trans,id_product,namabrg,size,jumlah_beli,disc,id_oln_auto,harga_act,subtotal_act,jumlah_beli_act) VALUES ('$idolnso','$idbrg','$namabrg','$size','$Qty','$disc','$idolnauto','$Harga','$sub','$Qty')";
        // var_dump($queryDetil);
 ...
 // die;
       ...
       $sql_validasi="SELECT dt.id_trans,DATE_FORMAT(m.lastmodified,'%d/%m/%Y') AS tglposted, m.ref_kode AS id_web,m.exp_fee AS ongkir,d.nama AS dropshipper,SUM(dt.jumlah_beli) AS qty_detail,m.totalqty,SUM(dt.subtotal * (1-m.discount)) AS total_detail,m.total,m.faktur ,m.nama AS pembeli,e.nama AS expedition,m.state,m.discount AS discdp,m.discount_faktur AS disc_faktur FROM olnsodetail dt  INNER JOIN olnso m ON dt.id_trans = m.id_trans  LEFT JOIN mst_dropshipper d ON m.id_dropshipper = d.id LEFT JOIN mst_expedition e ON m.id_expedition = e.id WHERE ((m.deleted=0) AND (m.state='0') AND m.id_trans='$idolnso') GROUP BY dt.id_trans ";
        ...
        if ($qtydetail <> $qtymaster) {
            echo "Qty Tidak Sama. Silahkan cinput ulang!<br>";
            $validasi='Y';
        } 

        // if ($totmaster <> $totdetail) {
        //     echo "Total Tidak Sama. Silahkan input ulang!<br>";
        //     $validasi='Y';
        // } 
        
        if ($validasi == 'Y') {
            //set deleted  = 1 olnso
            $queryState=" UPDATE olnso SET deleted=1 WHERE id_trans='$idolnso' ";
            $hasilState = mysql_query($queryState) or die (mysql_error());
           die;
        }

        // jangan lupa state rubah state menjadi 1 jika sudah berhasil
        $queryState=" UPDATE olnpreso_cr SET state='1' WHERE oln_order_id = '$ref' ";
        $hasilState = mysql_query($queryState) or die (mysql_error());
	...
	$sql_deposit="DELETE FROM olnpreso_cr where oln_order_id=".$ref;
    //  var_dump($sql_deposit);die;
	mysql_query($sql_deposit)or die (mysql_error());
	
        //update harga
     $sqldetail = "UPDATE olnso AS so JOIN (SELECT SUM(CEIL(dt.subtotal * (1-m.discount))) AS total FROM olnsodetail dt INNER JOIN olnso m ON dt.id_trans = m.id_trans WHERE ((m.deleted=0) AND (m.state='0') AND m.id_trans='".$idolnso."') GROUP BY dt.id_trans) AS tot SET so.total=CEIL(tot.total), so.faktur=CEIL(tot.total), so.piutang=CEIL(tot.total) WHERE so.id_trans='".$idolnso."';";   
