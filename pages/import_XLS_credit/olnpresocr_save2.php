    ...
    // end validasi
        ...
        for ($i=1; $i<$row; $i++)
        {
            ...
            if ($iddelete==null && $iddet=='' && $idbrg != '') {
                ...
                $query = "INSERT INTO olnpreso_cr(id_product, namabrg, size, jumlah_beli, harga_satuan, tax, subtotal, total, oln_order_id ,oln_note ,oln_customer, oln_customer_telp, oln_expnote, tunai, transfer, id_expedition, exp_fee, oln_penerima, oln_address, oln_telp, oln_customerid, id_dropshipper,oln_productid) VALUES('$idbrg', '$namabrg', '$size', '$Qty', '$Harga', '$tax', '$sub', '$grandtotal', '$ref','$keterangan',  '$dropshipper', '$telp', '$exp_code', '$tunai',  '$transfer','$id_expedisi', '$exp_fee','$nama','$alamat', '$telp', '$id_onlineDropshipper', '$id_dropshipper','$idolnbrg')";
                $hasil = mysql_query($query) or die (mysql_error());
            }else if($iddelete==null && $iddet!='' && $idbrg != ''){
                //id detail ada = update
                $query = " UPDATE olnpreso_cr SET id_product='$idbrg',namabrg='$namabrg',size='$size',jumlah_beli='$Qty',harga_satuan='$Harga',tax='$tax',subtotal='$sub',total='$grandtotal',oln_order_id='$ref',oln_note='$keterangan',oln_customer='$dropshipper',oln_customer_telp='$telp',oln_expnote='$exp_code',tunai='$tunai',transfer='$transfer',id_expedition='$id_expedisi',exp_fee='$exp_fee', oln_penerima='$nama', oln_address='$alamat', oln_telp='$telp', oln_customerid='$id_onlineDropshipper', id_dropshipper='$id_dropshipper',oln_productid='$idolnbrg' WHERE id='$iddet' ";
                $hasil = mysql_query($query) or die (mysql_error());
            }
                // var_dump($query);
                $sub = $Qty * ($Harga + $tax);
