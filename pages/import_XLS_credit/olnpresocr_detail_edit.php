...
$id=$_GET['ids'];
$sql_init="SELECT p.deposit,sum(p.harga_satuan) as harga_satuan,sum(p.tax) as tax,p.total as grandtotal,p.subtotal ,p.oln_customer_telp,p.oln_order_id,p.oln_tgl,p.oln_customerid,p.oln_customer,p.id_dropshipper,p.oln_tgl,p.oln_penerima,p.oln_address,p.oln_provinsi,p.oln_postalcode,p.oln_kotakab,p.oln_kecamatan,p.oln_telp,p.oln_expnote,sum(p.jumlah_beli) as qty, sum(p.subtotal) as total,p.deposit as grand_deposit,p.oln_note,p.oln_expnote,d.disc as discdp, p.id_expedition, e.nama as expedition, p.exp_fee as exp_fee,oln_noteexp,oln_keterangan FROM `olnpreso_cr` p LEFT JOIN mst_dropshipper d on p.id_dropshipper=d.id LEFT JOIN mst_expedition e ON e.id = p.id_expedition WHERE p.oln_order_id ='".$id."' GROUP by p.oln_order_id ";
...
<body>
	<?php
	$totalnya = "Rp " . number_format($displayTotal,2,',','.');
	echo"<form id='form2' name='form2' action='' method='post'>
	<table width='100%'>
	<tr>
	<td  class='fontjudul'>PRE SALES CREDIT</td>
	<td class='fontjudul'> TOTAL <input type='text' class='' name='total' id='total' value='$totalnya' style='text-align:right;font-size: 30px;background-color:white;height:40px;border:1px dotted #f30; border-radius:4px; -moz-border-radius:4px;' />
	...
	<input type='hidden' name='totalparent' id='totalparent'>
	</td>
	</tr>
	</table>
	<hr />
	...
	<input type='hidden' value='$id_customer_oln' name='id_onlineDropshipper' id='id_onlineDropshipper'/>
	</td>
	</tr>
	...
	"<input type='hidden' value='$id_dropshipper' name='id_dropshipper' id='id_dropshipper'/>
	</td>
	</tr>
	<tr height='1'>
	<td colspan='4'></td>
	...
	<td colspan='3'><textarea name='keterangan' id='keterangan' cols='200' rows='5'  >$info</textarea></td>
	...
	<td><input type='text' class='inputform' name='expedition' id='expedition' placeholder='Autosuggest Ekspedisi' value='$expedition'/>
	<input type='hidden' name='id_expedition' id='id_expedition' value='$id_expedition'/></td>
	...
	<td><input type='text' class='inputform' name='exp_fee' id='exp_fee' placeholder='Biaya Ekspedisi' value='$exp_fee' onkeyup='hitungpiutang();'/></td>
...
function get_products(a){  
	...
	$.ajax({
		...
		success: function(data) {
			...
			var harga_awal = data.harga;
			var discdrop = document.getElementById("disc_dropshipper").value;
			var harga_disc = parseInt(harga_awal) - Math.round(parseFloat(harga_awal)*parseFloat(discdrop));
			// var harga_pd  = data.harga;
			var harga_akhir = Math.round(harga_disc/1.11);
			$("#Harga"+a+"").val(harga_akhir);
...
function hitungtotal(){
	...
	document.getElementById("piutang").value = totaltf;	
}
...
if (saldo_deposit <= 0)
{
	...
	//totalhidden dipake buat validasi saja
	document.getElementById("totalhidden").value = total;	
	document.getElementById("totalparent").value = totalhid;
	document.getElementById("tunai").value = 0;	
	document.getElementById("total").value = total.toLocaleString('IND', {style: 'currency', currency: 'IDR'});
	//totalqty
	document.getElementById("totalqty").value = totalqty;
...
function cetak(){
	...
	var totalparent = document.getElementById("totalparent").value;
 ...
 //        }
    ...
    { 
    	...
    	{	
    		...
    		document.form2.action="olnpresocr_save.php?action=save&jum="+baris1;
...
function cetak2(){
    ...
    { 
    	...
    	{	
    		...
    		document.form2.action="olnpresocr_post.php?jum="+baris1;
    		document.form2.submit();
    	}
    	else
    	{}
...
$sql_edit = "Select a.* from olnpreso_cr a where a.oln_order_id = '".$_GET['ids']."'";
//var_dump($sql_edit);
...
while($rs1=mysql_fetch_array($sql1)){
	...
	totalnya = totalnya + <?=$rs1['subtotal'];?>;
	<?php
	$i++;
} ?>
document.getElementById('totalparent').value = totalnya;
</script>
