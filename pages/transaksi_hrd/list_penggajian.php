...
<table width='100%'>
<tr>
    <td class='fontjudul'>PENGGAJIAN DETAIL</td>
			<td class='fontjudul'> TOTAL PENDAPATAN <input type='text' class='' name='totalpendapatan' id='totalpendapatan' value='Rp. 0' style='text-align:right;font-size: 25px;background-color:white;height:35px;border:1px dotted #f30; border-radius:4px; -moz-border-radius:4px;' />
			<input type='hidden' name='totalpendapatanhidden' id='totalpendapatanhidden' value='0'>
			<input type='hidden' name='totalpendapatanSebelumnya' id='totalpendapatanSebelumnya' value='0'>
				</td>

		<td class='fontjudul'> TOTAL POTONGAN <input type='text' class='' name='totalpotongan' id='totalpotongan' value='Rp. 0' style='text-align:right;font-size: 25px;background-color:white;height:35px;border:1px dotted #f30; border-radius:4px; -moz-border-radius:4px;' />
			<input type='hidden' name='totalpotonganhidden' id='totalpotonganhidden' value='0'>
			<input type='hidden' name='totalpotonganSebelumnya' id='totalpotonganSebelumnya' value='0'>
				</td>

	<td class='fontjudul'> TOTAL <input type='text' class='' name='total' id='total' value='Rp. 0' style='text-align:right;font-size: 25px;background-color:white;height:35px;border:1px dotted #f30; border-radius:4px; -moz-border-radius:4px;' />
...
<thead>
    <tr>
    	...
    	<td align='center' width='10%' class='fonttext'>Metode Perhitungan</td>
    	<td align='center' width='10%' class='fonttext'>Mempengaruhi Total</td>
...
td3.appendChild(generateMenambah(baris1));
...
function generateMenambah(index) {
var idx = document.createElement("input");
idx.type = "text";
idx.name = "Menambah"+index+"";
idx.id = "Menambah"+index+"";
idx.size = "7";
idx.readOnly = "readonly";
idx.style="text-align:center;";
return idx;
}
...
//id_product
var idx = document.createElement("input");Menambah
idx.type = "text";
...
function hitungtotal(){
	...
	var Subtotal=document.getElementById("Subtotal"+i+"");
	 ...
	 {   
	    if(document.getElementById("Subtotal"+i+"").value == "") {
			...
			if(document.getElementById("Menambah"+i+"").value == "Ya"){
				if(document.getElementById("Tipe"+i+"").value=='Pendapatan'){
	...
	total = parseFloat(totalpendapatan) - parseFloat(totalpotongan);
...
function cetak(){
		...
		{	
			...
			window.opener.document.getElementById("TotalPendapatan"+b+"").value = parseFloat($('#totalpendapatanhidden').val());
			window.opener.document.getElementById("TotalPendapatanHidden"+b+"").value = parseFloat($('#totalpendapatanhidden').val());

			window.opener.document.getElementById("TotalPotongan"+b+"").value = parseFloat($('#totalpotonganhidden').val());
			window.opener.document.getElementById("TotalPotonganHidden"+b+"").value = parseFloat($('#totalpotonganhidden').val());
			...
			// hitungrow();
			document.form2.action="penggajian_save.php?action=list&row="+baris1+"&penggajian="+<?=$_GET['penggajian']?>;
...
WHERE `status` <> '' AND a.id_penggajian='$penggajian' AND a.id_karyawan='$id' ORDER BY b.type ASC,b.kode_penpot ASC";
...
while($rsdet=mysql_fetch_array($sqdet)){
	?>
		...
		document.getElementById("Menambah"+<?=$i?>).value = "<?php if($rsdet['total_pendapatan']=='0'){echo "Tidak";}else{echo "Ya";}  ?>";
	...
	<?php
	if($rsdet['metode_pethitungan']=='Manual Input'){$subtotal=$rsdet['subtotal_variabel'];}else{$subtotal= $rsdet['subtotal'];};
	if($rsdet['type']=="pendapatan"){
	...
	var element3 = document.getElementById("Menambah"+<?=$i;?>+"");
	var element4 = document.getElementById("Subtotal"+<?=$i;?>+"");
	var element5 = document.getElementById("Keterangan"+<?=$i;?>+"");
...
?>	
document.getElementById("totalpendapatanSebelumnya").value = "<?php echo $totalpendapatan ?>";
document.getElementById("totalpotonganSebelumnya").value = "<?php echo $totalpotongan ?>";
