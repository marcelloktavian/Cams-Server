...
$allow_edit = is_show_menu(EDIT_POLICY, transpendapatan, $group_acess);
	...
	if(isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
        ...
        foreach($data1 as $line) {
            ...
            $responce['rows'][$i]['cell'] = array(
                ...
                number_format($line['total_pendapatan_variabel'],2),               
				$edit,
            );
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'json_sub') {
		...
		IFNULL((SELECT SUM(subtotal_variabel) FROM hrd_penggajiandet b LEFT JOIN hrd_pendapatan_potongan c ON c.id_penpot=b.id_penpot WHERE `id_penggajian`='$id' AND b.`id_karyawan`=a.`id_karyawan` AND `status`='pendapatan' AND total_pendapatan=1),0) AS pendapatan, 
		a.status FROM hrd_penggajiandet a
...
<script type="text/javascript">
	...
	$( "#start_pendapatan" ).datepicker( 'setDate', '<?php echo date('01/m/Y')?>' );
	$( "#end_pendapatan" ).datepicker( 'setDate', '<?php echo date('t/m/Y')?>' );
