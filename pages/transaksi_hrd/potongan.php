...
$allow_edit = is_show_menu(EDIT_POLICY, transpotongan, $group_acess);
	...
	if(isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
        ...
        $startdate = isset($_GET['start_potongan'])?$_GET['start_potongan']:date('Y-m-d');
		$enddate = isset($_GET['end_potongan'])?$_GET['end_potongan']:date('Y-m-d'); 
		...
		IFNULL((select SUM(subtotal_variabel) from hrd_penggajiandet b LEFT JOIN hrd_pendapatan_potongan c ON c.id_penpot=b.id_penpot where c.`total_pendapatan`=1 AND status='potongan' AND id_penggajian = a.penggajian_id),0) as subtotalmanual
        ...
        foreach($data1 as $line) {
			...
			if($allow_edit)
				$edit = '<a onclick="window.open(\''.BASE_URL.'pages/transaksi_hrd/potongan_detail.php?id='.$line['penggajian_id'].'\',\'table_potongan\')" href="javascript:;">Edit</a>';
			else
            ...
            $responce['rows'][$i]['cell'] = array(
                ...
                number_format($line['subtotalmanual'],2),               
				$edit,
            );

			$grandtotal += $line['subtotalmanual'];
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'json_sub') {
		...
		$q = $db->query("SELECT a.`id_penggajiandet`, a.`id_karyawan`, b.`nama_karyawan`, d.`nama_dept`, b.no_telp, a.wa,
		IFNULL((SELECT SUM(subtotal_variabel) FROM hrd_penggajiandet b LEFT JOIN hrd_pendapatan_potongan c ON c.id_penpot=b.id_penpot WHERE `id_penggajian`='$id' AND b.`id_karyawan`=a.`id_karyawan` AND `status`='potongan' AND total_pendapatan=1),0) AS potongan, a.status FROM hrd_penggajiandet a
    ...
    <div class="ui-widget-content ui-corner-bottom">
        <form id="report_project_form" method="" action="" class="ui-helper-clearfix">
            ...
            <div class="ui-corner-all form-control">
				...
				<tr>
				<td>
				 <input value="" type="text" class="required datepicker"   id="start_potongan" name="start_potongan">
				</td>
				<td> s.d.  
				<input value="" type="text" class="required datepicker"  id="end_potongan" name="end_potongan">
				</td>
				</tr>
				</table>
            </div>
            ...
            <div class="ui-corner-all form-control">
            	<button onclick="gridReloadPotongan()" class="btn" type="button">Cari</button>
            </div>
       	</form>
   	</div>
</div>

<table id="table_potongan"></table>
<div id="pager_table_potongan"></div>

<script type="text/javascript">
    $('#start_potongan').datepicker({
		dateFormat: "dd/mm/yy"
	});
	$('#end_potongan').datepicker({
		dateFormat: "dd/mm/yy"
	});
	$( "#start_potongan" ).datepicker( 'setDate', '<?php echo date('01/m/Y')?>' );
	$( "#end_potongan" ).datepicker( 'setDate', '<?php echo date('t/m/Y')?>' );
	
	
	function gridReloadPotongan(){
		var start_potongan = $("#start_potongan").val();
		var end_potongan = $("#end_potongan").val();
		var v_url ='<?php echo BASE_URL?>pages/transaksi_hrd/potongan.php?action=json&start_potongan='+start_potongan+'&end_potongan='+end_potongan ;
		jQuery("#table_potongan").setGridParam({url:v_url,page:1}).trigger("reloadGrid");
	}

    $(document).ready(function(){

        $("#table_potongan").jqGrid({
            url:'<?php echo BASE_URL.'pages/transaksi_hrd/potongan.php?action=json'; ?>',
            
            datatype: "json",
            colNames:['ID','Nama Periode','Tipe Karyawan','Tanggal Penggajian','Jumlah Hari Kerja', 'Total Potongan Variabel', 'Edit'],
            colModel:[
			...
			subGrid : true,
			subGridUrl : '<?php echo BASE_URL.'pages/transaksi_hrd/potongan.php?action=json_sub'; ?>',
            subGridModel: [
			            	{ 
			            		name : ['No','Nama Karyawan','Departemen','Total Potongan Variabel'], 
			            		width : [40,250,100,100],
			            		align : ['center','left','center','right'],
			            	} 
			            ],
        });
        $("#table_potongan").jqGrid('navGrid','#pager_table_potongan',{search:false,edit:false,add:false,del:false});
    })
</script>