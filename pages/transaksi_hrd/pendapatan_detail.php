...
echo"<form id='form2' name='form2' action='' method='post'>
    <table width='100%'>
  	<tr>
    	<td  class='fontjudul'>EDIT PENDAPATAN</td>
		<td class='fontjudul'> TOTAL PENDAPATAN VARIABEL
...
<thead>
	...
	<td align='center' width='7%' class='fonttext'>Total Pendapatan Variabel</td>
...
function popDetail(a){
	...
	window.open('list_pendapatan.php?baris='+a+'&karyawan='+karyawan+'&penggajian='+<?=$id?>,'',params);
};
...
function cetak(){
		...
		{	
		hitungrow() ;
		document.form2.action="pendapatan_save.php?action=edit&id=<?=$_GET['id']?>&jum="+baris1;
...
<?php
		...
		IFNULL((SELECT SUM(subtotal_variabel) FROM hrd_penggajiandet b LEFT JOIN hrd_pendapatan_potongan c ON c.id_penpot=b.id_penpot WHERE `id_penggajian`='$id' AND b.`id_karyawan`=a.`id_karyawan` AND `status`='pendapatan' AND total_pendapatan=1),0) AS subtotal_variabel, 
		...
		while($rs1 = mysql_fetch_array($sqdet)){
			?>
				...
				document.getElementById('TotalHidden'+<?=$i;?>+'').value = '<?=$rs1['subtotal'];?>';
			<?php
			$i++;
		}
