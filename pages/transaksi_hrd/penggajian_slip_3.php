...
error_reporting(0);
    ...
    $karyawannya=explode(',',$karyawan);
    for ($a=0; $a < count($karyawannya); $a++) { 
        $sql_title = "SELECT a.id_karyawan, a.nama_karyawan, c.nama_dept FROM hrd_karyawan a LEFT JOIN hrd_jabatan b ON b.id_jabatan=a.id_jabatan LEFT JOIN hrd_departemen c ON c.id_dept=b.id_dept WHERE a.deleted=0 AND a.id_karyawan = '".$karyawannya[$a]."' ";
  	...
  	<td width="46%" style='vertical-align:top'>
  		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
  			<tr>
			   ...
			   WHERE `status` = 'pendapatan' AND a.id_penggajian='$id' AND a.id_karyawan='".$karyawannya[$a]."' AND b.total_pendapatan=1 ";
  	...
  	<td width="46%" style='vertical-align:top'>
  		<table width="100%" border="0	" align="center" cellpadding="0" cellspacing="0">
  			<tr>
		   	...
		   	$sql_detail = "SELECT * FROM hrd_penggajiandet a 
			   LEFT JOIN hrd_pendapatan_potongan b ON b.id_penpot=a.id_penpot
			   WHERE `status` = 'potongan' AND a.id_penggajian='$id' AND a.id_karyawan='".$karyawannya[$a]."' AND b.total_pendapatan=1";
		   	$sq2 = mysql_query($sql_detail);
		   	while($rs2=mysql_fetch_array($sq2))
			{ 	
				$totalb = 0;
