...
<?php
    ...
    IFNULL(ROUND((SELECT SUM(subtotal)+SUM(subtotal_variabel) FROM hrd_penggajiandet b LEFT JOIN hrd_pendapatan_potongan c ON c.id_penpot=b.id_penpot WHERE `id_penggajian`='$id' AND b.`id_karyawan`=a.`id_karyawan` AND `status`='pendapatan' AND total_pendapatan=1),2),0) AS pendapatan, 
    IFNULL(ROUND((SELECT SUM(subtotal)+SUM(subtotal_variabel) FROM hrd_penggajiandet b LEFT JOIN hrd_pendapatan_potongan c ON c.id_penpot=b.id_penpot WHERE `id_penggajian`='$id' AND b.`id_karyawan`=a.`id_karyawan` AND `status`='potongan' AND total_pendapatan=1),2),0) AS potongan, a.status FROM hrd_penggajiandet a
		...
		while($rs1 = mysql_fetch_array($sqdet)){
			?>
                ...
                if('<?=$posting?>'=='T'){
                    ...
                    document.getElementById('CheckId'+<?=$i;?>+'').disabled = true;
