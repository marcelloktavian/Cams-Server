...
echo"<form id='form2' name='form2' action='' method='post'>
    <table width='100%'>
  	<tr>
    	<td  class='fontjudul'>EDIT DEDUCTION</td>
		<td class='fontjudul'> TOTAL DEDUCTION
...
<thead>
	...
	<td align='center' width='7%' class='fonttext'>Value BPJS Kesehatan</td>
	<td align='center' width='7%' class='fonttext'>Value BPJS TK</td>
	<td align='center' width='7%' class='fonttext'>Simpanan Koperasi</td>
	 <td align='center' width='7%' class='fonttext'>Pinjaman Koperasi</td>
	<td align='center' width='7%' class='fonttext'>PPH 21</td>
	<td align='center' width='7%' class='fonttext'>PPH 21 THR</td>
	<td align='center' width='7%' class='fonttext'>Potongan Lain Lain</td>
	<td align='center' width='7%' class='fonttext'>Total Deduction</td>
    </tr>
";
...
while($rs1=mysql_fetch_array($sql1)){
    ?>
        <tr id="t1<?=$i?>">
            ...
            <td class='fonttext' align='right'><input type='hidden' name="Kesehatan<?=$i?>" id="Kesehatan<?=$i?>" value='<?=$rs1['up_b_kesehatan']?>'><?=number_format($rs1['up_b_kesehatan'],0)?></td>
            <td class='fonttext' align='right'><input type='hidden' name="TK<?=$i?>" id="TK<?=$i?>" value='<?=$rs1['up_b_tk']?>'><?=number_format($rs1['up_b_tk'],0)?></td>

            <td class='fonttext'><input type="text" name="Simpanan<?=$i?>" id="Simpanan<?=$i?>" size="15" style='text-align:right' value="<?=$rs1['simpanan_kop']?>" onkeyup='hitungjml(<?=$i?>)'></td>
            <td class='fonttext'><input type="text" name="Pinjaman<?=$i?>" id="Pinjaman<?=$i?>" size="15" style='text-align:right' value="<?=$rs1['pinjaman_kop']?>" onkeyup='hitungjml(<?=$i?>)'></td>
            <td class='fonttext'><input type="text" name="PPH21<?=$i?>" id="PPH21<?=$i?>" size="15" style='text-align:right' value="<?=$rs1['pph21']?>" onkeyup='hitungjml(<?=$i?>)'></td>
            <td class='fonttext'><input type="text" name="PPH21THR<?=$i?>" id="PPH21THR<?=$i?>" size="15" style='text-align:right' value="<?=$rs1['pph21_thr']?>" onkeyup='hitungjml(<?=$i?>)'></td>
            <td class='fonttext'><input type="text" name="Potongan<?=$i?>" id="Potongan<?=$i?>" size="15" style='text-align:right' value="<?=$rs1['potongan']?>" onkeyup='hitungjml(<?=$i?>)'></td>
            <td class='fonttext'><input type="text" name="Total<?=$i?>" id="Total<?=$i?>" size="15" style='text-align:right' value="<?=$rs1['total_deduction']?>" readonly></td>
...
{
	if(document.getElementById("Simpanan"+a+"").value == ""){
    	var simpanan = 0;
	}
	else
	{
	    var simpanan = document.getElementById("Simpanan"+a+"").value;
	}
	
	if(document.getElementById("Pinjaman"+a+"").value == ""){
    	var pinjaman = 0;
	}
	else
	{
		var pinjaman = document.getElementById("Pinjaman"+a+"").value;
	}

    if(document.getElementById("PPH21"+a+"").value == ""){
    	var pph21 = 0;
	}
	else
	{
		var pph21 = document.getElementById("PPH21"+a+"").value;
	}

    if(document.getElementById("PPH21THR"+a+"").value == ""){
    	var pph21thr = 0;
	}
	else
	{
		var pph21thr = document.getElementById("PPH21THR"+a+"").value;
	}

    if(document.getElementById("Potongan"+a+"").value == ""){
    	var potongan = 0;
	}
	else
	{
		var potongan = document.getElementById("Potongan"+a+"").value;
	}

    var kesehatan = document.getElementById("Kesehatan"+a+"").value;
    var tk = document.getElementById("TK"+a+"").value;

	var total=0;
	
    total = parseInt(kesehatan) + parseInt(tk) + parseInt(simpanan) + parseInt(pinjaman) + parseInt(pph21) + parseInt(pph21thr) + parseInt(potongan);
...
function cetak(){
		...
		{	
		hitungrow() ;
		document.form2.action="deduction_save.php?id_trans=<?=$_GET['id']?>&jum="+baris1;
