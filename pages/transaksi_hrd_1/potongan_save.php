...
if($action == 'list'){
    ...
    $query = "UPDATE `hrd_penggajian` SET total_potongan=(SELECT SUM(subtotal) FROM hrd_penggajiandet where status='potongan' and id_penggajian='$id'),total_potongan_variabel=(SELECT SUM(subtotal_variabel) FROM hrd_penggajiandet where status='potongan' and id_penggajian='$id')  WHERE penggajian_id='$id' ";
    $hasil = mysql_query($query) or die ("error".mysql_error());
}
?>
<script>
