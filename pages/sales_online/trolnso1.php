...
<?php
	if(isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
		...
		$startdate = isset($_GET['startdate_jual'])?$_GET['startdate_jual']:date('Y-m-d');
		$enddate = isset($_GET['enddate_jual'])?$_GET['enddate_jual']:date('Y-m-d'); 
     ...
     // }
		...
		$sql = "SELECT p.*,j.nama as dropshipper FROM `olnso` p Left Join `mst_dropshipper` j on (p.id_dropshipper=j.id) Left Join `mst_expedition` e on (p.id_expedition=e.id) ".$where;
        $q = $db->query($sql);
		$count = $q->rowCount();
    //    var_dump($sql);
        ...
        foreach($data1 as $line) {
			...
			$edit = '<a onclick="window.open(\''.BASE_URL.'pages/sales_online/trolnso_nota.php?id_trans='.$line['id_trans'].'\',\'table_jual\')" href="javascript:;">Receipt</a>';
			}
			else
			...
			if(in_array($_SESSION['user']['access'], $allowDelete))
			    ...
			    $select = '<input type="checkbox" name="select"  value='.$line['id_trans'].'>';
...
<script type="text/javascript">
	...
	function gridReloadJual(){
		...
		var v_url ='<?php echo BASE_URL?>pages/sales_online/trolnso.php?action=json&startdate_jual='+startdate_jual+'&enddate_jual='+enddate_jual ;
    ...
    $(document).ready(function(){
        ...
        $("#table_jual").jqGrid({
            ...
            colNames:['ID','ID_web','Dropshipper','Date','Receiver','Address','Qty','Print','Cancel','Select'],
            colModel:[
