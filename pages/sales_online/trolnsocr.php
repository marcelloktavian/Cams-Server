...
$allow_delete = is_show_menu(DELETE_POLICY, OnlineCredit, $group_acess);
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'posting') {
		...
		$exp_id='';
		$exp_fee='';
		$q = mysql_fetch_array( mysql_query("SELECT olnso.id_trans,olnso.total,olnso.id_dropshipper,mst_dropshipper.nama, id_expedition, exp_fee FROM olnso LEFT JOIN mst_dropshipper ON mst_dropshipper.id=olnso.id_dropshipper WHERE id_trans='".$_GET['id']."' LIMIT 1"));
		$idtrans=$q['id_trans'];
		$total=$q['total'];
...
<script type="text/javascript">
	$(document).ready(function () {
		...
		$("#table_jualcr").jqGrid('navGrid', '#pager_table_jualcr', {
			edit: false,
			add: false,
			del: false,
			search: true
