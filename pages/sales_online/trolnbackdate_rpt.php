...
error_reporting(0);
	...
	$sql_title ="SELECT (SELECT p.id_trans FROM tbl_backdatecancel p Left Join `mst_dropshipper` j on (p.id_dropshipper=j.id) Left Join `mst_expedition` e on p.id_expedition=e.id WHERE $where_title ORDER BY p.id_trans ASC LIMIT 1) AS first_order,
    (SELECT p.id_trans FROM tbl_backdatecancel p Left Join `mst_dropshipper` j on (p.id_dropshipper=j.id) Left Join `mst_expedition` e on p.id_expedition=e.id WHERE  $where_title ORDER BY p.id_trans DESC LIMIT 1) AS last_order ,
    (SELECT COUNT(p.id_trans) FROM tbl_backdatecancel p Left Join `mst_dropshipper` j on (p.id_dropshipper=j.id) Left Join `mst_expedition` e on p.id_expedition=e.id WHERE  $where_title) AS jumlah_order";
...
<form id="form2" name="form2" action="" method="post"  onSubmit="return validasi(this)">
  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
    <tr>
      <td height="123" valign="top"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
          ...
          <tr>
            <td width="100%" class="style99" colspan="6"><strong>
			BACKDATE CANCEL </strong></td>
          ...
          <tr>
            ...
            <?php echo"".$tglstart;?>
            &nbsp;-&nbsp;<?php echo"".$tglend;?> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Jumlah Order:&nbsp;<?php echo"".$rs_title['jumlah_order'];?></td>           
		  </tr>
  ...
  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
      ...
      <th width="5%" class="style_title"><div align="center">No.web <br><font style="font-size: 9px;">(Backdate)</font></div></td>
	...
	$sql_detail = "SELECT dt.id_trans,DATE_FORMAT(m.tgl_ship,'%d/%m/%Y') AS tglposted,DATE_FORMAT(m.lastmodified,'%d/%m/%Y') AS tglbackdate, m.ref_kode AS id_web,m.exp_fee as ongkir,d.nama AS dropshipper,dt.namabrg,dt.jumlah_beli,dt.size,dt.harga_satuan,dt.subtotal,m.nama AS pembeli,e.nama AS expedition,m.state,m.discount as discdp,m.discount_faktur as disc_faktur,m.total,m.faktur FROM tbl_backdatecanceldetail dt
    INNER JOIN tbl_backdatecancel m ON dt.id_trans = m.id_trans
  ...
  ?>
    ...
    <?
	  ...
	  if ($kode!=$rs2['id_trans'])
	  {
		...
		echo"<td class='style_detail'><div align='center'>".$rs2['id_web']."</br>(".$rs2['tglbackdate'].")</div></td>";
