...
if(isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
  ...
  if((!isset($_GET['startdate_olnsocr']) && !isset($_GET['enddate_olnsocr']))||($_GET['startdate_olnsocr'] == '' && $_GET['enddate_olnsocr'] == '')){
    $where = " WHERE j.deleted=0 AND p.tgl_trans>='".date("Y-m-d")."' AND p.tgl_trans<='".date("Y-m-d")."'".$filter_value;
  }else if($_GET['startdate_olnsocr'] != '' && $_GET['enddate_olnsocr'] == ''){
    $where = " WHERE j.deleted=0 AND p.tgl_trans>='".$_GET['startdate_olnsocr']."'".$filter_value;
  }
  else if($_GET['startdate_olnsocr'] == '' && $_GET['enddate_olnsocr'] != ''){
    $where = " WHERE j.deleted=0 AND p.tgl_trans<='".$_GET['enddate_olnsocr']."'".$filter_value;
  }
  else{
    $where = " WHERE j.deleted=0 AND p.tgl_trans>='".$_GET['startdate_olnsocr']."' AND p.tgl_trans<='".$_GET['enddate_olnsocr']."'".$filter_value;
  }
  // -------------------- end of searching _filter >>

  if ($_REQUEST["_search"] == "false") {
    //all transaction kecuali yang batal
    $where_state = " AND p.state='0' AND (p.totalqty <> 0) AND (p.piutang> 0)";
  } else {
    $operations = array(
      'eq' => "= '%s'",            // Equal
      'ne' => "<> '%s'",           // Not equal
  ...
  $where_state = sprintf(" AND (p.totalqty <> 0) AND (p.state ='0') AND (p.piutang> 0) and (p.deleted=0) AND %s ".$operations[$_REQUEST["searchOper"]], $_REQUEST["searchField"], $value);
  }
  $sql = "SELECT p.*,j.nama as dropshipper, j.type as dsType, e.nama as expedition, SUM(p.total) as sumTotal FROM `olnso` p Left Join `mst_dropshipper` j on (p.id_dropshipper=j.id) Left Join `mst_expedition` e on (p.id_expedition=e.id) ".$where.$where_state." GROUP BY `id_dropshipper`";
  $q = $db->query($sql);
  ...
  foreach($data1 as $line) {
    if ($statusToko == 'Tutup') {
      ...
      if($allow_delete)
        ...
        $pay = '<a href="javascript:;">Pay</a>';

        $detail = '<a href="javascript:;">Detail</a>';
    }
  ...
  <div class="ui-widget-content ui-conrer-bottom">
    <form id="filter_ap" method="" action="" class="ui-helper-clearfix">
      ...
      <div class="ui-corner-all form-control">
        <table>
          <tr>
            <td><input type="text" class="required datepicker" id="startdate_olnsocr" name="startdate_olnsocr" readonly></td>
            <td> s.d <input type="text" class="required datepicker" id="enddate_olnsocr" name="enddate_olnsocr" readonly></td>
            <td> Filter <input type="text" id="filtervalue_olnsocr" name="filtervalue_olnsocr" />(ID OLN, Dropshipper, Receiver)</td>
          </tr>
        </table>
      </div>
...
<script type="text/javascript">
  $('#startdate_olnsocr').datepicker({dateFormat: "dd-mm-yy"});
	$('#enddate_olnsocr').datepicker({dateFormat: "dd-mm-yy"});

	$("#startdate_olnsocr").datepicker('setDate', '<?php echo date('d-m-Y')?>');
	$("#enddate_olnsocr").datepicker('setDate', '<?php echo date('d-m-Y')?>');

  function gridReloadTR(){
    var startdate     = ($("#startdate_olnsocr").val()).split("-");
    var enddate       = ($("#enddate_olnsocr").val()).split("-");

    var startdate   = startdate[2]+"-"+startdate[1]+"-"+startdate[0];
    var enddate     = enddate[2]+"-"+enddate[1]+"-"+enddate[0];

    var filter      = $("#filtervalue_olnsocr").val();

		var v_url       = '<?php echo BASE_URL?>pages/sales_online/trolnsocr.php?action=json&startdate_olnsocr='+startdate+'&enddate_olnsocr='+enddate+'&filter='+filter;
  ...
  $(document).ready(function(){
    $("#table_jualcr").jqGrid({
      ...
      datatype : "json",
      colNames: ['Dropshipper', 'Type', 'Qty', 'Total', 'Pay', 'Detail', 'Posting', 'Cancel'],
      colModel: [
				{name: 'j.nama', index: 'j.nama', searchoptions: {sopt: ['cn']}},
        {name: 'type', index: 'type', align: 'right', width: 10, searchoptions: {sopt: ['cn']}},
				{name: 'totalqty', index: 'totalqty', align: 'right', width: 20, searchoptions: {sopt: ['cn']}},
        {name: 'sumtotal', index: 'sumtotal', align: 'right', width: 50, searchoptions: {sopt: ['cn']}},
