...
// $allow_post = is_show_menu(POST_POLICY, OnlineBackDate, $group_acess);
// $allow_delete = is_show_menu(DELETE_POLICY, OnlineBackDate, $group_acess);
	...
	if(isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
		...
		$startdate = isset($_GET['startdate_backdate'])?$_GET['startdate_backdate']:date('Y-m-d');
		$enddate = isset($_GET['enddate_backdate'])?$_GET['enddate_backdate']:date('Y-m-d'); 
        $filter=$_GET['filter'];
        ...
        foreach($data1 as $line) {
			...
			// $invoice = '<a onclick="window.open(\''.BASE_URL.'pages/sales_online/trolnso_invoice.php?id_trans='.$line['id_trans'].'\',\'table_backdate\')" href="javascript:;">Invoice</a>';
			// }
			// else
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'pass') {
		//tgl beda
		include 'trolnbackdate_pass_form.php';exit();
		exit;
	}
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'process_pass') {
		...
		if($affected_rows > 0) {
			...
			$stmt = $db->prepare("INSERT INTO tbl_backdatecancel SELECT * FROM olnso WHERE id_trans=?;");
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'json_sub') {
        ...
        $q = $db->query("SELECT pd.* FROM `tbl_backdatecanceldetail` pd ".$where);
    ...
    <div class="ui-widget-content ui-corner-bottom">
        <form id="report_project_form" method="" action="" class="ui-helper-clearfix">
            ...
            <div class="ui-corner-all form-control">
				...
				<tr>
				<td>
				 <input value="" type="text" class="required datepicker"   id="startdate_backdate" name="startdate_backdate">
				</td>
				<td> s.d.  
				<input value="" type="text" class="required datepicker"  id="enddate_backdate" name="enddate_backdate">
				</td>
				<td> Filter
				 <input value="" type="text" id="filterbackdate" name="filterbackdate">(Dropshipper,Receiver,Expedition,Exp_Code,ID OLN,ID WEB)
				</td>
				</tr>
				</table>
            </div>
            ...
            <div class="ui-corner-all form-control">
            	<button onclick="gridReloadbackdate()" class="btn" type="button">Cari</button>
				<button onclick="javascript:window_open('<?php echo BASE_URL ?>pages/sales_online/trolnbackdate_rpt.php?action=preview&start='+$('#startdate_backdate').val()+'&end='+$('#enddate_backdate').val()+'&filter='+$('#filterbackdate').val())" class="btn" type="button">Print</button>
            </div>
       	</form>
   	</div>
</div>
...
<!-- <button id="btn-barcodebackdate"  class="btn btn-success">Print Selected BARCODE</button> -->
<!-- <button id="btn-1by1backdate"  class="btn btn-success">Print Selected 1 By 1</button> -->
</div>
 
<table id="table_backdate"></table>
<div id="pager_table_backdate"></div>
...
<script type="text/javascript">
	...
	$('#startdate_backdate').datepicker({
		dateFormat: "dd/mm/yy"
	});
	$('#enddate_backdate').datepicker({
		dateFormat: "dd/mm/yy"
	});
	$( "#startdate_backdate" ).datepicker( 'setDate', '<?php echo date('d/m/Y')?>' );
	$( "#enddate_backdate" ).datepicker( 'setDate', '<?php echo date('d/m/Y')?>' );
	
	
	function gridReloadbackdate(){
		var startdate_backdate = $("#startdate_backdate").val();
		var enddate_backdate = $("#enddate_backdate").val();
		var filter_backdate = $("#filterbackdate").val();
		var v_url ='<?php echo BASE_URL?>pages/sales_online/trolnbackdate.php?action=json&startdate_backdate='+startdate_backdate+'&enddate_backdate='+enddate_backdate+'&filter='+filter_backdate;
		jQuery("#table_backdate").setGridParam({url:v_url,page:1}).trigger("reloadGrid");
	}
	
	$("#btn-xlsdo").on('click',function(){
		var ids = getSelectedRows();
		if (ids!=='')
				window.open('<?php echo BASE_URL?>pages/sales_online/trolnbackdate_xls.php?ids='+ids,'_blank');
	});
	...
	$("#btn-barcodebackdate").on('click',function(){
		var ids = getSelectedRows();
		if (ids!=='')
				window.open('<?php echo BASE_URL?>pages/sales_online/trolnbackdate_3nota_bd.php?ids='+ids,'_blank');
	});
	
	$("#btn-1by1backdate").on('click',function(){
		var ids = getSelectedRows();
		if (ids!=='')
				window.open('<?php echo BASE_URL?>pages/sales_online/trolnbackdate_4_1by1.php?ids='+ids,'_blank');
	});
	 function getSelectedRows() {
            var grid = $("#table_backdate");
    ...
    $(document).ready(function(){
		...
		//alert('kdie');
        $("#table_backdate").jqGrid({
            url:'<?php echo BASE_URL.'pages/sales_online/trolnbackdate.php?action=json'; ?>',
            /*postData: {
        ...
        $("#table_backdate").jqGrid('navGrid','#pager_table_backdate',{edit:false,add:false,del:false,search:false});
