...
$allow_delete = is_show_menu(DELETE_POLICY, OnlineDelivery, $group_acess);
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'process_pass') {
		...
		if($affected_rows > 0) {
			...
			// insert jurnal
			$stmt = $db->prepare("INSERT INTO jurnal (SELECT NULL,'$masterNo',NOW(),REPLACE(keterangan,'Penjualan','CANCELLED'),total_debet,total_kredit,0,'$id_user',NOW() FROM jurnal WHERE jurnal.keterangan LIKE '%".$_POST['id']."%') ");
			$stmt->execute();
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'delete') {
		...
		// insert jurnal
		$stmt = $db->prepare("INSERT INTO jurnal (SELECT NULL,'$masterNo',NOW(),REPLACE(keterangan,'Penjualan','CANCELLED'),total_debet,total_kredit,0,'$id_user',NOW() FROM jurnal WHERE jurnal.keterangan LIKE '%".$_GET['id']."%') ");
		$stmt->execute();
