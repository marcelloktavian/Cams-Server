...
$allow_delete = is_show_menu(DELETE_POLICY, OnlineSales, $group_acess);
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'posting') {
		...
		if($blocking <= 0)
		{
			...
			$dropshipper='';
			$namadropshipper='';
			$q = mysql_fetch_array( mysql_query("SELECT olnso.id_trans, olnso.total, olnso.transfer, olnso.tunai, olnso.deposit, olnso.id_dropshipper,mst_dropshipper.nama FROM olnso LEFT JOIN mst_dropshipper ON mst_dropshipper.id=olnso.id_dropshipper WHERE id_trans='".$_GET['id']."' LIMIT 1"));
			$tunai=$q['tunai'];
			$transfer=$q['transfer'];
			$deposit=$q['deposit'];
			$total=$q['total'];
			...
			// execute for master
			$sql_master="INSERT INTO `jurnal`(`no_jurnal`,`tgl`,`keterangan`, `total_debet`, `total_kredit`, `deleted`, `user`, `lastmodified`) VALUES ('$masterNo',NOW(),'Penjualan OLN Cash - $namadropshipper - $idtrans','$total','$total','0','$id_user',NOW()) ";
