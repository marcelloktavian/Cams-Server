...
include("../../include/koneksi.php");
	...
	if(isset($_POST['ref_code']) and !empty($_POST['ref_code'])){
		$sql_validasi="SELECT * FROM olnso WHERE (ref_kode IS NOT NULL OR ref_kode <> '0' OR ref_kode <> '' ) and ref_kode ='".$_POST['ref_code']."' ORDER BY id DESC";
		...
		$sql_validasi="SELECT * FROM olnso WHERE (exp_code IS NOT NULL OR exp_code <> '0' OR exp_code <> '' ) and exp_code ='".$_POST['exp_code']."' ORDER BY id DESC";
		//var_dump($sql_validasi);die;
		$rows = mysql_num_rows($sql_validasi);
		//var_dump($sql_validasi);	
		$sql = mysql_query($sql_validasi);
