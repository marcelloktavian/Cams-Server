...
error_reporting(0);
	...
	$where_title=" p.deleted=0 AND p.state='0' ";	
	/*
	if($filter != null)
	{
	$where_title .= " AND DATE(p.tgl_trans) BETWEEN STR_TO_DATE('$tglstart','%d/%m/%Y')  AND STR_TO_DATE('$tglend','%d/%m/%Y') AND ((j.nama like '%$filter%') or (p.nama like '%$filter%') or (e.nama like '%$filter%') or (p.exp_code like '%$filter%'))";
	}
	else
	{
	...
	*/
    ...
    (SELECT COUNT(p.id_trans) FROM olnsoreturn p Left Join `mst_dropshipper` j on (p.id_dropshipper=j.id) Left Join `mst_expedition` e on p.id_expedition=e.id WHERE  $where_title) AS jumlah_order,(SELECT p.tgl_trans FROM olnsoreturn p Left Join `mst_dropshipper` j on (p.id_dropshipper=j.id) Left Join `mst_expedition` e on p.id_expedition=e.id WHERE $where_title ORDER BY p.id_trans ASC LIMIT 1) AS first_date,
	(SELECT p.tgl_trans FROM olnsoreturn p Left Join `mst_dropshipper` j on (p.id_dropshipper=j.id) Left Join `mst_expedition` e on p.id_expedition=e.id WHERE  $where_title ORDER BY p.id_trans DESC LIMIT 1) AS last_date ";
...
<form id="form2" name="form2" action="" method="post"  onSubmit="return validasi(this)">
    ...
    <tr>
      <td height="123" valign="top"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
          ...
          <tr>
			...
			ONLINE RETURN REPORT </strong></td>
          ...
          <tr>
            ...
            <?php echo"".$rs_title['first_date'];?>
            &nbsp;-&nbsp;<?php echo"".$rs_title['last_date'];?> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Jumlah Retur:&nbsp;<?php echo"".$rs_title['jumlah_order'];?> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Nomor_awal:&nbsp;<?php echo"".$rs_title['first_order'];?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Nomor_akhir:&nbsp;<?php echo"".$rs_title['last_order'];?></td>           
		  </tr>
  ...
  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
	...
	$where = " WHERE (m.deleted=0) AND (m.state='0')";
	/*
	if($filter != null)
	{
	$where .= " AND DATE(m.tgl_trans) BETWEEN STR_TO_DATE('$tglstart','%d/%m/%Y')  AND STR_TO_DATE('$tglend','%d/%m/%Y') AND ((d.nama like '%$filter%') or (m.nama like '%$filter%') or (e.nama like '%$filter%') or (m.exp_code like '%$filter%'))";
	}
	...
	*/
	...
	$sql_detail = "SELECT dt.id_trans,m.tgl_trans, m.ref_kode AS id_web,m.exp_fee as ongkir,d.nama AS dropshipper,dt.namabrg,dt.jumlah_beli,dt.size,dt.harga_satuan,dt.subtotal,m.nama AS pembeli,e.nama AS expedition,m.state,d.disc as discdp,m.discount_faktur as disc_faktur,m.total,m.faktur FROM olnsoreturn_detail dt
