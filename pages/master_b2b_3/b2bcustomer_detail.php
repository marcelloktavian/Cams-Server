...
<table width='100%' cellspacing='0' cellpadding='0'>
     <tr>
		...
		<td><input type='text' class='inputform' name='nama' id='nama' 	placeholder='Nama B2B Customer'  />
	 ...
	 <tr>
		<td class='fonttext'>NO AKUN</td>
		<td>
			<select id='noakun' name='noakun' class='inputform'>
                <option value=''>-- Pilih No Akun --</option>";
				$query = '';
                $countnya = 0;
				$sql1 = 'SELECT a.* FROM `mst_coa` a where a.deleted=0 ORDER BY noakun ASC';
				$q1 = mysql_query($sql1);
				while($rs1=mysql_fetch_array($q1)){
					if ($countnya == 0) {
                        $query .= "select id, noakun, nama, jenis from mst_coa where id='".$rs1['id']."' ";
                    } else {
                        $query .= " UNION ALL select id, noakun, nama, jenis from mst_coa  where id='".$rs1['id']."' ";
                    }
                    $countnya++;
                    $sql2 = "SELECT * FROM det_coa WHERE id_parent='".$rs1['id']."' ORDER by noakun ASC";
					$q2 = mysql_query($sql2);
                    while($rs2=mysql_fetch_array($q2)){
                        $query .= " UNION ALL select '' as id, noakun, nama, '' as jenis from det_coa where id='".$rs2['id']."' ";
                    }
				}
				$q3 = mysql_query($query);
				while($rs3=mysql_fetch_array($q3)){
					echo "<option value='".$rs3['noakun']."'>".$rs3['noakun']." - ".$rs3['nama']."</option>";
				}
