...
$allow_post = is_show_menu(POST_POLICY, PreSALES, $group_acess);
$allow_delete = is_show_menu(DELETE_POLICY, PreSALES, $group_acess);
	...
	if(isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
       ...
       $operations = array(
        ...
        foreach($data1 as $line) {
		    ...
		    if($allow_post){
				$edit = '<a onclick="window.open(\''.BASE_URL.'pages/import_XLS/olnpreso_detail_edit.php?ids='.$line['oln_order_id'].'\',\'table_olnpreso\')" href="javascript:;">Posting</a>';
			}
			else
				$edit = '<a onclick="javascript:custom_alert(\'Tidak Boleh Posting Data\')" href="javascript:;">Posting</a>';
			
			if($allow_delete)
				$delete = '<a onclick="javascript:link_ajax(\''.BASE_URL.'pages/import_XLS/olnpreso.php?action=delete&id='.$line['oln_order_id'].'\',\'table_olnpreso\')" href="javascript:;">Cancel</a>';
			else
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'posting') {
		...
		$stmt = $db->prepare("Update olnpreso set state='1',lastmodified=now() WHERE oln_order_id=?");
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'delete') {
		...
		$stmt = $db->prepare("DELETE FROM olnpreso WHERE oln_order_id=?");
		$stmt->execute(array($_GET['id']));
		//var_dump($stmt);die;
...
--> 
<table id="table_olnpreso"></table>
<div id="pager_table_olnpreso"></div>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
		...
		//alert('kdie');
        $("#table_olnpreso").jqGrid({
            url:'<?php echo BASE_URL.'pages/import_XLS/olnpreso.php?action=json'; ?>',
            datatype: "json",
            ...
            colModel:[
                ...
                {name:'oln_customer',index:'p.oln_customer', width:35, search:true, stype:'text', searchoptions:{sopt:['cn']}},
            ...
            subGrid : true,
            subGridUrl : '<?php echo BASE_URL.'pages/import_XLS/olnpreso.php?action=json_sub'; ?>',
            subGridModel: [
			            	{ 
        ...
        $("#table_olnpreso").jqGrid('navGrid','#pager_table_olnpreso',{edit:false,add:false,del:false,search:true});
