...
//--------------end of searching--------------------		
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'process') {
		if(isset($_POST['id'])) {
			$stmt = $db->prepare("UPDATE mst_b2bsalesman SET nama=?,alamat=?,no_telp=?,disc=?,type=?,user=?, lastmodified = NOW() WHERE id=?");
			$stmt->execute(array($_POST['nama'],$_POST['alamat'],$_POST['no_telp'],$_POST['disc'],$_POST['tipe'], $_SESSION['user']['username'], $_POST['id']));
			//var_dump($stmt);die;
			$affected_rows = $stmt->rowCount();
			if($affected_rows > 0) {
				$r['stat'] = 1;
			...
			$stmt = $db->prepare("INSERT INTO  mst_b2bsalesman(`nama`,`alamat`,`no_telp`,`disc`,`type`,`user`,`lastmodified`) VALUES(?, ?, ?, ?, ?, ?,NOW())");
			if($stmt->execute(array($_POST['nama'],$_POST['alamat'], $_POST['no_telp'], $_POST['disc'], $_POST['tipe'],$_SESSION['user']['username']))) {
				$r['stat'] = 1;
				$r['message'] = 'Success';
			}
			else {
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#table_b2bsalesman").jqGrid({
            ...
            datatype: "json",
            colNames:['ID','Name','Address','Phone','Disc','Type','Edit','Delete'],
            colModel:[
