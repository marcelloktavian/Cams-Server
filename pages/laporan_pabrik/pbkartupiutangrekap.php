...
error_reporting(0);
	...
	$sql = mysql_query("SELECT id,id_cust,namaperusahaan,substring(id_cust,3,3)as nopel FROM tblsupplier where  id= ".$id_pelanggan);
...
<form id="form2" name="form2" action="" method="post"  onSubmit="return validasi(this)">
    ...
    <tr>
      <td height="123" valign="top"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
          ...
          <tr>
			...
			<a href="javascript:window.print()">CETAK</a>&nbsp;&nbsp;<?echo"REKAP KARTU PIUTANG Tahun ". date('Y');?> </strong></td>
          </tr>
          <tr>
            ...
            <td class="style9" width="19%" ><a href="pbkartupiutangrekapxl.php?start=<?php echo"".$tglstart;?>&end=<?php echo"".$tglend;?>&id=<?php echo"".$id_pelanggan;?>">Export ke Excell</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Dari</td>
  ...
  <table width="100%" id="tbd" border="0" align="center" cellpadding="0" cellspacing="0">
	...
	$where .= " AND DATE(tgl_trans) BETWEEN STR_TO_DATE('$tglstart','%d/%m/%Y')   AND STR_TO_DATE('$tglend','%d/%m/%Y') AND id_supplier=$id_pelanggan";
	}
	else
	{
	...
	,jual.piutang_faktur,jual.piutang_tunai,jual.piutang_transfer,jual.piutang_bayar from tblsupplier b 
	left join 
	(Select id_supplier,sum(faktur_murni) as faktur,sum(biaya) as biaya,sum(tunai) as tunai,sum(transfer) as transfer,sum(totalfaktur) as totalfaktur
	...
	from (Select tgl_trans,id_supplier,id_trans,kode,faktur_murni,biaya,tunai,transfer,totalfaktur,(totalfaktur-piutang) as bayar_faktur,piutang as piutang_faktur,'' as piutang_tunai ,'' as piutang_transfer,'' as piutang_bayar from trbeli ".$where."
	union all
	Select tgl_trans,id_supplier,id_trans,kode,'','','','','','','',tunai,transfer,totalfaktur from trbelipiutang ".$where.") temp  
	group by id_supplier
	) as jual on b.id = jual.id_supplier 
	where (b.type = 2) and (b.id <> 0) order by b.id_cust asc";
	...
	$totalbayar=0;
	$saldoakhir=0;
	$konversi=30*1.11;//ditambah ppn 10%
	...
	{ 
	  ...
	  //$kode=$rs2['kode'];
	  //$keterangan="JUAL";
	  $piutang_awal=$rs2['faktur']*$konversi;
	  $ongkos_kuli=$rs2['biaya'];
	  $jumlah_piutang=$rs2['totalfaktur']*$konversi;
	  $kas="";//dianggap ke bank semua
	  //$bca=($rs2['tunai']+$rs2['transfer']+$rs2['piutang_tunai'] + $rs2['piutang_transfer'])*$konversi;
	  $bca=($rs2['tunai']+$rs2['transfer'])*$konversi+$rs2['piutang_tunai'] + $rs2['piutang_transfer'];
	  $total_bayar=($rs2['tunai']+$rs2['transfer'])*$konversi+$rs2['piutang_tunai'] + $rs2['piutang_transfer'];
	  $saldo_akhir=($rs2['totalfaktur']*$konversi-($rs2['tunai']+$rs2['transfer'])*$konversi-$rs2['piutang_tunai']-$rs2['piutang_transfer']);
