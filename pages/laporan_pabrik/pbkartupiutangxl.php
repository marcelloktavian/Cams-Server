...
<form id="form2" name="form2" action="" method="post"  onSubmit="return validasi(this)">
    ...
    <tr>
      <td height="123" valign="top"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
          ...
          <tr>
			...
			<a href="javascript:window.print()">CETAK</a> KARTU PIUTANG DETAIL PABRIK</strong></td>
          </tr>
          <tr>
  ...
  <table width="100%" id="tbd" border="0" align="center" cellpadding="0" cellspacing="0">
	...
	$sql_union="
	Select 'JUAL' as trans,'' as id_transjual,tgl_trans,date_format(tgl_trans,'%d-%m-%Y')as tgl_edit,id_supplier, id_trans,kode,faktur_murni,biaya,tunai,transfer,totalfaktur,(totalfaktur-piutang) as bayar_faktur,piutang as piutang_faktur,'' as piutang_tunai ,'' as piutang_transfer,'' as piutang_bayar from trbeli ".$where."   
    union all
	Select 'JUALBAYAR' as trans,'' as id_transjual,tgl_trans,date_format(tgl_trans,'%d-%m-%Y')as tgl_edit,id_supplier,id_trans,kode,faktur_murni,biaya,tunai,transfer,totalfaktur,(totalfaktur-piutang) as bayar_faktur,piutang as piutang_faktur,'' as piutang_tunai ,'' as piutang_transfer,'' as piutang_bayar from trbeli ".$where." and ((tunai+transfer)>0)
    union all
    Select 'PIUTANG' as trans,id_transjual,tgl_trans,date_format(tgl_trans,'%d-%m-%Y')as tgl_edit,id_supplier,id_trans,kode,'','','','','','','',tunai,transfer,totalfaktur from trbelipiutang ".$where."  
	...
	{ 
	  ...
	  $kode=$rs2['kode'];
	  $keterangan="JUAL";
	  $piutang_awal=$rs2['faktur_murni']*$konversi;
	  ...
	  */
	  ...
	  $jumlah_piutang="";
	  $kas="";//dianggap ke bank semua
	  $bca=($rs2['tunai']+$rs2['transfer'])*$konversi;
	  $total_bayar=($rs2['tunai']+$rs2['transfer'])*$konversi;
	  $tmp= -($total_bayar);	  	  
	  }
