...
<form id="form2" name="form2" action="" method="post"  onSubmit="return validasi(this)">
  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
    <tr>
      <td height="123" valign="top"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
          ...
          <tr>
            <td width="19%" class="style9"><a href="accreport_jualpbblmbyrxl.php?start=<?php echo"".$tglstart;?>&end=<?php echo"".$tglend;?>&id=<?php echo"".$id_pelanggan;?>">Export ke Excell</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
  ...
  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
	...
	$filtertgl = "";
	//dicut off dari bulan 01.05.2018
	$filtertgl = "AND DATE(tgl_trans) >= STR_TO_DATE('01/05/2018','%d/%m/%Y') AND DATE(tgl_trans) <= STR_TO_DATE('$tglend','%d/%m/%Y') ";
	...
	//$sql_data= "select b.namaperusahaan,a.id_trans,a.kode,b.id_cust, date_format(a.tgl_trans,'%d-%m-%Y') as tgl_trans, a.totalqty,a.faktur_murni, a.totalfaktur, a.tunai,a.transfer,a.piutang,(a.piutang-a.pelunasan) as saldo,a.biaya from trbeli a left join tblsupplier b on b.id = a.id_supplier where ((a.piutang-a.pelunasan) > 0)  and b.type=2 and a.id_supplier <>0 ".$where." order by b.id_cust asc";
	
	$sql_data="Select b.namaperusahaan,j.id_trans,j.kode,b.id_cust,date_format(j.tgl_trans,'%d-%m-%Y') as tgl_trans,j.totalqty,j.faktur,j.totalfaktur,j.tunai,j.transfer,j.piutang,j.pelunasan,ifnull(byr.piutangbyr,0) as piutangbyr,(j.piutang*30*1.11 -(ifnull(byr.piutangbyr,0))) as saldo,j.biaya from trbeli j left join tblsupplier b on j.id_supplier=b.id left join (select p.id_transjual,sum(p.totalfaktur) as piutangbyr from trbelipiutang p where TRUE ".$filtertgl." group by p.id_transjual) as byr on j.id_trans=byr.id_transjual where (j.piutang*30*1.11 -(ifnull(byr.piutangbyr,0))) > 0 ".$where." and j.id_supplier <> 0 and j.faktur=1 and j.deleted=0 order by j.tgl_trans desc";
