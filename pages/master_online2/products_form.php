           ...
           <label for="size" class="ui-helper-reset label-control">Size</label>
            <div class="ui-corner-all form-control">
                <select name="size" id="size">
                	...
                	<?php
                		$query = $db->query("SELECT * FROM mst_size ORDER BY id ASC");
						$rows = $query->fetchAll(PDO::FETCH_ASSOC);
						foreach($rows as $r) {
							$select = isset($row['size']) && $row['size'] == $r['size'] ? 'selected' : ''; 
							echo '<option '.$select.' value="'.$r['size'].'">'.$r['size'].'</option>';
						}
                	?>
                </select>
            </div>

            <label for="brand" class="ui-helper-reset label-control">Brand</label>
            <div class="ui-corner-all form-control">
            <select class="required" name="brand" id="brand">
                	<option value="">-choose(pilih)-</option>
                	<?php
                		$query = $db->query("SELECT * FROM mst_brand ORDER BY nama ASC");
						$rows = $query->fetchAll(PDO::FETCH_ASSOC);
						foreach($rows as $r) {
							$select = isset($row['brand']) && $row['brand'] == $r['id'] ? 'selected' : ''; 
            ...
            <label for="tipe" class="ui-helper-reset label-control">Limit Stok</label>
            <div class="ui-corner-all form-control">
                <input value="<?php echo isset($row['limit_stok']) ? $row['limit_stok'] : ''; ?>" type="text" class="required" id="limit" name="limit">   
            </div>
