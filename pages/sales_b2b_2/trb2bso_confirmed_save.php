...
include("../../include/koneksi.php");
	...
	if ($num_rows == 0) {
		$sqlno1 = "SELECT CONCAT('AK',SUBSTRING(YEAR(NOW()),3,2),'00001') AS no_faktur";
		$sqno1 = mysql_query($sqlno1);
		while($rsno1=mysql_fetch_array($sqno1)){
			$no_faktur = $rsno1['no_faktur'];
		}
	}else{
		$sqlno1 = "SELECT IF(SUBSTRING(MAX(no_faktur),3,2) = SUBSTRING(YEAR(NOW()),3,2), CONCAT('AK',SUBSTRING(YEAR(NOW()),3,2),IF(LENGTH(SUBSTRING(MAX(no_faktur),5,5)+1)=1,CONCAT('0000',SUBSTRING(MAX(no_faktur),5,5)+1),IF(LENGTH(SUBSTRING(MAX(no_faktur),5,5)+1)=2,CONCAT('000',SUBSTRING(MAX(no_faktur),5,5)+1),IF(LENGTH(SUBSTRING(MAX(no_faktur),5,5)+1)=3,CONCAT('00',SUBSTRING(MAX(no_faktur),5,5)+1),IF(LENGTH(SUBSTRING(MAX(no_faktur),5,5)+1)=4,CONCAT('0',SUBSTRING(MAX(no_faktur),5,5)+1),CONCAT(SUBSTRING(MAX(no_faktur),5,5)+1)) ) ) )),CONCAT('AK',SUBSTRING(YEAR(NOW()),3,2),'00001')) AS no_faktur FROM b2bdo LIMIT 1";
		$sqno1 = mysql_query($sqlno1);
		while($rsno1=mysql_fetch_array($sqno1)){
			$no_faktur = $rsno1['no_faktur'];
		}
	...
	//--- simpan master -----------------------------
	$sql_master="";
	$sql_master="insert into b2bdo(id_trans,id_transb2bso,no_faktur,tgl_trans,id_salesman,id_customer,totalfaktur,faktur,piutang,totalkirim,id_address,id_expedition,exp_code,exp_note,exp_fee,note,user)values('".$id_pkb."','".$id_transb2bso."','".$no_faktur."','".$tglkirim."','".$id_salesman."','".$id_customer."','".$totalfaktur."','".$faktur."','".$totalfaktur."','".$totalkirim."','".$id_address."','".$id_expedition."','".$exp_code."','".$exp_note."','".$exp_fee."','".$note."','".$user."')";
