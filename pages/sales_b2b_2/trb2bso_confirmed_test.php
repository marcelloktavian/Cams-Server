...
$allow_print = is_show_menu(PRINT_POLICY, ConfirmedSales, $group_acess);
...
if(isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
  ...
  if ($_REQUEST["_search"] == "false") {
    ...
    $where = "WHERE TRUE AND p.state='1' AND (p.deleted=0) AND (p.totalqty <> 0)";
} else {
 $operations = array(
        'eq' => "= '%s'",            // Equal
        'ne' => "<> '%s'",           // Not equal
...
elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'json_sub') {
  ...
  $where = "WHERE do.id_transb2bso = '".$id."' ";
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#table_b2bso_confirmed").jqGrid({
            ...
            datatype: "json",
            colNames:['ID','Code','Customer','Date','Salesman','Address','Exp.Fee','Expedition','CAT','Qty','Sent','Detail','Send'],
            colModel:[
