...
// $allow_print = is_show_menu(PRINT_POLICY, OnlineDelivery, $group_acess);
...
if(isset($_GET['action']) && strtolower($_GET['action']) == 'json') {
	...
	$filter=$_GET['filter'];
        ...
        foreach($data1 as $line) {
        	...
        	if($line['stkirim'] == '1') {
        		if(in_array($_SESSION['user']['access'], $allowPack)) {
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'delete') {
		...
		$q = $db->query("SELECT sodet.id_product FROM `olnsodetail` sodet where sodet.id_trans='".$_GET['id']."'");
		$data1 = $q->fetchAll(PDO::FETCH_ASSOC);
		foreach($data1 as $line){

            //update inventory balance
			$stmt1 = $db->query("UPDATE inventory_balance ib SET ib.stok=IFNULL(ib.stok,0)-(SELECT SUM(i.qty) FROM inventory i  WHERE i.id_product='". $line['id_product']."' and i.id_trans='".$_GET['id']."'),ib.lastmodified=NOW() where ib.id='". $line['id_product']."'");

			// var_dump("UPDATE inventory_balance ib SET ib.stok=IFNULL(ib.stok,0)-(SELECT SUM(i.qty) FROM inventory i  WHERE i.id_product='". $line['id_product']."' and i.id_trans='".$_GET['id']."'),ib.lastmodified=NOW() where ib.id='". $line['id_product']."'");
		}
		...
		//delete inventory
		$stmt2 = $db->query("DELETE FROM inventory where id_trans='".$_GET['id']."'");

		//delete olnso_id
		$stmt2 = $db->query("DELETE FROM olnso_id where id_trans='".$_GET['id']."'");
				...
				<div class="ui-corner-all form-control">
					<table>
						<tr>
							...
							<td> Filter
								<input value="" type="text" id="filterdo" name="filterdo">(Dropshipper,Receiver,Expedition,Exp_Code)
							</td>
						</tr>
					</table>
				</div>
