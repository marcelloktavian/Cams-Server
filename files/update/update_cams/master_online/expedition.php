...
//--------------end of searching--------------------		
     //   $where = "WHERE deleted=0 ";
        $q = $db->query("SELECT * FROM `mst_expedition` a ".$where);
		$count = $q->rowCount();
        ...
        $q = $db->query("SELECT * FROM `mst_expedition` a ".$where." ORDER BY `".$sidx."` ".$sord." LIMIT ".$start.", ".$limit);
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'process') {
		if(isset($_POST['id'])) {
			$stmt = $db->prepare("UPDATE mst_expedition SET kode=?,nama=?,kode_warna=?,user=?, lastmodified = NOW() WHERE id=?");
			...
			$stmt = $db->prepare("INSERT INTO  mst_expedition(`kode`,`nama`,`kode_warna`,`user`,`lastmodified`) VALUES( ?,  ?,  ?,  ?, NOW())");
			//var_dump($stmt);die;
