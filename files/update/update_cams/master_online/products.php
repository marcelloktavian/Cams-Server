...
//--------------end of searching--------------------		
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'delete') {
		$stmt = $db->prepare("UPDATE mst_products SET deleted=? WHERE id=?");
	...
	elseif(isset($_GET['action']) && strtolower($_GET['action']) == 'process') {
		if(isset($_POST['id'])) {
			$stmt = $db->prepare("UPDATE mst_products SET kode=?,nama=?,harga=?,type=?,id_category=?,user=?, lastmodified = NOW() WHERE id=?");
			$stmt->execute(array(strtoupper($_POST['kode']),strtoupper($_POST['nama']),$_POST['harga'],strtoupper($_POST['type']),$_POST['id_category'], $_SESSION['user']['username'], $_POST['id']));
			//var_dump($stmt);die;
			$affected_rows = $stmt->rowCount();
			if($affected_rows > 0) {
				$r['stat'] = 1;
			...
			$stmt = $db->prepare("INSERT INTO  mst_products(`kode`,`nama`,`harga`,`type`,`id_category`,`user`,`lastmodified`) VALUES(?, ?, ?, ?, ?, ?,NOW())");
			if($stmt->execute(array(strtoupper($_POST['kode']),strtoupper($_POST['nama']), $_POST['harga'],strtoupper($_POST['type']),$_POST['id_category'],$_SESSION['user']['username']))) {
				$r['stat'] = 1;
				$r['message'] = 'Success';
			}
			else {
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#table_products").jqGrid({
            ...
            datatype: "json",
            colNames:['ID','Code','Name','Price','Type','Category','Edit','Delete'],
            colModel:[
